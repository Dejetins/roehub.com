...
def _build_payload_v1(
    *,
    ...
    sorted_indicators = tuple(sorted(indicators, key=lambda item: item.indicator_id))
    ids = [item.indicator_id for item in sorted_indicators]
        ...
        serialized.append(
            {
                ...
                "inputs": _sorted_pairs(item.inputs),
                "params": _sorted_pairs(item.params),
            }
        )
