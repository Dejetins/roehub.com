"""
NumPy oracle implementation for structure/normalization indicators.
...
def is_supported_structure_indicator(*, indicator_id: str) -> bool:
    """
    Return whether indicator id is supported by structure oracle implementation.
    ...
    Returns:
        bool: True when id is supported by structure oracle.
    Assumptions:
...
def compute_structure_grid_f32(
    *,
    ...
    """
    Compute structure indicator matrix `(V, T)` using pure NumPy/Python loops.
    ...
    if normalized_id in _CANDLE_STATS_OUTPUT_MODE:
        open_f64 = _prepare_series(name="open", values=open)
...
def _candle_stats_series_f64(
    ...
    for time_index in range(t_size):
        ...
        if (
            math.isnan(open_value)
            or math.isnan(high_value)
            or math.isnan(low_value)
        ...
        upper_wick_value = high_value - max(open_value, close_value)
        lower_wick_value = min(open_value, close_value) - low_value
...
def _pivot_series_f64(*, values: np.ndarray, left: int, right: int, mode: int) -> np.ndarray:
    ...
    Raises:
        ValueError: If left/right are non-positive.
    Side Effects:
        Allocates one output array.
    """
    ...
    for time_index in range(t_size):
        ...
        for idx in range(start, end + 1):
            ...
            if idx == center:
                continue
            if mode == 0 and value >= candidate:
                valid = False
                break
            if mode == 1 and value <= candidate:
                valid = False
                break
...
def _zscore_variants_f64(source_variants: np.ndarray, windows: np.ndarray) -> np.ndarray:
    ...
    for variant_index in range(variants):
        ...
        mean = _rolling_mean_series_f64(source=series, window=window)
        ...
        for time_index in range(t_size):
            ...
            if (
                ...
                or math.isnan(mean_value)
                or math.isnan(variance_value)
...
def _candle_stats_atr_norm_variants_f64(
    ...
    for variant_index in range(variants):
        for time_index in range(t_size):
            ...
            if math.isnan(raw_value) or math.isnan(atr_value) or atr_value == 0.0:
                out[variant_index, time_index] = np.nan
            else:
...
def _pivots_variants_f64(
    ...
    for variant_index in range(variants):
        out[variant_index, :] = _pivot_series_f64(
            values=values,
            left=int(lefts[variant_index]),
            right=int(rights[variant_index]),
            mode=mode,
        )
...
def _distance_to_ma_norm_variants_f64(
    ...
    for variant_index in range(variants):
        source = source_variants[variant_index, :]
        ema = _ema_series_f64(source=source, window=int(windows[variant_index]))
