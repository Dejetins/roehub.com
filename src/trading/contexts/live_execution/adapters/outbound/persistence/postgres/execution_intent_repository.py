...
class PostgresExecutionIntentRepository(ExecutionIntentRepository):
    ...
    def record_source_event(self, *, event: ExecutionSourceEvent) -> ExecutionSourceEvent:
        row = self._gateway.fetch_one(
            ...
            (
                source_event_id, organization_id, owner_user_id, source_type, source_event_ref,
                source_ref_json, strategy_signal_id, idempotency_key_hash,
                outcome, outcome_reason, intent_id, received_at
            )
            VALUES
            (
                %(source_event_id)s, %(organization_id)s, %(owner_user_id)s, %(source_type)s,
            ...
            ON CONFLICT (organization_id, owner_user_id, source_type, idempotency_key_hash)
            DO NOTHING
            RETURNING *
            """,
    ...
    def record_intent(self, *, intent: ExecutionIntent) -> ExecutionIntent:
        row = self._gateway.fetch_one(
            ...
            (
                intent_id, source_event_id, organization_id, owner_user_id, source_type,
                ...
                idempotency_key_hash, constraints_json, canonical_intent_hash,
                created_at, dispatch_attempt_count,
                dispatch_stream_name, dispatch_redis_message_id, dispatch_last_error,
                dispatch_updated_at
            )
            VALUES
            (
                %(intent_id)s, %(source_event_id)s, %(organization_id)s, %(owner_user_id)s,
                ...
                %(idempotency_key_hash)s, %(constraints_json)s::jsonb,
                %(canonical_intent_hash)s, %(created_at)s, %(dispatch_attempt_count)s,
                %(dispatch_stream_name)s, %(dispatch_redis_message_id)s,
                %(dispatch_last_error)s, %(dispatch_updated_at)s
            )
            ON CONFLICT (organization_id, owner_user_id, idempotency_key_hash) DO NOTHING
            RETURNING *
            """,
    ...
    def record_risk_audit_event(
        ...
        row = self._gateway.fetch_one(
            ...
            (
                event_id, intent_id, source_event_id, organization_id, owner_user_id, source_type,
                event_type, risk_status, risk_reason, check_name, metadata_json, created_at
            )
            VALUES
            (
                %(event_id)s, %(intent_id)s, %(source_event_id)s, %(organization_id)s,
                %(owner_user_id)s,
    ...
    def record_notification_outbox(
        ...
        row = self._gateway.fetch_one(
            ...
            (
                notification_id, organization_id, owner_user_id, source_type, event_type, severity,
                reason, source_event_id, intent_id, order_id, strategy_signal_id,
                labels_json, status, created_at, sent_at
            )
            VALUES
            (
                %(notification_id)s, %(organization_id)s, %(owner_user_id)s, %(source_type)s,
            ...
            ON CONFLICT (
                organization_id, owner_user_id, event_type,
                source_event_key, intent_key, order_key, reason
            ) DO NOTHING
            RETURNING *
            """,
        ...
        if strategy_id is None:
            ...
            rows = self._gateway.fetch_all(
                ...
                WHERE n.organization_id = %(organization_id)s
                  AND n.owner_user_id = %(owner_user_id)s
                  AND (
        ...
        rows = self._gateway.fetch_all(
            ...
            LEFT JOIN LATERAL (
                ...
                WHERE ef.order_id = o.order_id
                  AND ef.organization_id = o.organization_id
            ...
            LEFT JOIN LATERAL (
                ...
                WHERE er.order_id = o.order_id
                  AND er.organization_id = o.organization_id
            ...
            LEFT JOIN LATERAL (
                ...
                WHERE n.organization_id = e.organization_id
                  AND (
                    n.source_event_id = e.source_event_id
            ...
            WHERE e.organization_id = %(organization_id)s
              AND e.owner_user_id = %(owner_user_id)s
...
def _intent_params(intent: ExecutionIntent) -> dict[str, object]:
    return {
        ...
        "idempotency_key_hash": intent.idempotency_key_hash,
        "constraints_json": json.dumps(dict(intent.constraints), sort_keys=True),
        "canonical_intent_hash": intent.canonical_intent_hash,
...
def _map_intent(row: Mapping[str, Any]) -> ExecutionIntent:
    return ExecutionIntent(
        ...
        constraints=_json_mapping(row.get("constraints_json")),
        canonical_intent_hash=str(row.get("canonical_intent_hash") or ""),
