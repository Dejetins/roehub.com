...
class CapitalReservationPaperAccountingService:
    ...
    def record_rl_paper_execution(
        ...
        *,
        ...
        if side not in {"buy", "sell"}:
            raise CapitalReservationBlockedError(reason="paper_rl_invalid_side")
        if quote_notional <= 0:
            raise CapitalReservationBlockedError(reason="paper_rl_invalid_notional")
        if reference_price <= 0:
        ...
        order = PaperOrder(
            ...
            reference_price=reference_price,
            status="filled",
            reason="paper_market_fill_from_ml_agent_decision",
            created_at=now,
        )
        fill = PaperFill(
            paper_fill_id=_stable_uuid(
