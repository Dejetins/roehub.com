...
The canonical Figma file defines implementable four-theme Roehub foundations,
components, shell, resizable panels, system states, and the read-only Backtest
list/detail golden slice.

## Start probe

Use `figma-use` for every Figma operation, `figma-generate-library` for variables,
tokens, and components, and `figma-generate-design` for application layouts.
Confirm the authenticated account, professional plan, project `roehub.com`
(`projectId` `629113387`), and canonical file `Roehub Authenticated Platform UI`
...
- the canonical Roehub authenticated-platform Figma file;
...
the file key, page/node IDs, before/after inventories, variable collections and
modes, component/variant matrix, four-theme matrix, mapping-table coverage,
visual QA, and the explicit absence of browser/runtime proof.

## Acceptance

All design targets pass structural and visual checks without orphan or duplicate
