...
WORKTREE_ADD_PATTERN = re.compile(rf"{GIT}\s+worktree\s+add\b", re.IGNORECASE)
...
def validate(payload: dict[str, Any]) -> list[Finding]:
    ...
    if created and BRANCH_APPROVAL_MARKER not in command:
        findings.append(
            Finding(
                severity=FATAL_BLOCK,
                ...
                message=(
                    ...
                    f"`{BRANCH_APPROVAL_MARKER}` in the command."
                ),
                validator="branch_workflow_guard",
                target=", ".join(created),
            )
        )
    if WORKTREE_ADD_PATTERN.search(command) and WORKTREE_APPROVAL_MARKER not in command:
        findings.append(
            Finding(
                severity=FATAL_BLOCK,
