...
context_sources:
  always_read:
    ...
    - path: <latest_executor_final_report_or_equivalent_if_available>
  ...
  conditional_bundles:
    ...
    <browser_surface_bundle_if_applicable>:
      read_when: "<only if browser-visible behavior, routes, forms, rendered defaults, or DOM state are relevant>"
      paths:
        - <browser_related_path_a>
        - <browser_related_path_b>
...
hard_requirements:
  ...
  <browser_verification_required_if_applicable>: true

task_toggles:
  <toggle_key_1>: true
  <toggle_key_2>: true
  <toggle_key_3>: true
  <browser_visible_task_if_applicable>: <true|false>
...
State the goal, the intended change, the main invariants, the exact boundaries, and whether browser-visible behavior is in scope.
...
If browser-visible behavior is relevant:
- use browser-related bundles only when runtime verification or browser-surface impact is actually in scope
- do not turn browser verification into broad exploratory browsing
...
For browser-visible tasks, add one explicit step equivalent to:
- Verify the relevant route, UI state, form flow, or browser-visible default with an available browser surface.
...
If browser-visible behavior is in scope, also include acceptance criteria equivalent to:
- the relevant browser-visible behavior is verified through runtime browser verification,
- runtime/browser evidence is distinguished from static code reasoning,
- no browser-visible claim is presented as fact without verification.
...
- If browser-visible behavior changes, treat that as an operational contract surface when relevant.
...
- Use browser automation only for task-relevant routes, pages, controls, or flows.
- Keep browser automation narrow and purposeful.
- Distinguish clearly between:
  - browser-observed runtime behavior,
  - test evidence,
  - code inference,
...
- a runtime browser verification requirement in the work plan and acceptance criteria, or
- an explicit runtime/browser validation note when a formal command-style gate is not appropriate.
...
For browser-visible tasks, the report SHOULD also make clear:
- what was verified through runtime browser verification,
- what was verified through tests,
