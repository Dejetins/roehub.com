---
prompt_name: 10a-retraining-promotion-lifecycle
repo: roehub.com
branch: main
branch_policy:
  default_branch: main
...
context_sources:
  ...
  task_entrypoints:
    ...
    - path: docs/runbooks/mac-studio-native-backend-operations.md
      why: "operator command/runbook style"
      inspect_symbols:
        - "host-local operations"
    - path: configs/dev
      why: "scheduled trigger config style"
      inspect_symbols:
        - "disabled by default"
...
skill_routing:
  ...
  - skill: publish-ci-deploy
    use_when: "operator commands or scheduler config need target-host evidence"
    timing: "before ship"
...
validation_strategy:
  ...
  acceptance_surfaces:
    ...
    - "promotion profile"
    - "host-local rollback command"
    - "no-auto-activation tests"
  tests_only_allowed_reason: ""
  evidence_target: docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/10a-retraining-promotion-lifecycle.md
...
- Numeric promotion scorecard profile exists, even if thresholds are initially configured as soft/warn.
- Candidate never auto-promotes; promotion requires all gates plus operator/admin confirmation.
...
- open_items:
  - Make model lifecycle production-governed without user-controlled training or in-place live mutation.
...
- Verify the stage prerequisites before implementation. Required accepted prerequisites: Stage 08, Stage 09B, Stage 10. If any required prerequisite is not accepted in the ledger, stop, write/update the stage report as blocked, update the ledger, and do not implement dependent work.
...
- Keep the change bounded to Stage `10A`. Do not start later stages or silently repair unrelated legacy issues.
- Update or create the stage report at `docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/10a-retraining-promotion-lifecycle.md` using the Stage 01 report shape: scope, file manifest, observed state, contract impact, quality gates, evidence, blockers, and next-stage handoff.
...
- Both full retrain and fine-tune modes must be available.
- Drift signals may create/run candidate training but must not auto-promote or mutate live champion in place.
- Rollback to previous accepted champion/calibration must have a host-local command and internal API/use-case test.
- Promotion scorecard must include PnL after fees/funding/slippage, max drawdown, trades count, stability by ticker, out-of-sample period, overfit signs, latency/resource budget, and artifact/registry integrity.
- Web UI controls are not required before Stage 11; any Stage 10A operator command is host-local or internal and guarded.
...
- `publish-ci-deploy`: use before ship when operator commands or scheduler config need target-host evidence; records CI/deploy/Mac Studio verification after GitHub publication or merge.

1. Read the required context using the bounded protocol and verify Stage `10A` prerequisites in the ledger.
2. Record `User required before start`, prompt path/hash, planned concrete file list, and any initial blockers in `docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/10a-retraining-promotion-lifecycle.md`.
...
- Deterministic rerun evidence and run/config hashes are recorded.
- Schedule disabled-by-default proof is recorded.
- Candidate no-auto-activation and rollback tests pass.
- Stage 11 can expose authorized controls without inventing lifecycle semantics.
- The stage report exists at `docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/10a-retraining-promotion-lifecycle.md` and includes prompt path/hash plus a strict file manifest.
