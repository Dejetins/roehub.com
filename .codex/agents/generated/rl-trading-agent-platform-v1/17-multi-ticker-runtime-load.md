---
prompt_name: 17-multi-ticker-runtime-load
repo: roehub.com
branch: main
branch_policy:
  default_branch: main
...
context_sources:
  ...
  task_entrypoints:
    - path: apps/worker/strategy_live_runner/main/main.py
      why: "existing supervised worker pattern"
      inspect_symbols:
        - "worker main"
        - "runtime config"
...
skill_routing:
  ...
  - skill: backend-performance-evidence
    use_when: "building/running load and latency evidence"
    timing: "during verification"
    reason: "owns comparable benchmark methodology"
...
validation_strategy:
  ...
  evidence_target: docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/17-multi-ticker-runtime-load.md
...
Implement Stage 17 multi-ticker runtime/load gate. Prove quotas, inference scheduling, live feed handling, rate limits, latency, resource use, and no DLQ/Redis lag growth across tariff-like ticker counts on Mac Studio.

Done means:

- Load harness exercises Free/Pro/Premium-like active ticker counts within supported markets.
- p95 segment latency, CPU/MPS/RSS, Redis lag, queue/DLQ growth, and model decision throughput are recorded.
- Resource contention with training/backtest jobs is bounded or explicitly blocked.
...
- open_items:
  - Show Mac Studio can sustain planned RL inference before long soak and mainnet review.
...
- Update or create the stage report at `docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/17-multi-ticker-runtime-load.md` using the Stage 01 report shape: scope, file manifest, observed state, contract impact, quality gates, evidence, blockers, and next-stage handoff.
...
- Do not reduce safety/audit/risk gates to meet latency; remove hot-path blocking work instead.
- Measure concurrent training/backtest contention if those processes can overlap.
- Record degraded behavior when feed gaps, model load failure, or disk watermark occurs.
- No mainnet submit is allowed.
...
- `backend-performance-evidence`: use during verification when building/running load and latency evidence; owns comparable benchmark methodology.
...
1. Read the required context using the bounded protocol and verify Stage `17` prerequisites in the ledger.
2. Record `User required before start`, prompt path/hash, planned concrete file list, and any initial blockers in `docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/17-multi-ticker-runtime-load.md`.
