---
prompt_name: 07e-original-hf-full-training-run
repo: roehub.com
branch: main
branch_policy:
  default_branch: main
...
context_sources:
  always_read:
    ...
    - path: docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/rl-trading-agent-platform-v1-stage-ledger.md
      why: "stage ledger"
    - path: docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/07d-upstream-methodology-core-port.md
...
required_literals:
  ...
  - "07E"
non_goals:
  - "Do not train on Roehub-native Stage 06 data in Stage 07E."
  - "Do not run Stage 08A evaluation inside Stage 07E."
...
validation_strategy:
  ...
  evidence_target: docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/07e-original-hf-full-training-run.md
...
expected_primary_touches:
  ...
  - "docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/07e-original-hf-full-training-run.md"
...
Implement Stage `07E` original HF full training run.

Train the upstream-compatible Roehub agent on the original external HF dataset splits using the accepted Stage `07D` core. This is the reproducibility gate before Roehub-native training.
...
- Previous-stage ledger gate: before any edits or training launch, read the ledger and verify Stage `07D` is `accepted`, Stage `04` is `accepted`, and `current_stage` allows Stage `07E`. If not true, write/update the Stage `07E` report as `blocked`, update the ledger, and stop.
...
- If full training cannot complete in the active session, launch only through a managed resumable/background path, prove a fresh `running` event and leave Stage `07E` `in_progress`. Do not mark accepted until full training completes.
...
- Do not run test/backtest acceptance here; Stage `08A` owns evaluation.
...
- Ledger advances to `08A` only after completed `hf_original_candidate` evidence.

## Final Output

Respond in Russian with result/status, file manifest, training evidence, progress/resource evidence, quality gates, residual risks and `08A` handoff.
