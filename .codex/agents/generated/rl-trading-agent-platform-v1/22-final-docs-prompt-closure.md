---
prompt_name: 22-final-docs-prompt-closure
repo: roehub.com
branch: main
scope: "Close RL trading plan docs, ledger, prompt pack, runbooks, delivery state, and final go/no-go record."
language:
...
context_sources:
  ...
  task_entrypoints:
    - path: docs/architecture/ml/rl-trading-agent-platform-v1.md
      why: "source plan closure"
      inspect_symbols:
        - "Change Log"
        - "Risks"
    - path: docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/rl-trading-agent-platform-v1-stage-ledger.md
      why: "stage status closure"
      inspect_symbols:
        - "Stage Status"
        - "Checks And Evidence"
    - path: docs/runbooks/exchange-execution.md
      why: "existing operations runbook style"
      inspect_symbols:
        - "rollback"
        - "mainnet"
...
skill_routing:
  ...
  - skill: architecture-review
    use_when: "checking final plan/ledger/docs consistency"
    timing: "before ship"
    reason: "owns architecture/docs closure review"
...
quality_gates:
  - cmd: "python -m tools.docs.generate_docs_index --check"
    expect: "passes after final docs updates"
validation_strategy:
  depth: ci_deploy
  e2e_required: true
  acceptance_surfaces:
  ...
  evidence_target: docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/22-final-docs-prompt-closure.md
...
expected_primary_touches:
  ...
  - "docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/22-final-docs-prompt-closure.md"
  - "docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/rl-trading-agent-platform-v1-stage-ledger.md"
...
Implement Stage 22 final docs and prompt closure. Close the RL architecture plan, stage ledger, prompt pack, runbooks, delivery state, and final go/no-go record after all required prior stages are accepted or explicitly skipped/superseded.

Done means:

- Plan and ledger accurately reflect final accepted, blocked, skipped, or superseded stages.
- Runbooks and docs index are current.
- Prompt pack status and final delivery/deploy evidence are recorded.
...
- open_items:
  - Leave a durable, audit-ready implementation record for the RL trading agent platform lifecycle.
...
- Verify the stage prerequisites before implementation. Required accepted prerequisites: all required prior stages. If any required prerequisite is not accepted in the ledger, stop, write/update the stage report as blocked, update the ledger, and do not implement dependent work.
- Compute this prompt hash with `shasum -a 256 .codex/agents/generated/rl-trading-agent-platform-v1/22-final-docs-prompt-closure.md` and record the prompt path and hash in the stage report.
...
- Keep the change bounded to Stage `22`. Do not start later stages or silently repair unrelated legacy issues.
- Update or create the stage report at `docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/22-final-docs-prompt-closure.md` using the Stage 01 report shape: scope, file manifest, observed state, contract impact, quality gates, evidence, blockers, and next-stage handoff.
...
- Use `architecture-review` if any stage status, docs, or runtime claim is ambiguous.
- Do not mark incomplete stages accepted.
- Do not hide residual risks; classify them and record owner/next action.
- Verify docs/runbooks do not contain secrets or raw provider payloads.
- Record final delivery state: local-only, delivered-to-main, deployed-on-macstudio, or explicitly not deployed.
...
- `architecture-review`: use before ship when checking final plan/ledger/docs consistency; owns architecture/docs closure review.
...
1. Read the required context using the bounded protocol and verify Stage `22` prerequisites in the ledger.
2. Record `User required before start`, prompt path/hash, planned concrete file list, and any initial blockers in `docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/22-final-docs-prompt-closure.md`.
...
- Ledger closure is internally consistent and every required stage has evidence or blocker.
- Final report identifies residual non-blocking risks and exact next operational owner.
- The stage report exists at `docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/22-final-docs-prompt-closure.md` and includes prompt path/hash plus a strict file manifest.
...
- `python -m tools.docs.generate_docs_index --check` - passes after final docs updates
- Perform a manual redaction audit of changed docs and prompt files; record that no raw secrets, tokens, cookies, ciphertext, provider payloads, or checkpoint tensor contents are present.
