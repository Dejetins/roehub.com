---
prompt_name: 09-model-registry-activation
repo: roehub.com
branch: main
branch_policy:
  default_branch: main
...
prompt_pack_execution:
  ...
  stage_gate: "read ledger before edits; run only when current_stage is 09 and prerequisites match"
...
change_ownership:
  allowed_files:
    ...
    - docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/09-model-registry-activation.md
...
context_sources:
  ...
  task_entrypoints:
    - path: docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/08k-article-demo-profile-training-evaluation.md
      why: "expected accepted corrective Roehub-native research candidate gate"
      inspect_symbols:
    ...
    - path: docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/08i2-exhaustive-methodology-discrepancy-audit.md
      why: "complete blocked discrepancy matrix that later repair/recheck must reference"
      inspect_symbols:
        - "first material diff"
        - "parity verdict"
    ...
    - path: docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/08j-article-session-extractor-dataset.md
      why: "accepted article-selector dataset prerequisite"
      inspect_symbols:
    ...
    - path: src/trading/contexts/backtest_artifacts/application/services/v2/artifact_manifest_loader.py
      why: "artifact manifest/hash pattern"
      inspect_symbols:
        - "manifest loading"
        - "hash validation"
...
validation_strategy:
  ...
  evidence_target: docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/09-model-registry-activation.md
...
Implement Stage 09 model registry and activation gates. Persist dataset/model/calibration metadata with hashes, define registry state machine, artifact lifecycle, accepted-hash loading, checkpoint security, and candidate/champion activation audit.
...
- Stage gate: read the ledger before edits; run only when `current_stage=09`.
...
- Registry schema/use cases/tests exist for dataset, training run, model, calibration, candidate/champion, accepted/rejected states.
- Checkpoint loading validates canonical local path and sha256 before load, with no user upload path.
- Retention/quota/cleanup rules are recorded without deleting referenced accepted artifacts.
...
- open_items:
  - Make model artifacts safe, auditable, and loadable only through trusted local registry state.
...
- Previous-stage ledger gate: verify the stage prerequisites before implementation. Required accepted prerequisites: Stage `08I3`, Stage `08I4`, Stage `08J`, and Stage `08K` with an accepted Roehub-native corrective research candidate and `stage09_allowed=true` in the ledger. If a later explicitly accepted corrective stage supersedes `08K`, the ledger must name it and record `stage09_allowed=true`. Historical Stage `07B`/`08`, blocked `08F`/`08G`/`08H`, blocked first-diff-only `08I`, and blocked audit-only `08I2` are rejected/failed evidence and are not sufficient. If any required prerequisite is not accepted in the ledger, stop, write/update the stage report as blocked, update the ledger, and do not implement dependent work.
...
- Keep the change bounded to Stage `09`. Do not start later stages or silently repair unrelated legacy issues.
- Update or create the stage report at `docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/09-model-registry-activation.md` using the Stage 01 report shape: scope, file manifest, observed state, contract impact, quality gates, evidence, blockers, and next-stage handoff.
...
- Use trusted local artifacts only; do not accept arbitrary user-uploaded checkpoints.
- Validate accepted hashes before load; use PyTorch `weights_only` when available or document fallback and risk.
- State-machine invariant tests must block invalid transitions and missing/corrupt artifacts.
- Rejected old training runs are eligible for cleanup after configured N days, but accepted/champion/source manifests are retained.
- Postgres metadata must survive artifact deletion by marking missing/deleted state, not silently breaking queries.
...
1. Read the required context using the bounded protocol and verify Stage `09` prerequisites in the ledger.
2. Record `User required before start`, prompt path/hash, planned concrete file list, and any initial blockers in `docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/09-model-registry-activation.md`.
...
- Migrations and repository/use-case tests pass.
- Corrupt/missing hash checkpoint load is blocked before torch load.
- Registry activation/deactivation audit is recorded.
- Stage 09B can run backup/restore against accepted metadata and artifacts.
- The stage report exists at `docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/09-model-registry-activation.md` and includes prompt path/hash plus a strict file manifest.
