...
- Previous-stage ledger gate: verify the stage prerequisites before implementation. Required accepted prerequisites: Stage `08B` with an accepted Roehub-native research candidate. Historical Stage `07B`/`08` is rejected evidence and is not sufficient. If any required prerequisite is not accepted in the ledger, stop, write/update the stage report as blocked, update the ledger, and do not implement dependent work.
