...
context_sources:
  ...
  task_entrypoints:
    - path: docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/08f-roehub-native-backtest-evaluation.md
      why: "accepted Roehub-native research candidate gate"
      inspect_symbols:
...
- Previous-stage ledger gate: verify the stage prerequisites before implementation. Required accepted prerequisites: Stage `08F` with an accepted Roehub-native research candidate. Historical Stage `07B`/`08` is rejected evidence and is not sufficient. If any required prerequisite is not accepted in the ledger, stop, write/update the stage report as blocked, update the ledger, and do not implement dependent work.
