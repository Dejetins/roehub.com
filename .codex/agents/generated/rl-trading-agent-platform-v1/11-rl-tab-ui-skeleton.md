---
prompt_name: 11-rl-tab-ui-skeleton
repo: roehub.com
branch: main
branch_policy:
  default_branch: main
...
scope: "Add /strategies RL/ML tab, read models, signal/outcome surface, and guarded operator controls without live side effects."
language:
...
context_sources:
  ...
  task_entrypoints:
    - path: apps/api/routes/strategies.py
      why: "strategies surface"
      inspect_symbols:
        - "router"
        - "strategy routes"
    - path: apps/api/routes/ui_strategies_dashboard.py
      why: "dashboard/read model pattern"
      inspect_symbols:
        - "strategy dashboard DTOs"
...
skill_routing:
  ...
  - skill: browser-qa-evidence
    use_when: "verifying /strategies RL tab rendering or authenticated flows"
    timing: "during verification"
    reason: "owns real browser QA evidence"
...
validation_strategy:
  ...
  evidence_target: docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/11-rl-tab-ui-skeleton.md
...
expected_primary_touches:
  ...
  - "tests/unit/apps/api"
  - "tests/unit/apps/web"
  - "docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/11-rl-tab-ui-skeleton.md"
...
Implement Stage 11 RL tab UI skeleton on `/strategies`. Add model status, ticker slots, modes, risk config placeholders, authorized operator controls for retraining/rollback where server-side guard exists, and reusable signal/outcome read model for `source_type=ml_agent_decision`.

Done means:

- RL/ML tab renders in `/strategies` with model/ticker/mode/risk/status surfaces.
- Operator actions are hidden/disabled unless server-side operator/admin guard is present.
- Signal/outcome journal is reusable for future Telegram/notification delivery and not RL-only.
...
- open_items:
  - Expose RL/ML strategy UX and read models without live execution side effects.
...
- Verify the stage prerequisites before implementation. Required accepted prerequisites: Stage 09B, Stage 10A. If any required prerequisite is not accepted in the ledger, stop, write/update the stage report as blocked, update the ledger, and do not implement dependent work.
- Compute this prompt hash with `shasum -a 256 .codex/agents/generated/rl-trading-agent-platform-v1/11-rl-tab-ui-skeleton.md` and record the prompt path and hash in the stage report.
...
- Keep the change bounded to Stage `11`. Do not start later stages or silently repair unrelated legacy issues.
- Update or create the stage report at `docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/11-rl-tab-ui-skeleton.md` using the Stage 01 report shape: scope, file manifest, observed state, contract impact, quality gates, evidence, blockers, and next-stage handoff.
...
- Do not create order intents or exchange side effects in this stage.
- If operator/admin guard primitive is absent, show read-only/disabled controls and record blocked operator action controls.
- Use backend read models; do not trust browser state for entitlement or operator authorization.
- Verify in a real browser/runtime surface with smoke account if auth is required.
- Extend reusable source-event/outcome model for `ml_agent_decision`, not a parallel RL-only journal.
...
- `browser-qa-evidence`: use during verification when verifying /strategies RL tab rendering or authenticated flows; owns real browser QA evidence.
...
1. Read the required context using the bounded protocol and verify Stage `11` prerequisites in the ledger.
2. Record `User required before start`, prompt path/hash, planned concrete file list, and any initial blockers in `docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/11-rl-tab-ui-skeleton.md`.
...
- Browser QA proves the RL/ML tab renders and responsive text does not overlap.
- API/read model tests prove blocked reasons, model summaries, ticker slots, and source_type `ml_agent_decision` data shape.
- Operator control tests prove fail-closed authorization.
- The stage report exists at `docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/11-rl-tab-ui-skeleton.md` and includes prompt path/hash plus a strict file manifest.
