---
prompt_name: 02c-action-state-reward-contract
repo: roehub.com
branch: main
scope: "Freeze Roehub RL action, state, reward, and strategy-owned close semantics."
language:
...
context_sources:
  ...
  task_entrypoints:
    - path: src/trading/contexts/live_execution/domain/execution_source.py
      why: "ml_agent_decision contract"
      inspect_symbols:
        - "SUPPORTED_SOURCE_TYPES"
        - "ml_agent_decision"
    - path: src/trading/contexts/strategy/domain/entities/strategy_run.py
      why: "strategy run ownership anchor"
      inspect_symbols:
        - "StrategyRun"
    - path: src/trading/contexts/live_execution/domain/risk_gate.py
      why: "source-aware risk behavior"
      inspect_symbols:
        - "ml_agent_decision"
...
validation_strategy:
  ...
  acceptance_surfaces:
    - "domain fixtures"
    - "reward compatibility tests"
    - "ownership invariants"
  tests_only_allowed_reason: ""
  evidence_target: docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/02c-action-state-reward-contract.md
...
Implement and document the Stage 02C RL action/state/reward contract. Preserve external repo semantics while making Roehub ownership, no-pyramiding, no-cross-strategy-close, and reward/backtest/live distinction explicit.

Done means:

- Action 0/1/2/3 semantics are represented in domain code and tests.
- Training reward mirrors the external repo v1 behavior without Roehub risk-score rewrite.
- Close/open behavior is scoped to owner_user_id + strategy_run_id + exchange + market_type + symbol.
...
- open_items:
  - Make RL environment semantics executable before training code is ported.
...
- Verify the stage prerequisites before implementation. Required accepted prerequisites: Stage 02B. If any required prerequisite is not accepted in the ledger, stop, write/update the stage report as blocked, update the ledger, and do not implement dependent work.
- Compute this prompt hash with `shasum -a 256 .codex/agents/generated/rl-trading-agent-platform-v1/02c-action-state-reward-contract.md` and record the prompt path and hash in the stage report.
...
- Keep the change bounded to Stage `02C`. Do not start later stages or silently repair unrelated legacy issues.
- Update or create the stage report at `docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/02c-action-state-reward-contract.md` using the Stage 01 report shape: scope, file manifest, observed state, contract impact, quality gates, evidence, blockers, and next-stage handoff.
...
- Action 0 is hold/no order intent; action 1 open_long; action 2 open_short; action 3 close only this RL strategy run position.
- Preserve no-pyramiding from the external repo: repeated same-side open while already open becomes no_intent with audit reason.
- Preserve reward v1: pnl_change / initial_balance minus flat-hold inaction penalty; hold while open has no mark-to-market reward.
- Live outcome is not training reward source of truth; paper/testnet/live outcome comes from execution ledgers.
- User-specific live outcomes must not enter platform-wide retraining in v1.
...
1. Read the required context using the bounded protocol and verify Stage `02C` prerequisites in the ledger.
2. Record `User required before start`, prompt path/hash, planned concrete file list, and any initial blockers in `docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/02c-action-state-reward-contract.md`.
