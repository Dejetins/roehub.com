---
prompt_name: 18-rl-soak-incident-drills
repo: roehub.com
branch: main
branch_policy:
  default_branch: main
...
prompt_pack_execution:
  ...
  stage_gate: "read ledger before edits; run only when current_stage is 18 and accepted 08N explicitly allows the requested soak mode"
...
browser_auth:
  status: "Required only for visible safe/degraded-state browser QA in this stage"
...
context_sources:
  ...
  task_entrypoints:
    - path: docs/runbooks/mac-studio-native-backend-operations.md
      why: "Mac Studio runtime operations"
      inspect_symbols:
        - "services"
        - "smoke"
    - path: docs/runbooks/exchange-execution.md
      why: "execution incident style"
      inspect_symbols:
        - "reconciliation"
        - "unknown state"
...
skill_routing:
  ...
  - skill: browser-qa-evidence
    use_when: "verifying visible safe/degraded states"
  ...
  - skill: publish-ci-deploy
    use_when: "soak requires deployed Mac Studio runtime evidence"
    timing: "before ship"
...
non_goals:
  ...
  - "Do not claim model/product/trading quality from soak evidence; Stage 08N owns candidate quality reclassification."
...
validation_strategy:
  ...
  evidence_target: docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/18-rl-soak-incident-drills.md
...
- Update or create the stage report at `docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/18-rl-soak-incident-drills.md` using the Stage 01 report shape: scope, file manifest, observed state, contract impact, quality gates, evidence, blockers, and next-stage handoff.
...
- `browser-qa-evidence`: use during verification when verifying visible safe/degraded states; owns browser runtime QA.
...
- `publish-ci-deploy`: use before ship when soak requires deployed Mac Studio runtime evidence; records CI/deploy/Mac Studio verification after GitHub publication or merge.

1. Read the required context using the bounded protocol and verify Stage `18` prerequisites in the ledger.
2. Record `User required before start`, prompt path/hash, planned concrete file list, and any initial blockers in `docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/18-rl-soak-incident-drills.md`.
