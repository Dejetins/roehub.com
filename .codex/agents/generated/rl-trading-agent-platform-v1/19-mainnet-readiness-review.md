---
prompt_name: 19-mainnet-readiness-review
repo: roehub.com
branch: main
branch_policy:
  default_branch: main
...
skill_routing:
  ...
  - skill: architecture-review
    use_when: "performing mainnet readiness review"
    timing: "before implementation"
    reason: "owns source-backed go/no-go assessment"
...
quality_gates:
  - cmd: "python -m tools.docs.generate_docs_index --check"
    expect: "passes after readiness docs updates"
validation_strategy:
  ...
  acceptance_surfaces:
    ...
    - "risk register"
    - "runbook review"
    - "product/legal/support checklist"
  tests_only_allowed_reason: ""
  evidence_target: docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/19-mainnet-readiness-review.md
...
Perform Stage 19 mainnet readiness architecture review. Produce a go/no-go record for real-money enablement covering technical, operational, product, legal, support, backup/residual-risk, mainnet guard diff, rollback/runbook, and operator approval trail. Do not enable mainnet in this stage.

Done means:

- Risk register and go/no-go decision are documented.
- Product/legal/support readiness, disclaimer/risk disclosure, support policy, and mainnet enablement policy are accepted or blocked.
- Explicit approval requirement for Stage 20 is recorded.
...
- open_items:
  - Make a signed-off real-money readiness decision before bounded canary.
...
- Update or create the stage report at `docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/19-mainnet-readiness-review.md` using the Stage 01 report shape: scope, file manifest, observed state, contract impact, quality gates, evidence, blockers, and next-stage handoff.
...
- Use `architecture-review` before editing the readiness report.
- Do not implement mainnet submit or toggle live canary.
- Check backup evidence or record explicit no-backup/single-host residual risk exception.
- Verify every prior required stage is accepted and evidence is linked.
- Record operator/admin approval trail requirements for Stage 20.
...
- `architecture-review`: use before implementation when performing mainnet readiness review; owns source-backed go/no-go assessment.
...
1. Read the required context using the bounded protocol and verify Stage `19` prerequisites in the ledger.
2. Record `User required before start`, prompt path/hash, planned concrete file list, and any initial blockers in `docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/19-mainnet-readiness-review.md`.
...
- `python -m tools.docs.generate_docs_index --check` - passes after readiness docs updates
