---
prompt_name: 15-paper-rl-integration
repo: roehub.com
branch: main
branch_policy:
  default_branch: main
...
scope: "Convert accepted RL decisions to paper intents/orders through existing execution and prove accounting parity."
language:
...
context_sources:
  ...
  task_entrypoints:
    - path: docs/architecture/live_execution/strategy-producer-paper-testnet-trading-v1-stage-reports/strategy-producer-paper-testnet-trading-v1-stage-ledger.md
      why: "classic Stage 05/07 dependency"
      inspect_symbols:
        - "Stage 05"
        - "Stage 07"
    - path: src/trading/contexts/live_execution/domain/execution_source.py
      why: "source event and intent contract"
      inspect_symbols:
        - "ml_agent_decision"
    - path: tests/unit/contexts/live_execution/test_paper_accounting_service.py
      why: "paper accounting contract"
      inspect_symbols:
        - "paper accounting"
...
skill_routing:
  ...
  - skill: publish-ci-deploy
    use_when: "paper runtime path needs Mac Studio target evidence"
    timing: "before ship"
...
validation_strategy:
  ...
  acceptance_surfaces:
    ...
    - "simulator/paper parity"
    - "idempotency proof"
  tests_only_allowed_reason: ""
  evidence_target: docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/15-paper-rl-integration.md
...
expected_primary_touches:
  ...
  - "docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/15-paper-rl-integration.md"
...
- Classic producer Stage 07 accepted prerequisite is verified.
- RL decisions create paper intents/orders only through existing execution/risk path.
- Simulator/accounting/paper parity is within documented tolerance and duplicate/idempotency evidence is recorded.
...
- open_items:
  - Prove RL paper execution without bypassing existing live_execution/risk rails.
...
- Verify the stage prerequisites before implementation. Required accepted prerequisites: Stage 14, classic producer Stage 07. If any required prerequisite is not accepted in the ledger, stop, write/update the stage report as blocked, update the ledger, and do not implement dependent work.
- Compute this prompt hash with `shasum -a 256 .codex/agents/generated/rl-trading-agent-platform-v1/15-paper-rl-integration.md` and record the prompt path and hash in the stage report.
...
- Keep the change bounded to Stage `15`. Do not start later stages or silently repair unrelated legacy issues.
- Update or create the stage report at `docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/15-paper-rl-integration.md` using the Stage 01 report shape: scope, file manifest, observed state, contract impact, quality gates, evidence, blockers, and next-stage handoff.
...
- No testnet or mainnet submit is allowed.
...
- `publish-ci-deploy`: use before ship when paper runtime path needs Mac Studio target evidence; records CI/deploy/Mac Studio verification after GitHub publication or merge.

1. Read the required context using the bounded protocol and verify Stage `15` prerequisites in the ledger.
2. Record `User required before start`, prompt path/hash, planned concrete file list, and any initial blockers in `docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/15-paper-rl-integration.md`.
...
- Paper ledger proof shows ml_agent_decision to source_event to paper intent/order through existing execution path.
- No duplicate intents on replay with same idempotency key.
- Simulator/accounting/paper parity report is recorded.
- The stage report exists at `docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/15-paper-rl-integration.md` and includes prompt path/hash plus a strict file manifest.
