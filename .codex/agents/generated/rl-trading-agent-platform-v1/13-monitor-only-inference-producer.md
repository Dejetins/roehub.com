---
prompt_name: 13-monitor-only-inference-producer
repo: roehub.com
branch: main
scope: "Run monitor-only RL inference producer with durable ml_agent_decision no_intent events and train/live feature parity."
language:
...
skill_routing:
  ...
  - skill: backend-performance-evidence
    use_when: "measuring inference latency/RSS/CPU/MPS"
    timing: "during verification"
    reason: "requires comparable runtime evidence"
  - skill: browser-qa-evidence
    use_when: "verifying monitor-only signals in /strategies"
    timing: "during verification"
    reason: "owns browser-visible signal QA"
  ...
  - skill: publish-ci-deploy
    use_when: "supervised producer config is delivered to Mac Studio"
    timing: "before ship"
...
validation_strategy:
  ...
  e2e_required: true
  acceptance_surfaces:
    - "Mac Studio supervised runtime"
    - "Redis/live feature parity"
    - "DB source events"
    - "Prometheus/Monit"
    - "browser QA for /strategies signal journal"
  tests_only_allowed_reason: ""
  evidence_target: docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/13-monitor-only-inference-producer.md
...
expected_primary_touches:
  ...
  - "tests/unit/apps"
  - "docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/13-monitor-only-inference-producer.md"
...
Implement Stage 13 monitor-only inference producer. Run supervised RL inference on closed candles, emit durable `ml_agent_decision` source events with `no_intent`, prove train/live feature parity, and expose runtime health/metrics without creating order intents.

Done means:

- Inference producer is supervised on Mac Studio with health/metrics and degraded states.
- A live Redis/canonical window and offline dataset fixture produce identical feature vectors for the same candle window.
- `ml_agent_decision` source events are recorded as no_intent and visible in reusable UI signal/outcome journal.
...
- open_items:
  - Operate the model in production-like monitor_only mode before any paper/testnet/live order path.
...
- Verify the stage prerequisites before implementation. Required accepted prerequisites: Stage 10, Stage 10A, Stage 11, Stage 12. If any required prerequisite is not accepted in the ledger, stop, write/update the stage report as blocked, update the ledger, and do not implement dependent work.
- Compute this prompt hash with `shasum -a 256 .codex/agents/generated/rl-trading-agent-platform-v1/13-monitor-only-inference-producer.md` and record the prompt path and hash in the stage report.
...
- Keep the change bounded to Stage `13`. Do not start later stages or silently repair unrelated legacy issues.
- Update or create the stage report at `docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/13-monitor-only-inference-producer.md` using the Stage 01 report shape: scope, file manifest, observed state, contract impact, quality gates, evidence, blockers, and next-stage handoff.
...
- Do not create order intents in monitor_only.
- Model must be preloaded; do not load checkpoint per decision.
- If live feed lacks required features, block or degrade according to Stage 02B/05 decision; do not add full ClickHouse scans to hot path.
- Use ClickHouse repair only for gaps/degraded state, not normal inference path.
- Record p95 segment latency: candle close to feature ready, feature to decision, decision to source_event.
...
- `backend-performance-evidence`: use during verification when measuring inference latency/RSS/CPU/MPS; requires comparable runtime evidence.
- `browser-qa-evidence`: use during verification when verifying monitor-only signals in /strategies; owns browser-visible signal QA.
...
- `publish-ci-deploy`: use before ship when supervised producer config is delivered to Mac Studio; records CI/deploy/Mac Studio verification after GitHub publication or merge.

1. Read the required context using the bounded protocol and verify Stage `13` prerequisites in the ledger.
2. Record `User required before start`, prompt path/hash, planned concrete file list, and any initial blockers in `docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/13-monitor-only-inference-producer.md`.
...
- Mac Studio runtime evidence shows producer up, metrics exposed, and bounded resource use.
- Golden train/live parity fixture passes.
- DB/API/UI evidence shows monitor-only `ml_agent_decision` no_intent outcomes without Redis execution dispatch.
- The stage report exists at `docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/13-monitor-only-inference-producer.md` and includes prompt path/hash plus a strict file manifest.
