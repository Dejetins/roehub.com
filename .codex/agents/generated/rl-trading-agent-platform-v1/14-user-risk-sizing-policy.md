---
prompt_name: 14-user-risk-sizing-policy
repo: roehub.com
branch: main
branch_policy:
  default_branch: main
...
scope: "Implement user-configurable RL risk/sizing policy with synthetic platform-side exits and no exchange submit."
language:
...
context_sources:
  ...
  task_entrypoints:
    - path: src/trading/contexts/live_execution/domain/execution_source.py
      why: "order model constraints"
      inspect_symbols:
        - "ExecutionOrderModelRequest"
        - "unsupported fields"
    - path: src/trading/contexts/live_execution/domain/risk_gate.py
      why: "risk gate behavior"
      inspect_symbols:
        - "ml_agent_decision"
    - path: apps/api/dto/ui_execution.py
      why: "execution DTO surface"
      inspect_symbols:
        - "order model DTO"
...
skill_routing:
  ...
  - skill: browser-qa-evidence
    use_when: "verifying /strategies risk configuration UI"
    timing: "during verification"
    reason: "owns browser-visible risk config QA"
...
validation_strategy:
  ...
  acceptance_surfaces:
    - "API/UI/domain tests"
    - "synthetic exit fixtures"
    - "no exchange submit proof"
    - "browser QA for /strategies risk controls"
  tests_only_allowed_reason: ""
  evidence_target: docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/14-user-risk-sizing-policy.md
...
expected_primary_touches:
  ...
  - "tests/unit/apps/api"
  - "docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/14-user-risk-sizing-policy.md"
...
Implement Stage 14 RL user risk/sizing policy. Add configurable per-ticker activation risk/sizing settings, fail-closed validation, and synthetic platform-side TP/SL/trailing exits. Do not submit exchange orders in this stage.

Done means:

- Risk/sizing policy schema, validation, persistence, API/UI, and tests exist.
- TP/SL/trailing are represented as synthetic platform-side exit rules that create later close intents, not native exchange OCO/TP/SL/trailing fields.
- Invalid policy blocks activation and monitor-only decisions remain no_intent.
...
- open_items:
  - Give users necessary configuration while preserving existing execution/risk boundary.
...
- Verify the stage prerequisites before implementation. Required accepted prerequisites: Stage 13. If any required prerequisite is not accepted in the ledger, stop, write/update the stage report as blocked, update the ledger, and do not implement dependent work.
- Compute this prompt hash with `shasum -a 256 .codex/agents/generated/rl-trading-agent-platform-v1/14-user-risk-sizing-policy.md` and record the prompt path and hash in the stage report.
...
- Keep the change bounded to Stage `14`. Do not start later stages or silently repair unrelated legacy issues.
- Update or create the stage report at `docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/14-user-risk-sizing-policy.md` using the Stage 01 report shape: scope, file manifest, observed state, contract impact, quality gates, evidence, blockers, and next-stage handoff.
...
- Do not pass advanced/native exchange order fields into current execution order model.
- Risk/sizing is user-configured but bounded/fail-closed by platform rules.
- Support balance between PnL, drawdown, turnover, exposure, daily loss, and optional confidence thresholds as configurable policy inputs.
- No exchange submit, paper order, or testnet order is allowed in this stage.
- Policy must be owner/ticker/market scoped and audited.
...
- `browser-qa-evidence`: use during verification when verifying /strategies risk configuration UI; owns browser-visible risk config QA.
...
1. Read the required context using the bounded protocol and verify Stage `14` prerequisites in the ledger.
2. Record `User required before start`, prompt path/hash, planned concrete file list, and any initial blockers in `docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/14-user-risk-sizing-policy.md`.
...
- API/UI/domain tests cover valid policy, invalid policy, synthetic exits, and fail-closed defaults.
- Browser QA shows risk config controls and blocked reasons without text overflow.
- Stage 15 has enough policy to size paper intents through existing execution path.
- The stage report exists at `docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/14-user-risk-sizing-policy.md` and includes prompt path/hash plus a strict file manifest.
