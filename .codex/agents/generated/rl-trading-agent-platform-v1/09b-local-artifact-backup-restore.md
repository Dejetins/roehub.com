---
prompt_name: 09b-local-artifact-backup-restore
repo: roehub.com
branch: main
scope: "Implement local RL artifact backup, registry metadata dump, restore drill, hash validation, and rollback evidence."
language:
...
context_sources:
  ...
  task_entrypoints:
    - path: src/trading/contexts/backtest_artifacts/adapters/outbound/artifacts_fs/path_builder.py
      why: "local artifact path pattern"
      inspect_symbols:
        - "path builder"
        - "artifact paths"
    - path: docs/runbooks/mac-studio-native-backend-operations.md
      why: "Mac Studio operation style"
      inspect_symbols:
        - "backup"
        - "runtime paths"
    - path: scripts/macos/smoke_prod.sh
      why: "host smoke pattern"
      inspect_symbols:
        - "runtime smoke"
...
skill_routing:
  ...
  - skill: publish-ci-deploy
    use_when: "backup/restore commands or runtime runbook need Mac Studio verification"
    timing: "before ship"
...
validation_strategy:
  ...
  evidence_target: docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/09b-local-artifact-backup-restore.md
...
expected_primary_touches:
  ...
  - "docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/09b-local-artifact-backup-restore.md"
...
Implement Stage 09B local artifact backup and restore drill. Back up accepted champion/calibration/source manifests/registry metadata locally, restore to a separate path, validate hashes, and prove rollback to previous accepted champion.

Done means:

- Backup manifest and registry metadata dump exist for accepted artifacts.
- Restore drill to separate path validates hashes and registry references.
- Rollback command/runbook evidence is recorded with residual single-host disk risk.
...
- open_items:
  - Prevent runtime activation from depending on un-restorable single-copy artifacts.
...
- Verify the stage prerequisites before implementation. Required accepted prerequisites: Stage 09. If any required prerequisite is not accepted in the ledger, stop, write/update the stage report as blocked, update the ledger, and do not implement dependent work.
- Compute this prompt hash with `shasum -a 256 .codex/agents/generated/rl-trading-agent-platform-v1/09b-local-artifact-backup-restore.md` and record the prompt path and hash in the stage report.
...
- Keep the change bounded to Stage `09B`. Do not start later stages or silently repair unrelated legacy issues.
- Update or create the stage report at `docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/09b-local-artifact-backup-restore.md` using the Stage 01 report shape: scope, file manifest, observed state, contract impact, quality gates, evidence, blockers, and next-stage handoff.
...
- Use /opt/roehub/state/rl_trading/ as the primary artifact root.
- No cloud storage is introduced in v1.
- Do not delete accepted/champion artifacts during cleanup.
- Record what is retained forever, what is retained for N days, and what is removable.
- If there is no second disk/backup path available, record explicit residual single-host risk for Stage 19/21.
...
- `publish-ci-deploy`: use before ship when backup/restore commands or runtime runbook need Mac Studio verification; records CI/deploy/Mac Studio verification after GitHub publication or merge.

1. Read the required context using the bounded protocol and verify Stage `09B` prerequisites in the ledger.
2. Record `User required before start`, prompt path/hash, planned concrete file list, and any initial blockers in `docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/09b-local-artifact-backup-restore.md`.
...
- A restore drill succeeds against a separate local path with hash validation.
- Registry metadata can identify missing/deleted artifacts without crashing read paths.
- Stage 10/10A/13 runtime work is not allowed until this drill is accepted.
- The stage report exists at `docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/09b-local-artifact-backup-restore.md` and includes prompt path/hash plus a strict file manifest.
