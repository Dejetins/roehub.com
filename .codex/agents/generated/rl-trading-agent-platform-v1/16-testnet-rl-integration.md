---
prompt_name: 16-testnet-rl-integration
repo: roehub.com
branch: main
scope: "Run RL through real Binance/Bybit spot/futures testnet with safe guards and no mainnet."
language:
...
context_sources:
  ...
  task_entrypoints:
    - path: docs/architecture/live_execution/strategy-producer-paper-testnet-trading-v1-stage-reports/strategy-producer-paper-testnet-trading-v1-stage-ledger.md
      why: "classic Stage 09 dependency"
      inspect_symbols:
        - "Stage 05"
        - "Stage 09"
    - path: docs/architecture/identity/identity-exchange-connections-live-trading-v1.md
      why: "exchange credential/readiness contract"
      inspect_symbols:
        - "testnet"
        - "readiness"
    - path: apps/exchange_execution/main/app.py
      why: "exchange execution process"
      inspect_symbols:
        - "exchange execution"
...
skill_routing:
  ...
  - skill: publish-ci-deploy
    use_when: "testnet runtime changes need Mac Studio deployment and smoke evidence"
    timing: "before ship"
...
validation_strategy:
  ...
  e2e_required: true
  acceptance_surfaces:
    - "real testnet orders/fills"
    - "reconciliation"
    - "unsupported branch blocks"
  tests_only_allowed_reason: ""
  evidence_target: docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/16-testnet-rl-integration.md
...
expected_primary_touches:
  ...
  - "docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/16-testnet-rl-integration.md"
...
Implement Stage 16 testnet RL integration. Run Binance/Bybit spot/futures testnet through existing execution/exchange processes with safe guards, account readiness, real orders/fills/reconciliation, and no mainnet. This stage is blocked until classic producer Stage 09 is accepted after the current Stage 05 repair path.

Done means:

- Classic producer Stage 09 accepted prerequisite is verified.
- Real testnet order/fill/reconciliation evidence exists for supported branches.
- Unsupported market/account/model branches block with stable reasons and no mainnet submit.
...
- open_items:
  - Prove real exchange testnet behavior for RL without opening mainnet.
...
- Verify the stage prerequisites before implementation. Required accepted prerequisites: Stage 15, classic producer Stage 09. If any required prerequisite is not accepted in the ledger, stop, write/update the stage report as blocked, update the ledger, and do not implement dependent work.
- Compute this prompt hash with `shasum -a 256 .codex/agents/generated/rl-trading-agent-platform-v1/16-testnet-rl-integration.md` and record the prompt path and hash in the stage report.
...
- Keep the change bounded to Stage `16`. Do not start later stages or silently repair unrelated legacy issues.
- Update or create the stage report at `docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/16-testnet-rl-integration.md` using the Stage 01 report shape: scope, file manifest, observed state, contract impact, quality gates, evidence, blockers, and next-stage handoff.
...
- If classic producer Stage 05 remains blocked or Stage 09 is not accepted, stop and mark Stage 16 blocked.
- Do not ask for secrets in chat; use existing `/settings`/exchange custody path.
- Do not auto-config leverage, margin mode, or position mode unless a separate accepted approval exists.
- No blind retry after unknown exchange submit; use existing reconciliation/provider lookup rules.
- Record segment p95 latency through exchange submit attempt before provider latency.
...
- `publish-ci-deploy`: use before ship when testnet runtime changes need Mac Studio deployment and smoke evidence; records CI/deploy/Mac Studio verification after GitHub publication or merge.

1. Read the required context using the bounded protocol and verify Stage `16` prerequisites in the ledger.
2. Record `User required before start`, prompt path/hash, planned concrete file list, and any initial blockers in `docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/16-testnet-rl-integration.md`.
...
- Real testnet evidence includes order submit, ack/reject/fill, reconciliation, and no duplicate submit proof.
- Unsupported branches fail closed.
- Mainnet submit counters remain zero and mainnet config stays blocked.
- The stage report exists at `docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/16-testnet-rl-integration.md` and includes prompt path/hash plus a strict file manifest.
