---
prompt_name: 10-per-ticker-calibration
repo: roehub.com
branch: main
branch_policy:
  default_branch: main
...
prompt_pack_execution:
  ...
  stage_gate: "read ledger before edits; run only when current_stage is 10 and prerequisites match"
...
change_ownership:
  allowed_files:
    ...
    - docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/10-per-ticker-calibration.md
...
context_sources:
  ...
  task_entrypoints:
    - path: docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/08k-article-demo-profile-training-evaluation.md
      why: "accepted corrective Roehub-native evaluation and candidate metrics"
      inspect_symbols:
        - "scorecard"
    ...
    - path: docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/08i4-post-repair-methodology-recheck.md
      ...
      inspect_symbols:
        ...
        - "stage09_allowed"
    - path: docs/architecture/ml/rl-trading-agent-platform-v1.md
      why: "calibration and scorecard contract"
      inspect_symbols:
        - "Per-ticker calibration"
        - "Promotion scorecard"
    - path: docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/01-baseline-plan-freeze.md
      why: "calibration report style"
      inspect_symbols:
        - "Evidence"
        - "Contract Impact"
    - path: tests/unit/shared_kernel/primitives/test_paid_level.py
      why: "small domain test style"
      inspect_symbols:
        - "value-object tests"
  conditional_bundles:
    execution_boundary:
...
validation_strategy:
  ...
  evidence_target: docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/10-per-ticker-calibration.md
...
Implement Stage 10 per-ticker/per-market calibration. Create calibration thresholds, weights, normalization/calibration stats, or optional ticker heads only where supported by evidence and registry lineage.
...
- Stage gate: read the ledger before edits; run only when `current_stage=10`.
...
- Calibration artifacts are versioned, hashed, registry-linked, and ticker/market scoped.
- No global-only threshold is activated unless explicitly accepted with evidence.
- Calibration report explains adjustable balance between PnL, drawdown, turnover, and risk metrics.
...
- open_items:
  - Adapt the platform-wide model to specific tickers/markets without training one model per user.
...
- Keep the change bounded to Stage `10`. Do not start later stages or silently repair unrelated legacy issues.
- Update or create the stage report at `docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/10-per-ticker-calibration.md` using the Stage 01 report shape: scope, file manifest, observed state, contract impact, quality gates, evidence, blockers, and next-stage handoff.
...
- Keep the model family platform-wide; calibrations are per ticker/market, not per user.
- Do not promote calibration to champion without Stage 10A gates.
- Record how calibration affects action thresholds, confidence, skipped-action reasons, and risk/sizing inputs.
- Block branches with unstable or insufficient per-ticker evidence.
...
1. Read the required context using the bounded protocol and verify Stage `10` prerequisites in the ledger.
2. Record `User required before start`, prompt path/hash, planned concrete file list, and any initial blockers in `docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/10-per-ticker-calibration.md`.
...
- Per-ticker calibration tests and report pass for accepted initial ticker universe.
- Registry stores calibration hashes and lineage to model/dataset/evaluation artifacts.
- Stage 10A can evaluate candidate/champion promotion using calibration artifacts.
- The stage report exists at `docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/10-per-ticker-calibration.md` and includes prompt path/hash plus a strict file manifest.
