---
prompt_name: 02b-feature-live-feed-contract
repo: roehub.com
branch: main
branch_policy:
  default_branch: main
...
scope: "Freeze feature, live-feed, and futures metadata contract for RL training and inference."
language:
...
validation_strategy:
  ...
  acceptance_surfaces:
    ...
    - "Redis/live feed parity decision"
    - "activation matrix"
  tests_only_allowed_reason: ""
  evidence_target: docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/02b-feature-live-feed-contract.md
...
- open_items:
  - Make the training and live inference feature representation explicit and testable.
...
- Verify the stage prerequisites before implementation. Required accepted prerequisites: Stage 02A. If any required prerequisite is not accepted in the ledger, stop, write/update the stage report as blocked, update the ledger, and do not implement dependent work.
- Compute this prompt hash with `shasum -a 256 .codex/agents/generated/rl-trading-agent-platform-v1/02b-feature-live-feed-contract.md` and record the prompt path and hash in the stage report.
...
- Keep the change bounded to Stage `02B`. Do not start later stages or silently repair unrelated legacy issues.
- Update or create the stage report at `docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/02b-feature-live-feed-contract.md` using the Stage 01 report shape: scope, file manifest, observed state, contract impact, quality gates, evidence, blockers, and next-stage handoff.
...
1. Read the required context using the bounded protocol and verify Stage `02B` prerequisites in the ledger.
2. Record `User required before start`, prompt path/hash, planned concrete file list, and any initial blockers in `docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/02b-feature-live-feed-contract.md`.
