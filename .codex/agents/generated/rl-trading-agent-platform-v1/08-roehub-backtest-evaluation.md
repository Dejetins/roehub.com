---
prompt_name: 08-roehub-backtest-evaluation
repo: roehub.com
branch: main
branch_policy:
  default_branch: main
...
scope: "Historical superseded evaluation prompt for the old Stage 07B candidate path; do not execute for the current methodology-parity rollout unless explicitly asked to reproduce the rejected Stage 08 evidence."
language:
...
skill_routing:
  ...
  - skill: data-analytics-methodology
    use_when: "building scorecard, baselines, and overfit checks"
    timing: "during implementation"
    reason: "owns model evaluation methodology"
...
validation_strategy:
  ...
  evidence_target: docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/08-roehub-backtest-evaluation.md
...
- Scorecard structure exists and includes PnL after costs, max drawdown, trades count, stability by ticker, out-of-sample period, overfit signs, and latency/resource notes.
- Technical baselines hold/no-trade, random, and simple threshold are run as sanity checks.
- Simulator/accounting parity fixtures pass.
...
- open_items:
  - Detect simulator/reward bugs and produce research-grade model evidence before registry activation.
...
- Update or create the stage report at `docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/08-roehub-backtest-evaluation.md` using the Stage 01 report shape: scope, file manifest, observed state, contract impact, quality gates, evidence, blockers, and next-stage handoff.
...
- Do not use baseline comparison as product marketing; use it as ML sanity to catch reward/simulator bugs.
...
- `data-analytics-methodology`: use during implementation when building scorecard, baselines, and overfit checks; owns model evaluation methodology.
...
1. Read the required context using the bounded protocol and verify Stage `08` prerequisites in the ledger.
2. Record `User required before start`, prompt path/hash, planned concrete file list, and any initial blockers in `docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/08-roehub-backtest-evaluation.md`.
...
- Research candidate save is allowed only with positive PnL after costs plus scorecard and sanity baseline artifacts.
- Simulator/accounting/paper parity fixture is recorded.
- Stage 09 can register candidate artifacts with hashes but cannot skip Stage 10A promotion profile.
- The stage report exists at `docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/08-roehub-backtest-evaluation.md` and includes prompt path/hash plus a strict file manifest.
