---
prompt_name: 12-backend-entitlements
repo: roehub.com
branch: main
branch_policy:
  default_branch: main
...
scope: "Enforce backend RL live ticker entitlements from paid_level and optional Enterprise override."
language:
...
context_sources:
  ...
  task_entrypoints:
    - path: src/trading/shared_kernel/primitives/paid_level.py
      why: "current paid levels"
      inspect_symbols:
        - "PaidLevel"
    - path: src/trading/contexts/identity/adapters/inbound/api/deps/current_user.py
      why: "current user source"
      inspect_symbols:
        - "current user"
        - "paid_level"
    - path: apps/api/routes/strategies.py
      why: "strategy activation API"
      inspect_symbols:
        - "strategy routes"
...
skill_routing:
  ...
  - skill: browser-qa-evidence
    use_when: "verifying UI blocked reasons and slot counts"
    timing: "during verification"
    reason: "owns browser-visible entitlement QA"
...
validation_strategy:
  ...
  evidence_target: docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/12-backend-entitlements.md
...
expected_primary_touches:
  ...
  - "docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/12-backend-entitlements.md"
...
Implement Stage 12 backend entitlements for RL live ticker quotas. Enforce active live ticker slots from current Roehub `paid_level` values plus optional RL override, with transactional DB uniqueness and UI/API blocked reasons.

Done means:

- Mapping is explicit: free=1, pro=5, ultra=20, base=0/fail-closed unless product evidence changes it, override=custom.
- Only live mode consumes slots; monitor_only, paper, and testnet do not.
- API/use-case and producer-side checks agree.
...
- open_items:
  - Make active live RL ticker count a backend-enforced contract without full billing.
...
- Verify the stage prerequisites before implementation. Required accepted prerequisites: Stage 11. If any required prerequisite is not accepted in the ledger, stop, write/update the stage report as blocked, update the ledger, and do not implement dependent work.
- Compute this prompt hash with `shasum -a 256 .codex/agents/generated/rl-trading-agent-platform-v1/12-backend-entitlements.md` and record the prompt path and hash in the stage report.
...
- Keep the change bounded to Stage `12`. Do not start later stages or silently repair unrelated legacy issues.
- Update or create the stage report at `docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/12-backend-entitlements.md` using the Stage 01 report shape: scope, file manifest, observed state, contract impact, quality gates, evidence, blockers, and next-stage handoff.
...
- Do not trust Keycloak/browser claims as canonical entitlement source; use Roehub DB/current principal contract and optional RL override table.
- Count distinct active live RL ticker `(owner_user_id, exchange_name, market_type, symbol)`.
- Allow users to stop one ticker and start another; record audit/rate-limit if implemented.
- Enforce transactionally with uniqueness and fail-closed unknown/ambiguous level behavior.
- Show stable blocked reason in UI/API.
...
- `browser-qa-evidence`: use during verification when verifying UI blocked reasons and slot counts; owns browser-visible entitlement QA.
...
1. Read the required context using the bounded protocol and verify Stage `12` prerequisites in the ledger.
2. Record `User required before start`, prompt path/hash, planned concrete file list, and any initial blockers in `docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/12-backend-entitlements.md`.
...
- Transactional quota tests cover free/pro/ultra/base/override and mode-specific slot counting.
- Browser/API evidence shows used/limit and blocked reason.
- Inference producer Stage 13 has a backend contract to re-check live intent eligibility.
- The stage report exists at `docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/12-backend-entitlements.md` and includes prompt path/hash plus a strict file manifest.
