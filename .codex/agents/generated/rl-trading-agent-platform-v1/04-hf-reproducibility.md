---
prompt_name: 04-hf-reproducibility
repo: roehub.com
branch: main
scope: "Reproduce the external repo/HF baseline safely with attribution, hashes, and small training/evaluation smoke."
language:
...
context_sources:
  ...
  task_entrypoints:
    ...
    - path: scripts/backtest/validate_benchmark_accounting.py
      why: "existing evidence script pattern"
      inspect_symbols:
        - "CLI report pattern"
...
skill_routing:
  ...
  - skill: data-analytics-methodology
    use_when: "selecting reproducibility metrics and interpreting HF baseline"
    timing: "during investigation"
    reason: "keeps ML evidence and limitations explicit"
...
validation_strategy:
  ...
  evidence_target: docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/04-hf-reproducibility.md
...
Implement Stage 04 external repo/HF reproducibility. Import concepts from `YuriyKolesnikov/rl-trading-binance` safely, download or reference the HF dataset as allowed, and reproduce a small deterministic train/eval/backtest smoke with hashes and attribution.

Done means:

- External code/data license and attribution are recorded.
- HF dataset manifest/hash and run config hash are recorded.
- Small train/eval/backtest smoke runs deterministically on the Stage 03 ML environment.
...
- open_items:
  - Prove the article/repo concept is reproducible before Roehub-native dataset work.
...
- Verify the stage prerequisites before implementation. Required accepted prerequisites: Stage 03. If any required prerequisite is not accepted in the ledger, stop, write/update the stage report as blocked, update the ledger, and do not implement dependent work.
- Compute this prompt hash with `shasum -a 256 .codex/agents/generated/rl-trading-agent-platform-v1/04-hf-reproducibility.md` and record the prompt path and hash in the stage report.
...
- Keep the change bounded to Stage `04`. Do not start later stages or silently repair unrelated legacy issues.
- Update or create the stage report at `docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/04-hf-reproducibility.md` using the Stage 01 report shape: scope, file manifest, observed state, contract impact, quality gates, evidence, blockers, and next-stage handoff.
...
- `data-analytics-methodology`: use during investigation when selecting reproducibility metrics and interpreting HF baseline; owns keeps ML evidence and limitations explicit.
...
1. Read the required context using the bounded protocol and verify Stage `04` prerequisites in the ledger.
2. Record `User required before start`, prompt path/hash, planned concrete file list, and any initial blockers in `docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/04-hf-reproducibility.md`.
