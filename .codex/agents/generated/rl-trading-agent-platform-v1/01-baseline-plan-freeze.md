---
prompt_name: 01-baseline-plan-freeze
repo: roehub.com
branch: main
branch_policy:
  default_branch: main
...
scope: "Repair-only baseline and plan freeze prompt for already accepted Stage 01."
language:
...
skill_routing:
  ...
  - skill: architecture-review
    use_when: "Stage 01 plan or ledger facts appear inconsistent"
    timing: "before implementation"
    reason: "reviews plan readiness without implementation"
...
quality_gates:
  - cmd: "python -m tools.docs.generate_docs_index --check"
    expect: "passes after any Markdown repair"
validation_strategy:
  depth: docs_review
  e2e_required: false
  acceptance_surfaces:
  ...
  evidence_target: docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/01-baseline-plan-freeze.md
...
Repair or recreate Stage 01 baseline artifacts only if the ledger says Stage 01 is missing, inconsistent, or not accepted. If Stage 01 is already accepted, do not rewrite accepted plan/report content; record that this prompt is archival and proceed no further.

Done means:

- Stage 01 accepted state is confirmed or repaired.
- Plan, ledger, Stage 01 report, and docs index are consistent.
- No implementation/runtime changes are made.
...
- open_items:
  - Confirm or repair the RL architecture plan and ledger baseline.
...
- Verify the stage prerequisites before implementation. No previous RL stage is required. If Stage 01 is already accepted, treat this prompt as repair-only and do not rewrite accepted artifacts unless the ledger is inconsistent.
- Compute this prompt hash with `shasum -a 256 .codex/agents/generated/rl-trading-agent-platform-v1/01-baseline-plan-freeze.md` and record the prompt path and hash in the stage report.
...
- Keep the change bounded to Stage `01`. Do not start later stages or silently repair unrelated legacy issues.
- Update or create the stage report at `docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/01-baseline-plan-freeze.md` using the Stage 01 report shape: scope, file manifest, observed state, contract impact, quality gates, evidence, blockers, and next-stage handoff.
...
- If the ledger already marks Stage 01 accepted, treat this prompt as no-op repair-only.
- Do not change implementation code, schemas, API, UI, runtime services, or ML artifacts.
- If repairing, preserve existing accepted facts unless current evidence proves drift.
...
- `architecture-review`: use before implementation when Stage 01 plan or ledger facts appear inconsistent; owns reviews plan readiness without implementation.
...
1. Read the required context using the bounded protocol and verify Stage `01` prerequisites in the ledger.
2. Record `User required before start`, prompt path/hash, planned concrete file list, and any initial blockers in `docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/01-baseline-plan-freeze.md`.
...
- Stage 01 status is accepted or a blocker is recorded with evidence.
- Docs index check passes after any Markdown repair.
- No later stage is started.
- The stage report exists at `docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/01-baseline-plan-freeze.md` and includes prompt path/hash plus a strict file manifest.
...
- `python -m tools.docs.generate_docs_index --check` - passes after any Markdown repair
