---
prompt_name: 08d-original-hf-backtest-evaluation
repo: roehub.com
branch: main
branch_policy:
  default_branch: main
...
required_literals:
  ...
  - "08D"
non_goals:
  - "Do not train a new model in Stage 08D."
...
expected_primary_touches:
  ...
  - "docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/08d-original-hf-backtest-evaluation.md"
...
- Previous-stage ledger gate: before any edits or evaluation launch, read the ledger and verify Stage `08C` is `accepted`, provides `hf_original_candidate`, and `current_stage` allows Stage `08D`. If not true, write/update the Stage `08D` report as `blocked`, update the ledger, and stop.
...
- File manifest gate: every created, modified, deleted, and runtime artifact path must be listed in the Stage `08D` report and ledger update; any file outside expected paths requires explicit outside-manifest justification and must not be changed speculatively.
- Evaluate the `best` checkpoint by default; `final` may be diagnostic only unless the report explains otherwise.
- Use train-only normalization stats from `08C`.
