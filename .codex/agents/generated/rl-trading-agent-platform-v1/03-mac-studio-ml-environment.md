---
prompt_name: 03-mac-studio-ml-environment
repo: roehub.com
branch: main
scope: "Create isolated Mac Studio ML dependency/runtime environment with CPU/MPS evidence and no API torch dependency."
language:
...
context_sources:
  ...
  task_entrypoints:
    ...
    - path: docs/runbooks/mac-studio-native-backend-operations.md
      why: "Mac Studio operations style"
      inspect_symbols:
        - "runtime paths"
        - "service operations"
    - path: scripts/macos/smoke_prod.sh
      why: "runtime smoke pattern"
      inspect_symbols:
        - "smoke checks"
...
skill_routing:
  ...
  - skill: backend-performance-evidence
    use_when: "measuring CPU/MPS/RSS/thread performance"
    timing: "during verification"
    reason: "requires comparable benchmark evidence"
  ...
  - skill: publish-ci-deploy
    use_when: "accepted runtime/env changes need Mac Studio deployment or smoke evidence"
    timing: "before ship"
...
validation_strategy:
  ...
  evidence_target: docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/03-mac-studio-ml-environment.md
...
Implement Stage 03 Mac Studio ML environment. Add an isolated ML dependency/runtime path for PyTorch on Mac Studio, prove CPU/MPS availability and resource isolation, and ensure the API runtime does not import or require torch.

Done means:

- ML dependency group or env is documented and verified on Mac Studio.
- `torch.backends.mps.is_available()` and CPU/MPS microbenchmark evidence are recorded.
- API runtime dependency isolation is tested or smoke-verified.
...
- open_items:
  - Prepare the only ML host for training and inference without contaminating production API runtime.
...
- Verify the stage prerequisites before implementation. Required accepted prerequisites: Stage 02C. If any required prerequisite is not accepted in the ledger, stop, write/update the stage report as blocked, update the ledger, and do not implement dependent work.
- Compute this prompt hash with `shasum -a 256 .codex/agents/generated/rl-trading-agent-platform-v1/03-mac-studio-ml-environment.md` and record the prompt path and hash in the stage report.
...
- Keep the change bounded to Stage `03`. Do not start later stages or silently repair unrelated legacy issues.
- Update or create the stage report at `docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/03-mac-studio-ml-environment.md` using the Stage 01 report shape: scope, file manifest, observed state, contract impact, quality gates, evidence, blockers, and next-stage handoff.
...
- Do not add torch to the default API dependency path.
- Record accepted device policy: MPS preferred, CPU fallback, or blocked with reason.
- Measure wall-clock, RSS, CPU/thread counts, and MPS availability for a small tensor/training smoke.
- Define resource isolation limits for trainer, inference, backtest jobs, disk watermarks, and concurrency.
- Use Mac Studio path contract for remote checks and runtime evidence.
...
- `backend-performance-evidence`: use during verification when measuring CPU/MPS/RSS/thread performance; requires comparable benchmark evidence.
...
- `publish-ci-deploy`: use before ship when accepted runtime/env changes need Mac Studio deployment or smoke evidence; records CI/deploy/Mac Studio verification after GitHub publication or merge.

1. Read the required context using the bounded protocol and verify Stage `03` prerequisites in the ledger.
2. Record `User required before start`, prompt path/hash, planned concrete file list, and any initial blockers in `docs/architecture/ml/rl-trading-agent-platform-v1-stage-reports/03-mac-studio-ml-environment.md`.
