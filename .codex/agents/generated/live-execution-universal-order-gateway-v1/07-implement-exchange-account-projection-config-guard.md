---
prompt_name: 'live-execution-v1-stage-07-implement-exchange-account-projection-config-guard'
repo: 'roehub.com'
branch: 'main'
scope: 'Build read-only exchange account state projection and verify-only instrument/account configuration guard.'
language: 'en'
context_sources:
  always_read:
  - '.codex/AGENTS.md'
...
documentation_continuity:
  ...
  stage_report: 'docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/07-exchange-account-projection-config-guard.md'
...
hard_requirements:
  ...
  - 'Use only read-only exchange operations in this stage; auto-config of leverage, margin mode, position mode, precision, or filters is forbidden.'
  - 'Persist account/balance/position/open-order/filter snapshots without secrets or signed payloads.'
  - 'Expose freshness and config mismatch reasons for readiness/risk.'
  - 'Implement /strategies account readiness UI for fresh, stale, degraded, and config_mismatch states without showing balances as a portfolio dashboard or exposing secrets/raw exchange payloads.'
  - 'Hot path must read local projection only, not exchange account state directly.'
task_toggles:
...
skill_routing:
  - 'contract-impact-analysis for schema/config/readiness contracts'
  - 'backend-quality-gates during verification'
  - 'browser-qa-evidence plus playwright for mandatory /strategies account readiness UI proof'
  - 'publish-ci-deploy after validation'
target_envs:
  - 'local-dev'
  - 'exchange-testnet-readonly-if-available'
  - 'metrics-runtime'
  - 'browser-runtime'
required_literals:
  - '07'
  - 'docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/07-exchange-account-projection-config-guard.md'
  - 'fresh'
  - 'stale'
  - 'degraded'
  - 'config_mismatch'
...
quality_gates:
  - 'uv run ruff check src/trading/contexts/live_execution src/trading/contexts/identity apps tests'
  - 'uv run pyright src/trading/contexts/live_execution src/trading/contexts/identity apps tests'
  - 'uv run pytest -q tests/unit/contexts/live_execution tests/unit/contexts/identity tests/unit/apps'
  - 'python -m tools.docs.generate_docs_index --check'
  - 'git diff --check'
validation_strategy:
  depth: 'Real boundary evidence is mandatory; local tests are supporting gates only.'
  acceptance_surfaces:
      - 'Run safe read-only adapter/sync call against available testnet or configured sandbox; skip external calls only with explicit env-var skip policy and blocker record.'
      - 'Query snapshot tables and confirm redaction.'
      - 'Check metrics/health endpoints for sync/config guard counters.'
      - 'Use Playwright/browser proof on /strategies for fresh, stale, degraded, and config_mismatch readiness display, with console/network checks and no secrets in DOM/screenshots.'
...
stage_execution_ledger:
  ...
  previous_stage_requirement: 'Stage 06 must be accepted in docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/live-execution-universal-order-gateway-v1-iteration-ledger.md before starting.'
...
expected_primary_touches:
  - 'src/trading/contexts/live_execution/domain/**account**'
  - 'src/trading/contexts/live_execution/application/**account**'
  - 'src/trading/contexts/live_execution/adapters/outbound/**account**'
  - 'src/trading/contexts/live_execution/**config_guard**'
  - 'apps/workers*'
  - 'apps/web/templates/pages/strategies.html'
  - 'apps/web/dist/js/pages/strategies.js'
  - 'apps/web/dist/css/pages/strategies.css'
...
---
# Task
Implement Stage 07: Implement Exchange Account Projection and Config Guard.

Build read-only exchange account state projection and verify-only instrument/account configuration guard.

## Context / Current State
This prompt implements one stage of `docs/architecture/live_execution/live-execution-universal-order-gateway-v1.md`. The stage ledger is `docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/live-execution-universal-order-gateway-v1-iteration-ledger.md` and the stage report must be `docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/07-exchange-account-projection-config-guard.md`.

Previous-stage rule: Stage 06 must be accepted in docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/live-execution-universal-order-gateway-v1-iteration-ledger.md before starting.
...
- Real read-only account sync/config guard call or worker proves fresh, stale, degraded, and config-mismatch states.
- DB snapshots prove balances, positions, open orders, filters/config requirements, age/source hash, and no secrets.
- Metrics/health prove sync result, staleness gauge, and config guard counters.
- UI/API readiness shows fresh/stale/degraded/config_mismatch reason on /strategies; no exchange submit, auto-config, secrets, or raw exchange payloads are observed.
