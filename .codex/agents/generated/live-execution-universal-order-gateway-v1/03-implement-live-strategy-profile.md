---
prompt_name: 'live-execution-v1-stage-03-implement-live-strategy-profile'
repo: 'roehub.com'
branch: 'main'
scope: 'Add LiveStrategyProfile for mode, exchange binding, sizing, limits, readiness, and safe live-mode recent-auth behavior.'
language: 'en'
context_sources:
  always_read:
  - '.codex/AGENTS.md'
...
documentation_continuity:
  ...
  stage_report: 'docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/03-live-strategy-profile.md'
...
hard_requirements:
  ...
  - 'Model profile fields for mode monitor_only|paper|live, exchange connection binding, sizing, per-run limits, readiness status, and readiness reason.'
  - 'Default newly created profile to a non-money-moving mode; live mode requires explicit recent-auth and existing eligible exchange connection.'
  - 'Do not decrypt exchange credentials or call exchange submit from this stage.'
  - 'Expose a UI profile panel on strategy detail with clear blocked/readiness state.'
task_toggles:
...
skill_routing:
  - 'contract-impact-analysis before API/schema changes'
  - 'backend-quality-gates during verification'
  - 'browser-qa-evidence plus playwright for strategy UI proof'
...
quality_gates:
  - 'uv run ruff check src/trading/contexts/strategy apps/api tests'
  - 'uv run pyright src/trading/contexts/strategy apps/api tests'
  - 'uv run pytest -q tests/unit/contexts/strategy tests/unit/apps'
  - 'python -m tools.docs.generate_docs_index --check'
  - 'git diff --check'
validation_strategy:
  depth: 'Real boundary evidence is mandatory; local tests are supporting gates only.'
  acceptance_surfaces:
      - 'HTTP calls through strategy profile routes for create/update/readiness.'
      - 'SQL verification of profile rows and readiness reasons.'
      - 'Browser verification on /strategies detail.'
      - 'Audit/metrics evidence for profile changes and readiness outcomes.'
...
stage_execution_ledger:
  ...
  previous_stage_requirement: 'Stage 02 must be accepted in docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/live-execution-universal-order-gateway-v1-iteration-ledger.md before starting.'
...
expected_primary_touches:
  - 'src/trading/contexts/strategy/domain/**live_profile**'
  - 'src/trading/contexts/strategy/application/**profile**'
  - 'src/trading/contexts/strategy/adapters/outbound/persistence/**profile**'
...
---
# Task
Implement Stage 03: Implement LiveStrategyProfile.

Add LiveStrategyProfile for mode, exchange binding, sizing, limits, readiness, and safe live-mode recent-auth behavior.

## Context / Current State
This prompt implements one stage of `docs/architecture/live_execution/live-execution-universal-order-gateway-v1.md`. The stage ledger is `docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/live-execution-universal-order-gateway-v1-iteration-ledger.md` and the stage report must be `docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/03-live-strategy-profile.md`.

Previous-stage rule: Stage 02 must be accepted in docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/live-execution-universal-order-gateway-v1-iteration-ledger.md before starting.
...
- Profile create/update/readiness API calls succeed for monitor_only and paper.
- Live-mode update without recent-auth fails closed; success path is proven only if recent-auth infrastructure is available.
- DB rows prove profile mode/status/reasons and no secrets.
- Playwright proves strategy detail profile view, safe default, blocked reason, and persistence after refresh.
