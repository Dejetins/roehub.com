---
prompt_name: 'live-execution-v1-stage-16-implement-producer-integrations-notifications'
repo: 'roehub.com'
branch: 'main'
scope: 'Wire Strategy, Manual, ML, and Ops producers through one source-event/risk/ledger path and add redacted notification outbox.'
language: 'en'
context_sources:
  always_read:
  - '.codex/AGENTS.md'
...
documentation_continuity:
  ...
  stage_report: 'docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/16-producer-integrations-notifications.md'
...
hard_requirements:
  ...
  - 'All producers must write ExecutionSourceEvent and pass the same ingress, risk gate, ledger, dispatch, and reconciliation path.'
  - 'ML agent must not call exchange SDK/API/secrets and must not bypass risk gate.'
  - 'Strategy UI must show the link signal -> source event -> intent -> execution outcome where data exists.'
  - 'Notification outbox must emit redacted events for rejected, fill, unknown, kill switch, and terminal outcomes with bounded labels/fields.'
task_toggles:
...
skill_routing:
  - 'contract-impact-analysis for producer/notification contracts'
...
required_literals:
  - '16'
  - 'docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/16-producer-integrations-notifications.md'
  - 'strategy_signal'
  - 'manual_request'
  - 'ml_agent_decision'
  - 'ops_test'
...
validation_strategy:
  ...
  acceptance_surfaces:
      - 'Use API/runtime producer calls for every source type.'
      - 'Use SQL checks for source-event/intents/outbox rows and links.'
      - 'Use Redis checks for accepted dispatch only.'
      - 'Use Playwright/browser proof for strategy path and notification UI or admin/history surface.'
...
stage_execution_ledger:
  ...
  previous_stage_requirement: 'Stage 15 must be accepted in docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/live-execution-universal-order-gateway-v1-iteration-ledger.md before starting.'
...
expected_primary_touches:
  - 'src/trading/contexts/strategy/**producer**'
  - 'src/trading/contexts/live_execution/application/**source**'
  - 'src/trading/contexts/live_execution/domain/**notification**'
  - 'src/trading/contexts/live_execution/adapters/outbound/persistence/**notification**'
  - 'apps/api/routes/**'
  - 'apps/web/templates/pages/strategies.html'
  - 'apps/web/dist/js/pages/strategies.js'
  - 'apps/web/dist/css/pages/strategies.css'
...
---
# Task
Implement Stage 16: Implement Producer Integrations and Notifications.

Wire Strategy, Manual, ML, and Ops producers through one source-event/risk/ledger path and add redacted notification outbox.
...
Previous-stage rule: Stage 15 must be accepted in docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/live-execution-universal-order-gateway-v1-iteration-ledger.md before starting.
...
- Real calls for strategy, manual, ML, and ops producers create source events and eligible intents through one ingress.
- DB rows prove source refs, source_event_id links, no-intent outcomes, and notification outbox rows.
- Accepted safe intents dispatch through the same Redis stream; rejected/no-intent paths do not.
- Playwright proves strategy signal -> source event -> intent -> execution outcome link, and notification visibility for rejected, fill, unknown, kill-switch, and terminal outcomes that this stage creates.
