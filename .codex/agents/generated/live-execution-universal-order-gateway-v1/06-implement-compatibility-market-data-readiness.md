---
prompt_name: 'live-execution-v1-stage-06-implement-compatibility-market-data-readiness'
repo: 'roehub.com'
branch: 'main'
scope: 'Add strategy compatibility checker and market-data subscription/readiness provisioning before run/paper/live readiness.'
language: 'en'
context_sources:
  always_read:
  - '.codex/AGENTS.md'
...
documentation_continuity:
  ...
  stage_report: 'docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/06-compatibility-market-data-readiness.md'
...
hard_requirements:
  ...
  - 'Return stable compatibility states launchable, not_launchable, and degraded with reason codes.'
  - 'Check market-data readiness states ready, missing, stale, and pending for instrument/timeframe requirements.'
  - 'Persist or expose enough evidence to explain why a profile/run is blocked.'
  - 'Show safe readiness/blocked reasons on /backtests and /strategies.'
task_toggles:
...
skill_routing:
  - 'contract-impact-analysis for readiness DTOs/schema'
...
quality_gates:
  - 'uv run ruff check src/trading/contexts/backtest src/trading/contexts/strategy src/trading/contexts/market_data apps tests'
  - 'uv run pyright src/trading/contexts/backtest src/trading/contexts/strategy src/trading/contexts/market_data apps tests'
  - 'uv run pytest -q tests/unit/contexts/backtest tests/unit/contexts/strategy tests/unit/contexts/market_data tests/unit/apps'
  - 'python -m tools.docs.generate_docs_index --check'
  - 'git diff --check'
validation_strategy:
  depth: 'Real boundary evidence is mandatory; local tests are supporting gates only.'
  acceptance_surfaces:
      - 'HTTP compatibility/readiness calls against real API.'
      - 'SQL checks for compatibility and market-data readiness persistence. If market_data_subscription_requirements is not the chosen table, record the exact equivalent read model and prove it with SQL.'
      - 'Redis XINFO/XRANGE for market-data stream age and absence/presence evidence.'
      - 'Browser proof for readiness on both affected pages.'
...
stage_execution_ledger:
  ...
  previous_stage_requirement: 'Stage 05 must be accepted in docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/live-execution-universal-order-gateway-v1-iteration-ledger.md before starting.'
...
expected_primary_touches:
  - 'src/trading/contexts/strategy/application/**compatibility**'
  - 'src/trading/contexts/strategy/domain/**compatibility**'
  - 'src/trading/contexts/market_data*/**readiness**'
  - 'apps/api/routes/**'
  - 'apps/web/templates/pages/backtests.html'
  - 'apps/web/dist/js/pages/backtests.js'
  - 'apps/web/dist/css/pages/backtests.css'
...
---
# Task
Implement Stage 06: Implement Compatibility Checker and Market-Data Readiness.

Add strategy compatibility checker and market-data subscription/readiness provisioning before run/paper/live readiness.

## Context / Current State
This prompt implements one stage of `docs/architecture/live_execution/live-execution-universal-order-gateway-v1.md`. The stage ledger is `docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/live-execution-universal-order-gateway-v1-iteration-ledger.md` and the stage report must be `docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/06-compatibility-market-data-readiness.md`.

Previous-stage rule: Stage 05 must be accepted in docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/live-execution-universal-order-gateway-v1-iteration-ledger.md before starting.
...
- Real API calls prove launchable/not_launchable/degraded, forbidden owner, replay/idempotency, and feed ready/missing/stale/pending states.
- DB rows or durable read models prove variant/profile hash, feed requirement, status, and reason code.
- Redis XINFO proves ready/stale/missing candle streams; execution stream is absent.
- Playwright proves /backtests and /strategies readiness display and blocked launch behavior.
