---
prompt_name: 'live-execution-v1-stage-04-harden-strategy-run-stop-restart'
repo: 'roehub.com'
branch: 'main'
scope: 'Make run/stop/restart durable and explicit, with one-active-run invariant and UI controls.'
language: 'en'
context_sources:
  always_read:
  - '.codex/AGENTS.md'
...
documentation_continuity:
  ...
  stage_report: 'docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/04-run-stop-restart-lifecycle.md'
...
hard_requirements:
  ...
  - 'Implement restart as its own durable command or operation, not only a UI alias for stop plus run.'
  - 'Preserve existing run/stop compatibility while enforcing one active run per strategy.'
  - 'Define and prove stopping/draining/failed/unknown-state behavior.'
  - 'Add UI run/stop/restart controls with stable post-refresh state.'
task_toggles:
...
skill_routing:
  - 'root-cause-debugging if existing run/stop behavior regresses'
  - 'contract-impact-analysis for route/DTO changes'
...
validation_strategy:
  ...
  acceptance_surfaces:
      - 'HTTP calls through strategy lifecycle endpoints.'
      - 'SQL queries against strategy_runs and operation/event rows.'
      - 'Worker pickup/drain evidence is required. If Redis is not part of the existing worker path, state that explicitly and prove the actual worker boundary used.'
      - 'Browser runtime proof on strategy detail.'
...
stage_execution_ledger:
  ...
  previous_stage_requirement: 'Stage 03 must be accepted in docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/live-execution-universal-order-gateway-v1-iteration-ledger.md before starting.'
...
expected_primary_touches:
  - 'src/trading/contexts/strategy/application/use_cases/**restart**'
  - 'src/trading/contexts/strategy/application/use_cases/**run**'
  - 'src/trading/contexts/strategy/adapters/outbound/persistence/**run**'
  - 'apps/api/routes/strategies.py'
  - 'apps/web/templates/pages/strategies.html'
  - 'apps/web/dist/js/pages/strategies.js'
  - 'apps/web/dist/css/pages/strategies.css'
...
---
# Task
Implement Stage 04: Harden Strategy Run Stop Restart Lifecycle.

Make run/stop/restart durable and explicit, with one-active-run invariant and UI controls.

## Context / Current State
This prompt implements one stage of `docs/architecture/live_execution/live-execution-universal-order-gateway-v1.md`. The stage ledger is `docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/live-execution-universal-order-gateway-v1-iteration-ledger.md` and the stage report must be `docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/04-run-stop-restart-lifecycle.md`.

Previous-stage rule: Stage 03 must be accepted in docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/live-execution-universal-order-gateway-v1-iteration-ledger.md before starting.
...
- Real run, stop, restart, duplicate restart, active conflict, and stopping conflict calls are proven.
- DB rows prove operation ordering and at most one active run.
- Worker pickup/drain evidence proves lifecycle side effects through the actual worker boundary.
- Playwright proves controls, disabled states, refresh behavior, and no duplicate active run.
