---
prompt_name: 'live-execution-v1-stage-02-implement-backtest-variant-strategy-creation'
repo: 'roehub.com'
branch: 'main'
scope: 'Add owner-scoped immutable strategy creation from a launchable backtest variant, including /backtests UI create flow.'
language: 'en'
context_sources:
  always_read:
  - '.codex/AGENTS.md'
...
documentation_continuity:
  ...
  stage_report: 'docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/02-backtest-variant-strategy-creation.md'
...
hard_requirements:
  ...
  - 'Create strategies only from owner-scoped launchable variants; forbidden and not-launchable variants must fail closed with stable reason codes.'
  - 'Persist immutable provenance: source job id, variant id/key, strategy spec/hash, owner, created strategy id, and idempotency key.'
  - 'Implement duplicate replay as idempotent success or explicit duplicate result without creating a second strategy.'
  - 'Add /backtests UI create-from-variant CTA/modal that does not imply live/mainnet launch.'
task_toggles:
...
skill_routing:
  - 'contract-impact-analysis before finalizing API/DTO/persistence changes'
  - 'backend-quality-gates during verification'
  - 'browser-qa-evidence plus playwright for /backtests runtime proof'
...
quality_gates:
  - 'uv run ruff check src/trading/contexts/backtest src/trading/contexts/strategy apps/api tests'
  - 'uv run pyright src/trading/contexts/backtest src/trading/contexts/strategy apps/api tests'
  - 'uv run pytest -q tests/unit/contexts/backtest tests/unit/contexts/strategy tests/unit/apps'
  - 'python -m tools.docs.generate_docs_index --check'
  - 'git diff --check'
validation_strategy:
  depth: 'Real boundary evidence is mandatory; local tests are supporting gates only.'
  acceptance_surfaces:
      - 'Use real HTTP calls through apps/api, not direct use-case invocation only.'
      - 'Use SQL queries to prove persisted provenance and duplicate behavior.'
      - 'Use Playwright/browser evidence for the /backtests UI flow.'
      - 'Record metrics/audit evidence for the create-from-variant path, including strategy_variant_launch_total or the repository-standard equivalent metric/audit event.'
...
stage_execution_ledger:
  ...
  previous_stage_requirement: 'Stage 01 must be accepted in docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/live-execution-universal-order-gateway-v1-iteration-ledger.md before starting.'
...
---
# Task
Implement Stage 02: Implement Backtest Variant to Strategy Creation.

Add owner-scoped immutable strategy creation from a launchable backtest variant, including /backtests UI create flow.

## Context / Current State
This prompt implements one stage of `docs/architecture/live_execution/live-execution-universal-order-gateway-v1.md`. The stage ledger is `docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/live-execution-universal-order-gateway-v1-iteration-ledger.md` and the stage report must be `docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/02-backtest-variant-strategy-creation.md`.

Previous-stage rule: Stage 01 must be accepted in docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/live-execution-universal-order-gateway-v1-iteration-ledger.md before starting.
...
- Persist immutable provenance: source job id, variant id/key, strategy spec/hash, owner, created strategy id, and idempotency key.
- Implement duplicate replay as idempotent success or explicit duplicate result without creating a second strategy.
- Add /backtests UI create-from-variant CTA/modal for the create strategy from variant flow; it must not imply live/mainnet launch.
...
- GET variant and POST create-from-variant calls succeed for a valid owned launchable variant.
- Replay with the same idempotency key does not create another strategy.
- Forbidden owner, missing variant, and not-launchable cases are proven by real API calls.
- DB rows prove strategy provenance and no duplicates.
- Playwright proves /backtests variant detail -> create strategy flow and safe rejected-state UI.
