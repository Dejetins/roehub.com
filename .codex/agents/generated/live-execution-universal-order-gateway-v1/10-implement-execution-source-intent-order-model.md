---
prompt_name: 'live-execution-v1-stage-10-implement-execution-source-intent-order-model'
repo: 'roehub.com'
branch: 'main'
scope: 'Create producer-neutral ExecutionSourceEvent, ExecutionRequest, explicit v1 order model, source registry, and durable ExecutionIntent persistence.'
language: 'en'
context_sources:
  always_read:
  - '.codex/AGENTS.md'
...
documentation_continuity:
  ...
  stage_report: 'docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/10-execution-source-intent-order-model.md'
...
hard_requirements:
  ...
  - 'Support source types strategy_signal, manual_request, ml_agent_decision, and ops_test through one ingress path.'
  - 'Define v1 order allowlist explicitly as market and limit only.'
  - 'Reject OCO, trailing, TP/SL, amend/replace, multi-leg, and any unsupported order model before Redis/order submit.'
  - 'Persist source event and intent links, producer-specific source refs, strategy_signal_id for strategy_signal sources, idempotency, and durable status.'
task_toggles:
...
skill_routing:
  - 'contract-impact-analysis before API/schema/order model finalization'
...
required_literals:
  - '10'
  - 'docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/10-execution-source-intent-order-model.md'
...
validation_strategy:
  ...
  acceptance_surfaces:
      - 'HTTP/API calls for every source type and invalid/duplicate cases.'
      - 'SQL checks for source event and intent rows plus the partition/retention scaffolding required by the plan, or a documented blocker if the stage cannot safely introduce it.'
      - 'Redis XINFO proves dispatch stream is absent or empty by contract.'
      - 'Metrics/audit evidence for source events, intents, and order-model rejection.'
...
stage_execution_ledger:
  ...
  previous_stage_requirement: 'Stage 09 must be accepted in docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/live-execution-universal-order-gateway-v1-iteration-ledger.md before starting.'
...
expected_primary_touches:
  - 'src/trading/contexts/live_execution/domain/**source**'
  - 'src/trading/contexts/live_execution/domain/**intent**'
  - 'src/trading/contexts/live_execution/domain/**order**'
  - 'src/trading/contexts/live_execution/application/**intent**'
  - 'src/trading/contexts/live_execution/adapters/outbound/persistence/**intent**'
...
---
# Task
Implement Stage 10: Implement Execution Source Event and Intent Order Model.

Create producer-neutral ExecutionSourceEvent, ExecutionRequest, explicit v1 order model, source registry, and durable ExecutionIntent persistence.
...
Previous-stage rule: Stage 09 must be accepted in docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/live-execution-universal-order-gateway-v1-iteration-ledger.md before starting.
...
- Real POST calls create valid source events and intents for strategy/manual/ML/ops sources.
- Duplicate idempotency calls do not create duplicate intents.
- Invalid source policy and unsupported order model calls are rejected with stable reasons.
- DB rows prove execution_source_events, execution_intents, source links, strategy_signal_id, order model fields, and no Redis/order submit.
