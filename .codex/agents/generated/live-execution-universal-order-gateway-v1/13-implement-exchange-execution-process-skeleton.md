---
prompt_name: 'live-execution-v1-stage-13-implement-exchange-execution-process-skeleton'
repo: 'roehub.com'
branch: 'main'
scope: 'Create supervised exchange-execution runtime boundary with health, metrics, Redis consumption, limiter/backpressure/DLQ and clock drift guards, but adapters disabled.'
language: 'en'
context_sources:
  always_read:
  - '.codex/AGENTS.md'
...
documentation_continuity:
  ...
  stage_report: 'docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/13-exchange-execution-process-skeleton.md'
...
hard_requirements:
  ...
  - 'Create a separate exchange-execution process boundary; do not embed money-moving runtime inside Strategy or apps/api.'
  - 'Expose health/ready and metrics for dependency state, limiter/backpressure, DLQ, and clock drift.'
  - 'Consume or observe Redis using safe disabled-adapter mode; no exchange submit is allowed in this stage.'
  - 'Add launchd/Monit/Prometheus config and runbook with fail-closed defaults.'
task_toggles:
...
skill_routing:
  - 'contract-impact-analysis for runtime/config/health contracts'
  - 'backend-quality-gates during verification'
  - 'backend-performance-evidence for hot-path/latency claims'
...
required_literals:
  - '13'
  - 'docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/13-exchange-execution-process-skeleton.md'
  - 'exchange-execution'
  - '/health/ready'
  - 'Monit'
  - 'Prometheus'
...
validation_strategy:
  ...
  acceptance_surfaces:
      - 'Start exchange-execution locally or through project service wrapper and call health endpoints.'
      - 'Use redis-cli to prove consumer group/DLQ state.'
      - 'Use monit, launchctl, and Prometheus scrape evidence in the target runtime. If one of these runtime surfaces is absent, record the exact environment blocker and keep the stage blocked for production readiness until resolved.'
      - 'Run secret/log grep to prove no sensitive data leakage.'
...
stage_execution_ledger:
  ...
  previous_stage_requirement: 'Stage 12 must be accepted in docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/live-execution-universal-order-gateway-v1-iteration-ledger.md before starting.'
...
expected_primary_touches:
  - 'apps/exchange_execution/**'
  - 'src/trading/contexts/live_execution/**exchange_execution**'
  - 'configs/**exchange**'
  - 'deploy/**'
...
---
# Task
Implement Stage 13: Implement Exchange-Execution Process Skeleton.

Create supervised exchange-execution runtime boundary with health, metrics, Redis consumption, limiter/backpressure/DLQ and clock drift guards, but adapters disabled.
...
Previous-stage rule: Stage 12 must be accepted in docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/live-execution-universal-order-gateway-v1-iteration-ledger.md before starting.
...
- Local GET /health/ready proves ready/degraded states for DB, Redis, config, limiter, DLQ, and clock drift.
- Redis consumer group and DLQ are visible; no submit calls occur.
- launchd/Monit/Prometheus evidence proves service supervision and metrics scraping.
- Process heartbeat/lock rows are proven when implemented; if the repository-standard supervision model uses another durable runtime proof, record that exact proof and why heartbeat rows are non-applicable.
