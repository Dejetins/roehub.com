---
prompt_name: 'live-execution-v1-stage-08-implement-strategy-position-ownership-lock'
repo: 'roehub.com'
branch: 'main'
scope: 'Prevent multiple active strategies from controlling the same exchange connection, market type, and instrument.'
language: 'en'
context_sources:
  always_read:
  - '.codex/AGENTS.md'
...
documentation_continuity:
  ...
  stage_report: 'docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/08-strategy-position-ownership-lock.md'
...
hard_requirements:
  ...
  - 'Enforce unique active ownership for exchange_connection_id + market_type + instrument_key.'
  - 'Define states reserved, active, releasing, released, and stale/repair where needed.'
  - 'Block run/profile activation when another strategy owns the position scope.'
  - 'Expose safe conflict reason in UI without unsafe override.'
task_toggles:
...
skill_routing:
  - 'contract-impact-analysis for persistence/readiness impact'
...
required_literals:
  - '08'
  - 'docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/08-strategy-position-ownership-lock.md'
...
validation_strategy:
  ...
  acceptance_surfaces:
      - 'HTTP calls for two strategies/profiles against one connection+instrument.'
      - 'SQL uniqueness and state transition checks.'
      - 'Metrics/audit evidence for ownership acquire/conflict/release.'
      - 'Browser proof on strategy UI conflict/readiness state.'
...
stage_execution_ledger:
  ...
  previous_stage_requirement: 'Stage 07 must be accepted in docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/live-execution-universal-order-gateway-v1-iteration-ledger.md before starting.'
...
expected_primary_touches:
  - 'src/trading/contexts/strategy/domain/**ownership**'
  - 'src/trading/contexts/strategy/application/**ownership**'
  - 'src/trading/contexts/strategy/adapters/outbound/persistence/**ownership**'
...
---
# Task
Implement Stage 08: Implement Strategy Position Ownership Lock.

Prevent multiple active strategies from controlling the same exchange connection, market type, and instrument.

## Context / Current State
This prompt implements one stage of `docs/architecture/live_execution/live-execution-universal-order-gateway-v1.md`. The stage ledger is `docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/live-execution-universal-order-gateway-v1-iteration-ledger.md` and the stage report must be `docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/08-strategy-position-ownership-lock.md`.

Previous-stage rule: Stage 07 must be accepted in docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/live-execution-universal-order-gateway-v1-iteration-ledger.md before starting.
...
- Two run/profile attempts on the same connection+instrument prove first owner success and second conflict.
- Stop/release proves the lock can be released and a later strategy can acquire it.
- DB rows prove ownership state transitions and unique active invariant.
- Playwright proves conflict display and no unsafe override path.
