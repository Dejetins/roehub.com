---
prompt_name: 'live-execution-v1-stage-12-implement-redis-dispatch-transport'
repo: 'roehub.com'
branch: 'main'
scope: 'Dispatch only accepted durable intents to Redis with retry budget, backpressure, DLQ/quarantine, and ack-after-durable semantics.'
language: 'en'
context_sources:
  always_read:
  - '.codex/AGENTS.md'
...
documentation_continuity:
  ...
  stage_report: 'docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/12-redis-dispatch-transport.md'
...
hard_requirements:
  ...
  - 'Define stream names, consumer groups, retry stream, DLQ/quarantine stream, retry budget, and poison-message behavior.'
  - 'Dispatch only accepted durable intents after DB commit; never dispatch rejected or unknown intents.'
  - 'Use ack-after-durable-state-change semantics for any consumer that changes money-related state.'
  - 'Handle Redis outage and duplicate/replay without order submission.'
task_toggles:
...
skill_routing:
  - 'contract-impact-analysis for Redis/config/status contracts'
  - 'backend-quality-gates during verification'
  - 'backend-performance-evidence for dispatch latency/backpressure evidence'
...
required_literals:
  - '12'
  - 'docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/12-redis-dispatch-transport.md'
  - 'XINFO'
  - 'XREADGROUP'
  - 'XPENDING'
  - 'DLQ'
...
quality_gates:
  - 'uv run ruff check src/trading/contexts/live_execution apps configs tests'
...
validation_strategy:
  ...
  acceptance_surfaces:
      - 'Create accepted/rejected intents through API and observe Redis streams with redis-cli.'
      - 'Inject duplicate, poison, and outage/recovery scenarios safely.'
      - 'Query DB transitions and retry budget metadata.'
      - 'Scrape metrics for dispatch and DLQ evidence.'
...
stage_execution_ledger:
  ...
  previous_stage_requirement: 'Stage 11 must be accepted in docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/live-execution-universal-order-gateway-v1-iteration-ledger.md before starting.'
...
expected_primary_touches:
  - 'src/trading/contexts/live_execution/adapters/outbound/redis/**'
  - 'src/trading/contexts/live_execution/application/**dispatch**'
  - 'configs/**'
...
---
# Task
Implement Stage 12: Implement Redis Dispatch Transport.

Dispatch only accepted durable intents to Redis with retry budget, backpressure, DLQ/quarantine, and ack-after-durable semantics.

## Context / Current State
This prompt implements one stage of `docs/architecture/live_execution/live-execution-universal-order-gateway-v1.md`. The stage ledger is `docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/live-execution-universal-order-gateway-v1-iteration-ledger.md` and the stage report must be `docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/12-redis-dispatch-transport.md`.

Previous-stage rule: Stage 11 must be accepted in docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/live-execution-universal-order-gateway-v1-iteration-ledger.md before starting.
...
- Accepted intent creates dispatch state and Redis message; rejected intent does not.
- XINFO, XREADGROUP, XPENDING, retry stream, DLQ stream, duplicate replay, and outage recovery are proven by real Redis commands/messages.
- DB state transitions prove accepted/dispatching/dispatched/retry/quarantined.
- Metrics prove dispatch, retry, DLQ, backpressure, and Redis error counters.
