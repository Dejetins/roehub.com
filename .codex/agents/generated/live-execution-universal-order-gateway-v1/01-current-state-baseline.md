---
prompt_name: 'live-execution-v1-stage-01-current-state-baseline'
repo: 'roehub.com'
branch: 'main'
scope: 'Inventory the current Backtest, Strategy, Live Runner, Redis, exchange connections, bindings, and runtime ops surfaces before implementation starts.'
language: 'en'
context_sources:
  always_read:
  - '.codex/AGENTS.md'
...
documentation_continuity:
  ...
  stage_report: 'docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/01-current-state-baseline.md'
...
hard_requirements:
  ...
  - 'Create a factual baseline report only; do not implement product code in this stage.'
  - 'Inventory current API routes, DB tables, Redis streams, browser pages, metrics endpoints, Monit/launchd services, and known missing components.'
  - 'Explicitly identify the absence or presence of create-from-backtest-variant, live profile, restart, signal journal, execution source events, Redis execution dispatch, exchange-execution, order adapters, reconciliation, and notifications.'
task_toggles:
...
skill_routing:
  - 'architecture-review before writing the baseline conclusions'
  - 'browser-qa-evidence plus playwright only for browser inventory if runtime is available'
  - 'publish-ci-deploy only after the docs-only stage is validated and ready to publish'
target_envs:
  - 'local-dev'
  - 'macstudio-prod-readonly-if-available'
required_literals:
  - '01'
  - 'docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/01-current-state-baseline.md'
  - '/backtests'
  - '/strategies'
  - '/settings'
...
validation_strategy:
  ...
  acceptance_surfaces:
      - 'curl current API endpoints for backtests, strategies, exchange connections, and health/metrics. If a documented endpoint or metrics surface is absent, record the exact absence as baseline evidence.'
      - 'Run read-only SQL metadata queries against the local/dev database.'
      - 'Run redis-cli XINFO/SCAN read-only commands for current streams/groups.'
      - 'Capture monit summary and service/process inventory. If Monit or the service manager is not installed in the target environment, record that exact environment fact as baseline evidence.'
      - 'Use browser-qa-evidence plus playwright or Browser for page inventory if the web runtime is available.'
...
stage_execution_ledger:
  ...
  previous_stage_requirement: 'No previous stage is required, but the ledger must exist and be updated.'
...
- Real API calls prove current backtest/strategy/exchange endpoints and the absence of live execution submit paths.
- SQL inventory proves existing tables and missing live profile/execution ledgers.
- Redis XINFO proves existing market/strategy streams and missing execution streams.
- Ops evidence records current Monit/launchd/metrics state.
- Browser inventory records visible /backtests, /strategies, and /settings behavior when runtime is available.
