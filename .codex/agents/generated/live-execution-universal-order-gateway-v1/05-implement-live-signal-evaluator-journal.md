---
prompt_name: 'live-execution-v1-stage-05-implement-live-signal-evaluator-journal'
repo: 'roehub.com'
branch: 'main'
scope: 'Run strategy signal evaluation on controlled live candles and persist StrategySignal/no-signal journal without order submission.'
language: 'en'
context_sources:
  always_read:
  - '.codex/AGENTS.md'
...
documentation_continuity:
  ...
  stage_report: 'docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/05-live-signal-evaluator-journal.md'
...
hard_requirements:
  ...
  - 'Evaluate supported backtest-created strategy specs from controlled closed candles.'
  - 'Persist durable StrategySignal and no-signal journal entries with mode behavior for monitor_only, paper, and live readiness.'
  - 'Do not create execution intents, Redis execution messages, or exchange calls in this stage.'
  - 'Expose latest signals/journal in the strategy UI.'
task_toggles:
...
skill_routing:
  - 'contract-impact-analysis for new signal journal schema/API'
...
quality_gates:
  - 'uv run ruff check src/trading/contexts/strategy apps tests'
  - 'uv run pyright src/trading/contexts/strategy apps tests'
...
validation_strategy:
  ...
  acceptance_surfaces:
      - 'Inject or replay controlled market-data candles through the real runtime boundary used by the runner.'
      - 'Query StrategySignal/journal tables after evaluation.'
      - 'Use Redis XINFO/XRANGE for market-data stream evidence and absence of execution streams.'
      - 'Use Playwright/browser proof for latest signal journal.'
...
stage_execution_ledger:
  ...
  previous_stage_requirement: 'Stage 04 must be accepted in docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/live-execution-universal-order-gateway-v1-iteration-ledger.md before starting.'
...
expected_primary_touches:
  - 'src/trading/contexts/strategy/domain/**signal**'
  - 'src/trading/contexts/strategy/application/**signal**'
  - 'src/trading/contexts/strategy/adapters/outbound/persistence/**signal**'
  - 'apps/workers*'
  - 'apps/web/templates/pages/strategies.html'
  - 'apps/web/dist/js/pages/strategies.js'
  - 'apps/web/dist/css/pages/strategies.css'
...
---
# Task
Implement Stage 05: Implement Live Signal Evaluator and Journal.

Run strategy signal evaluation on controlled live candles and persist StrategySignal/no-signal journal without order submission.

## Context / Current State
This prompt implements one stage of `docs/architecture/live_execution/live-execution-universal-order-gateway-v1.md`. The stage ledger is `docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/live-execution-universal-order-gateway-v1-iteration-ledger.md` and the stage report must be `docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/05-live-signal-evaluator-journal.md`.

Previous-stage rule: Stage 04 must be accepted in docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/live-execution-universal-order-gateway-v1-iteration-ledger.md before starting.
...
- Controlled candle input creates warmup, signal, and no-signal outcomes through the live runner.
- DB rows prove signal/no-signal journal, mode, source strategy/run/profile, and unsupported variant blocking.
- Redis evidence proves Market-data Redis stream input and absence of execution dispatch.
- UI/API latest signal journal is visible and consistent after refresh.
