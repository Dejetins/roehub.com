---
prompt_name: 'live-execution-v1-stage-15-implement-order-fill-reconciliation-pitr'
repo: 'roehub.com'
branch: 'main'
scope: 'Add durable order/fill/funding ledger, private status updates, reconciliation, retention/partitioning, and backup/PITR proof.'
language: 'en'
context_sources:
  always_read:
  - '.codex/AGENTS.md'
...
documentation_continuity:
  ...
  stage_report: 'docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/15-order-fill-reconciliation-pitr.md'
...
hard_requirements:
  ...
  - 'Persist order events, fills, funding events, reconciliation runs, terminal states, and provider-state references without sensitive payloads.'
  - 'Handle private stream reconnect/backfill/dedupe and exchange state lookup after gaps or unknown submit status.'
  - 'Define retention, partitioning, archive, indexes, and PITR restore proof for money ledger tables.'
  - 'Reconcile fee/funding against testnet/provider facts when the provider exposes them. If funding facts are unavailable in the safe environment, mark funding/PnL completeness as pending and record the blocker; do not report complete PnL.'
task_toggles:
...
skill_routing:
  - 'contract-impact-analysis for ledger/retention contracts'
  - 'backend-quality-gates during verification'
  - 'backend-performance-evidence for reconciliation/retention metrics if claimed'
  - 'publish-ci-deploy after validation'
target_envs:
  - 'local-dev'
  - 'exchange-testnet'
  - 'postgres-backup-runtime'
  - 'metrics-runtime'
required_literals:
  - '15'
  - 'docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/15-order-fill-reconciliation-pitr.md'
...
validation_strategy:
  ...
  acceptance_surfaces:
      - 'Run safe testnet order lifecycle or adapter simulator plus exchange state lookup for gap recovery.'
      - 'Query DB ledger/reconciliation/partition/retention state.'
      - 'Restart private stream/session and prove backfill/dedupe.'
      - 'Run backup/PITR dry-run or target-host equivalent and record evidence path.'
...
stage_execution_ledger:
  ...
  previous_stage_requirement: 'Stage 14 must be accepted in docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/live-execution-universal-order-gateway-v1-iteration-ledger.md before starting.'
...
expected_primary_touches:
  - 'src/trading/contexts/live_execution/domain/**fill**'
  - 'src/trading/contexts/live_execution/domain/**funding**'
  - 'src/trading/contexts/live_execution/application/**reconciliation**'
  - 'src/trading/contexts/live_execution/adapters/outbound/persistence/**ledger**'
  - 'apps/exchange_execution/**private_stream**'
...
---
# Task
Implement Stage 15: Implement Order Fill Reconciliation and PITR Proof.

Add durable order/fill/funding ledger, private status updates, reconciliation, retention/partitioning, and backup/PITR proof.
...
Previous-stage rule: Stage 14 must be accepted in docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/live-execution-universal-order-gateway-v1-iteration-ledger.md before starting.
...
- Testnet lifecycle plus restart/gap scenario converges local ledger with exchange state.
- DB rows prove order events, fills, funding events, reconciliation runs, partitions/indexes/retention metadata, and terminal state.
- Private stream reconnect/backfill/dedupe metrics and logs are proven without secrets.
- Backup/PITR restore dry-run evidence is recorded for ledger durability.
