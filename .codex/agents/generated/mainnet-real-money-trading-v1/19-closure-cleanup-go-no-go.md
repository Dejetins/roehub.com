---
prompt_name: mainnet-real-money-trading-v1-19-closure-cleanup-go-no-go
repo: /Users/daniildegtyarev/Projects/roehub.com
branch: main
scope: "Closure cleanup and go/no-go record"
language:
...
skill_routing:
  - skill: pre-ship-gate
    use_when: "stage work crosses the pre-ship-gate boundary"
...
required_literals:
  - "User required before start: none unless residual open position/unknown state needs operator action"
  - "previous stage"
  - "file manifest"
non_goals:
...
change_ownership:
  ...
  owned_change_scope: ["Stage 19 scoped files/hunks only", "docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/19-closure-cleanup-go-no-go.md", "docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/mainnet-real-money-trading-v1-stage-ledger.md"]
...
validation_strategy:
  ...
  e2e_required: true
  acceptance_surfaces: ["browser/API/DB/Redis/Prometheus/Monit/docs", "stage report", "stage ledger"]
  tests_only_allowed_reason: null
  evidence_target: docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/19-closure-cleanup-go-no-go.md
proof_boundary:
...
file_manifest:
  ...
  expected_groups:
    ...
    docs_runbooks: ["docs/runbooks/**", "docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/19-closure-cleanup-go-no-go.md", "docs/architecture/README.md"]
...
expected_primary_touches: ["docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/19-closure-cleanup-go-no-go.md", "docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/mainnet-real-money-trading-v1-stage-ledger.md"]
...
safety_notes:
  - "closure must not broaden mainnet access"
  - "No blind retry after unknown provider state."
---

# Task

Closure cleanup and go/no-go record.
...
- previous-stage ledger gate / previous stage: before any implementation or runtime action, read `stage_ledger` and confirm Stage `18` is `accepted`. If not, update Stage `19` as `blocked`, write the blocker in `stage_ledger`, and stop.
...
- Record `User required before start: none unless residual open position/unknown state needs operator action` in the stage report and ledger.
- Do not place new orders.
- Verify no residual open orders, positions, unknown states, retry/DLQ growth or unresolved reconciliation.
- Verify metrics, alerts, user notifications, runbooks and browser/API final state.
- Mark ledger completed only if every stage is accepted and mainnet remains scoped.
- Update `docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/19-closure-cleanup-go-no-go.md` with evidence, blockers, file manifest, contract impact and next-stage handoff.
...
- Stage `19` is not accepted unless: No unexpected open orders/positions, no unknown/DLQ/retry growth, metrics/dashboard/alerts verified, ledgers updated, mainnet expansion remains scoped.
...
- Primary: `docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/19-closure-cleanup-go-no-go.md`, `docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/mainnet-real-money-trading-v1-stage-ledger.md`.
