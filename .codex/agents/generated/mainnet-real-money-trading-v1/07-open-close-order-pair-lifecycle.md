---
prompt_name: mainnet-real-money-trading-v1-07-open-close-order-pair-lifecycle
repo: /Users/daniildegtyarev/Projects/roehub.com
branch: main
scope: "Open/close order-pair lifecycle and auto-close model"
language:
...
required_literals:
  - "User required before start: nothing beyond accepted Stage 06"
  - "previous stage"
  - "file manifest"
non_goals:
...
change_ownership:
  ...
  owned_change_scope: ["Stage 07 scoped files/hunks only", "docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/07-open-close-order-pair-lifecycle.md", "docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/mainnet-real-money-trading-v1-stage-ledger.md"]
...
validation_strategy:
  ...
  e2e_required: true
  acceptance_surfaces: ["domain/API/DB/Redis tests and no-submit runtime proof", "stage report", "stage ledger"]
  tests_only_allowed_reason: null
  evidence_target: docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/07-open-close-order-pair-lifecycle.md
proof_boundary:
...
file_manifest:
  ...
  expected_groups:
    ...
    docs_runbooks: ["docs/runbooks/**", "docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/07-open-close-order-pair-lifecycle.md", "docs/architecture/README.md"]
...
expected_primary_touches: ["docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/07-open-close-order-pair-lifecycle.md", "docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/mainnet-real-money-trading-v1-stage-ledger.md"]
...
safety_notes:
  - "cancel_after_submit is not close proof for market orders"
  - "No blind retry after unknown provider state."
---

# Task

Open/close order-pair lifecycle and auto-close model.
...
- Record `User required before start: nothing beyond accepted Stage 06` in the stage report and ledger.
- Add intent_role=open|close and durable linkage between open and close orders.
- Represent parent_order_id, close_for_position_id, reduce_only, position_mode, auto_close_required, auto_close_deadline, auto_close_status and close_failure_reason or equivalent.
- For spot, close by actual acquired base quantity after fill, with dust/min-notional audit.
- For futures, require reduce-only close in one-way/net mode.
- Represent terminal states opened, close_submitted, closed, close_failed and unknown_requires_manual.
- Block any real canary until this lifecycle is proven.
- Update `docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/07-open-close-order-pair-lifecycle.md` with evidence, blockers, file manifest, contract impact and next-stage handoff.
...
- Stage `07` is not accepted unless: Durable open/close pair lifecycle, close linkage, spot filled-qty close, futures reduce-only close, partial-fill and close-failure states are proven without mainnet submit.
...
- Primary: `docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/07-open-close-order-pair-lifecycle.md`, `docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/mainnet-real-money-trading-v1-stage-ledger.md`.
