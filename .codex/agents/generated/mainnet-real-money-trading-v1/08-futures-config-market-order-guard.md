---
prompt_name: mainnet-real-money-trading-v1-08-futures-config-market-order-guard
repo: /Users/daniildegtyarev/Projects/roehub.com
branch: main
scope: "Futures account config and market-order guard"
language:
...
required_literals:
  - "User required before start: user approves default isolated 1x or explicit safe futures config"
  - "previous stage"
  - "file manifest"
non_goals:
...
change_ownership:
  ...
  owned_change_scope: ["Stage 08 scoped files/hunks only", "docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/08-futures-config-market-order-guard.md", "docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/mainnet-real-money-trading-v1-stage-ledger.md"]
...
validation_strategy:
  ...
  e2e_required: true
  acceptance_surfaces: ["exchange config read-back plus no-submit guard", "stage report", "stage ledger"]
  tests_only_allowed_reason: null
  evidence_target: docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/08-futures-config-market-order-guard.md
proof_boundary:
...
file_manifest:
  ...
  expected_groups:
    ...
    docs_runbooks: ["docs/runbooks/**", "docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/08-futures-config-market-order-guard.md", "docs/architecture/README.md"]
...
expected_primary_touches: ["docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/08-futures-config-market-order-guard.md", "docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/mainnet-real-money-trading-v1-stage-ledger.md"]
...
safety_notes:
  - "Hidden config mutation inside submit is forbidden"
  - "No blind retry after unknown provider state."
---

# Task

Futures account config and market-order guard.
...
- Record `User required before start: user approves default isolated 1x or explicit safe futures config` in the stage report and ledger.
- Preflight connection ownership, environment=mainnet, market_type=futures and symbol=BTCUSDT.
- Prove open orders=0 and existing position qty=0 before changing futures config.
- Set/read back isolated 1x by default or user-selected safe config.
- Require one-way/net position mode for v1.
- Persist config observed_at and source_hash linked to readiness snapshot/canary token.
- Prove submit path does not silently change leverage, margin or position mode.
- Update `docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/08-futures-config-market-order-guard.md` with evidence, blockers, file manifest, contract impact and next-stage handoff.
...
- Stage `08` is not accepted unless: Binance/Bybit futures config is set/read back, no open orders/positions preflight passes, fresh config snapshot is linked to canary scope; no strategy order yet.
...
- Primary: `docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/08-futures-config-market-order-guard.md`, `docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/mainnet-real-money-trading-v1-stage-ledger.md`.
