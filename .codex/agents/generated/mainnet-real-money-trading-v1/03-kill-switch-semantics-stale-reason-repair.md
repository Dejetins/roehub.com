---
prompt_name: mainnet-real-money-trading-v1-03-kill-switch-semantics-stale-reason-repair
repo: /Users/daniildegtyarev/Projects/roehub.com
branch: main
scope: "Kill-switch semantics and stale reason repair"
language:
...
required_literals:
  - "User required before start: nothing beyond accepted Stage 02"
  - "previous stage"
  - "file manifest"
non_goals:
...
change_ownership:
  ...
  owned_change_scope: ["Stage 03 scoped files/hunks only", "docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/03-kill-switch-semantics-stale-reason-repair.md", "docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/mainnet-real-money-trading-v1-stage-ledger.md"]
...
validation_strategy:
  ...
  e2e_required: true
  acceptance_surfaces: ["API/DTO/domain/docs/tests/browser if visible", "stage report", "stage ledger"]
  tests_only_allowed_reason: null
  evidence_target: docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/03-kill-switch-semantics-stale-reason-repair.md
proof_boundary:
  required_when: "Mac Studio or production runtime proof is collected"
  label: post_main_production_runtime_proof for changed code
...
file_manifest:
  ...
  expected_groups:
    ...
    docs_runbooks: ["docs/runbooks/**", "docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/03-kill-switch-semantics-stale-reason-repair.md", "docs/architecture/README.md"]
...
expected_primary_touches: ["docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/03-kill-switch-semantics-stale-reason-repair.md", "docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/mainnet-real-money-trading-v1-stage-ledger.md"]
...
safety_notes:
  - "Unsafe kill-switch polarity can enable trading when operator expects stop"
  - "No blind retry after unknown provider state."
---

# Task

Kill-switch semantics and stale reason repair.
...
- Record `User required before start: nothing beyond accepted Stage 02` in the stage report and ledger.
- Resolve the existing kill_switch_open semantic drift before caps or submit stages.
- Prefer kill_switch_active=true for emergency stop or execution_enabled=true for allow semantics; do not keep ambiguous meaning.
- Update reason codes so active stop rejects trading and inactive stop permits only when other gates pass.
- Sweep stale mainnet stage copy: stage19_20, stage05, stage13 and similar obsolete operational reasons.
- Add tests proving normal trading is allowed only when stop is inactive and rejected when active.
- Update `docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/03-kill-switch-semantics-stale-reason-repair.md` with evidence, blockers, file manifest, contract impact and next-stage handoff.
...
- Stage `03` is not accepted unless: kill_switch_active/execution_enabled semantics are fixed or explicitly mapped, stale stage reason/copy drift is repaired, docs/runbooks/tests agree; no order submit.
...
- Primary: `docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/03-kill-switch-semantics-stale-reason-repair.md`, `docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/mainnet-real-money-trading-v1-stage-ledger.md`.
