---
prompt_name: mainnet-real-money-trading-v1-05-metrics-alerts-user-alert-contract
repo: /Users/daniildegtyarev/Projects/roehub.com
branch: main
scope: "Mainnet metrics, alerts, dashboard and user-alert contract"
language:
...
required_literals:
  - "User required before start: Telegram gate accepted"
  - "previous stage"
  - "file manifest"
non_goals:
...
change_ownership:
  ...
  owned_change_scope: ["Stage 05 scoped files/hunks only", "docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/05-metrics-alerts-user-alert-contract.md", "docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/mainnet-real-money-trading-v1-stage-ledger.md"]
...
validation_strategy:
  ...
  e2e_required: true
  acceptance_surfaces: ["Prometheus/runbook/notification runtime", "stage report", "stage ledger"]
  tests_only_allowed_reason: null
  evidence_target: docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/05-metrics-alerts-user-alert-contract.md
proof_boundary:
...
file_manifest:
  ...
  expected_groups:
    ...
    docs_runbooks: ["docs/runbooks/**", "docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/05-metrics-alerts-user-alert-contract.md", "docs/architecture/README.md"]
...
expected_primary_touches: ["docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/05-metrics-alerts-user-alert-contract.md", "docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/mainnet-real-money-trading-v1-stage-ledger.md"]
...
safety_notes:
  - "Metrics must not use invalid histogram base names or mixed units"
  - "No blind retry after unknown provider state."
---

# Task

Mainnet metrics, alerts, dashboard and user-alert contract.
...
- previous-stage ledger gate / previous stage: before any implementation or runtime action, read `stage_ledger` and confirm Stage `04` is `accepted`. If not, update Stage `05` as `blocked`, write the blocker in `stage_ledger`, and stop.
...
- Record `User required before start: Telegram gate accepted` in the stage report and ledger.
- Use histogram base names without _bucket: Prometheus will generate _bucket, _sum and _count series.
- Use seconds for latency metrics and do not mix with existing millisecond metrics without explicit conversion.
- Add mainnet latency, slippage, reconciliation, unknown-state, exposure, gross notional, auto-close and user-alert delivery metrics.
- Add alert severity, owner and runbook actions for kill switch active, unknown order state, auto-close failure, stale stream/fallback, high latency/slippage and failed user alert.
- Update `docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/05-metrics-alerts-user-alert-contract.md` with evidence, blockers, file manifest, contract impact and next-stage handoff.
...
- Stage `05` is not accepted unless: Prometheus base metric names in seconds, alerts, dashboard reference, user-alert contract and runbooks are proven; no order submit.
...
- Primary: `docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/05-metrics-alerts-user-alert-contract.md`, `docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/mainnet-real-money-trading-v1-stage-ledger.md`.
