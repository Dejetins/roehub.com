---
prompt_name: mainnet-real-money-trading-v1-18-strategy-driven-mainnet-canaries
repo: /Users/daniildegtyarev/Projects/roehub.com
branch: main
scope: "Strategy-driven mainnet canaries per market"
language:
...
task_toggles:
  ...
  allow_mainnet_orders: true
skill_routing:
  - skill: backend-performance-evidence
    use_when: "stage work crosses the backend-performance-evidence boundary"
...
required_literals:
  - "User required before start: explicit scoped automatic strategy canary window approval"
  - "previous stage"
  - "file manifest"
non_goals:
...
change_ownership:
  ...
  owned_change_scope: ["Stage 18 scoped files/hunks only", "docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/18-strategy-driven-mainnet-canaries.md", "docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/mainnet-real-money-trading-v1-stage-ledger.md"]
...
validation_strategy:
  ...
  e2e_required: true
  acceptance_surfaces: ["live candles/strategy signal/real orders/fills/reconcile/alerts/browser/API/metrics", "stage report", "stage ledger"]
  tests_only_allowed_reason: null
  evidence_target: docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/18-strategy-driven-mainnet-canaries.md
proof_boundary:
...
file_manifest:
  ...
  expected_groups:
    ...
    docs_runbooks: ["docs/runbooks/**", "docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/18-strategy-driven-mainnet-canaries.md", "docs/architecture/README.md"]
...
expected_primary_touches: ["docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/18-strategy-driven-mainnet-canaries.md", "docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/mainnet-real-money-trading-v1-stage-ledger.md"]
...
safety_notes:
  - "automatic real-money strategy execution; stop on first unknown"
  - "No blind retry after unknown provider state."
---

# Task

Strategy-driven mainnet canaries per market.
...
- previous-stage ledger gate / previous stage: before any implementation or runtime action, read `stage_ledger` and confirm Stage `17` is `accepted`. If not, update Stage `18` as `blocked`, write the blocker in `stage_ledger`, and stop.
...
- Record `User required before start: explicit scoped automatic strategy canary window approval` in the stage report and ledger.
- Run scoped automatic strategy canaries only inside the approved window and allowlists.
- Execute one signal per required market surface and stop that scope after first canary.
- Measure candle close -> StrategySignal -> ExecutionSourceEvent -> intent -> risk -> Redis -> exchange ack -> fill -> reconciliation -> alert.
- Auto-close immediately and prove no duplicates or residual exposure.
- Do not run a long soak or broaden live strategy access.
- Update `docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/18-strategy-driven-mainnet-canaries.md` with evidence, blockers, file manifest, contract impact and next-stage handoff.
...
- Stage `18` is not accepted unless: One real strategy signal per required market surface executes automatically under allowlist/caps, auto-closes, records candle-to-fill-to-alert latency, and stops after first canary per scope.
...
- Primary: `docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/18-strategy-driven-mainnet-canaries.md`, `docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/mainnet-real-money-trading-v1-stage-ledger.md`.
