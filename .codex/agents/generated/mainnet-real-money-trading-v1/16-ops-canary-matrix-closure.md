---
prompt_name: mainnet-real-money-trading-v1-16-ops-canary-matrix-closure
repo: /Users/daniildegtyarev/Projects/roehub.com
branch: main
scope: "Ops canary matrix closure"
language:
...
required_literals:
  - "User required before start: Stage 15 accepted"
  - "previous stage"
  - "file manifest"
non_goals:
...
change_ownership:
  ...
  owned_change_scope: ["Stage 16 scoped files/hunks only", "docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/16-ops-canary-matrix-closure.md", "docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/mainnet-real-money-trading-v1-stage-ledger.md"]
...
validation_strategy:
  ...
  e2e_required: true
  acceptance_surfaces: ["DB/Redis/Prometheus/Monit/browser/API", "stage report", "stage ledger"]
  tests_only_allowed_reason: null
  evidence_target: docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/16-ops-canary-matrix-closure.md
proof_boundary:
...
file_manifest:
  ...
  expected_groups:
    ...
    docs_runbooks: ["docs/runbooks/**", "docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/16-ops-canary-matrix-closure.md", "docs/architecture/README.md"]
...
expected_primary_touches: ["docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/16-ops-canary-matrix-closure.md", "docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/mainnet-real-money-trading-v1-stage-ledger.md"]
...
safety_notes:
  - "must prove the whole matrix is safe before strategy live mode"
  - "No blind retry after unknown provider state."
---

# Task

Ops canary matrix closure.
...
- previous-stage ledger gate / previous stage: before any implementation or runtime action, read `stage_ledger` and confirm Stage `15` is `accepted`. If not, update Stage `16` as `blocked`, write the blocker in `stage_ledger`, and stop.
...
- Record `User required before start: Stage 15 accepted` in the stage report and ledger.
- Do not place new orders.
- Verify no unexpected open orders, positions or residual exposure across all canary surfaces.
- Verify no unknown state, retry/DLQ/pending growth, stale stream/fallback or unreconciled close.
- Summarize latency/slippage/fees/gross notional/open exposure and alerts for all rows.
- Keep mainnet disabled outside accepted canary scopes.
- Update `docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/16-ops-canary-matrix-closure.md` with evidence, blockers, file manifest, contract impact and next-stage handoff.
...
- Stage `16` is not accepted unless: Matrix-wide residual state is clean, no unknown/retry/DLQ growth, alerts delivered, metrics complete, budget/gross/fee audit recorded.
...
- Primary: `docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/16-ops-canary-matrix-closure.md`, `docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/mainnet-real-money-trading-v1-stage-ledger.md`.
