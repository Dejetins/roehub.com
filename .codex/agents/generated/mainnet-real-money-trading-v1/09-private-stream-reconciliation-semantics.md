---
prompt_name: mainnet-real-money-trading-v1-09-private-stream-reconciliation-semantics
repo: /Users/daniildegtyarev/Projects/roehub.com
branch: main
scope: "Private stream or REST fallback and reconciliation semantics"
language:
...
required_literals:
  - "User required before start: nothing beyond accepted Stage 08"
  - "previous stage"
  - "file manifest"
non_goals:
...
change_ownership:
  ...
  owned_change_scope: ["Stage 09 scoped files/hunks only", "docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/09-private-stream-reconciliation-semantics.md", "docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/mainnet-real-money-trading-v1-stage-ledger.md"]
...
validation_strategy:
  ...
  e2e_required: true
  acceptance_surfaces: ["runtime/provider/API/DB/metrics no-submit or safe-readiness evidence", "stage report", "stage ledger"]
  tests_only_allowed_reason: null
  evidence_target: docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/09-private-stream-reconciliation-semantics.md
proof_boundary:
...
file_manifest:
  ...
  expected_groups:
    ...
    docs_runbooks: ["docs/runbooks/**", "docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/09-private-stream-reconciliation-semantics.md", "docs/architecture/README.md"]
...
expected_primary_touches: ["docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/09-private-stream-reconciliation-semantics.md", "docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/mainnet-real-money-trading-v1-stage-ledger.md"]
...
safety_notes:
  - "Testnet auth probe is not mainnet private stream proof"
  - "No blind retry after unknown provider state."
---

# Task

Private stream or REST fallback and reconciliation semantics.
...
- Record `User required before start: nothing beyond accepted Stage 08` in the stage report and ledger.
- Implement/prove private stream lifecycle: connect/auth/heartbeat/order/fill events/reconnect/backfill/freshness age, or record explicit REST polling fallback with stricter thresholds.
- Block submit when private stream/fallback is stale or missing.
- Separate order/fill reconciliation, position reconciliation and funding reconciliation.
- Allow short-lived futures canary closure when order/fill and position match and funding is not due yet.
- Add metrics and alerts for stream age, fallback active and reconciliation lag by type.
- Update `docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/09-private-stream-reconciliation-semantics.md` with evidence, blockers, file manifest, contract impact and next-stage handoff.
...
- Stage `09` is not accepted unless: Mainnet private order/fill stream lifecycle or explicit REST polling fallback is accepted; order/fill, position and funding reconciliation are separated.
...
- Primary: `docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/09-private-stream-reconciliation-semantics.md`, `docs/architecture/live_execution/mainnet-real-money-trading-v1-stage-reports/mainnet-real-money-trading-v1-stage-ledger.md`.
