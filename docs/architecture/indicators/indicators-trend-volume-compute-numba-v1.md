...
`trend.psar` в формуле имеет `*_dir: series<int>`.
...
- `Tier B` (`mixed precision`): `trend.adx`, `trend.vortex`, `trend.psar`, `volume.vwap`.
...
- Trend: `trend.adx`, `trend.psar`, `trend.linreg_slope` (покрывает rolling+stateful+source axis)
- Volume: `volume.vwap`, `volume.mfi`, `volume.obv` (покрывает rolling + cumsum state)
...
- grid: например `trend.adx: window 5..120 × smoothing 5..50` (guarded subset),
  и `volume.vwap: window 5..200` (guarded subset)
...
2) Реализованы Numba kernels для всех:
   - Trend: `adx, aroon, donchian, linreg_slope, psar, vortex`
   - Volume: `ad_line, cmf, mfi, obv, volume_sma, vwap`
