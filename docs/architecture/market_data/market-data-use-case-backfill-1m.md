...
### 3) Batching policy
Фиксируем v1 константу:

- `BATCH_SIZE = 50_000` строк

Пояснение:
- ClickHouse лучше принимает крупные вставки;
- batching ограничивает память и стабилизирует throughput.

(В будущем `BATCH_SIZE` может стать конфигом, но в v1 это implementation detail use-case.)
