...
| `02B` Feature and live-feed contract | pending | `02b-feature-live-feed-contract.md` | feature contract + live-feed parity decision + Binance Futures metadata gate | TBD | Stage `02A` accepted; must freeze `binance:futures` training feature/live-feed/futures metadata contract and record non-training branches as `blocked_not_training_source_v1` | no |
...
| Stage `02B` must output a training-source matrix: `binance:futures` is the only v1 training branch, while `binance:spot`, `bybit:spot`, and `bybit:futures` are `blocked_not_training_source_v1`; it must also decide Binance Futures live-feed `trades_count|blocked`. | Prevents accidental training on spot/Bybit data and prevents incomplete feature/funding/contract/live-feed coverage from reaching training. | `rl-trading-agent-platform-v1.md`; `02a-data-source-inventory.md`; Redis live candle payload currently publishes OHLCV and `volume_quote`, not `trades_count` |
