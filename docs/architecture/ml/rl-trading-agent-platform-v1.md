...
18. Stage `08L` is the active fail-closed research branch after accepted `08I3`/`08I4`/`08J` and blocked `08K`. It may investigate reward shaping, supervised warm-start/behavior cloning, or contextual-bandit sanity, but it cannot silently replace the frozen reward/action contract or open Stage `09` without an accepted candidate scorecard.
