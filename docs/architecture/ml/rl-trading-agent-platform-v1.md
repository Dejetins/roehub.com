...
19. Stage `08M` is the active candidate-scorecard branch after accepted `08L`. It must convert the warm-start research proxy into a bounded final-holdout scorecard and may open Stage `09` only if the strict native gate records `stage09_allowed=true`.
20. `training_run` создает candidate model от frozen config. Для дообучения допустимы два режима:
...
21. `candidate` никогда не активируется автоматически. Promotion требует:
...
| `08M` | Supervised warm-start candidate scorecard | Convert accepted `08L` proxy research into a bounded final-holdout candidate scorecard on the accepted Stage `08J` article-selector surface. | accepted `08L`, accepted `08I3`, accepted `08I4`, accepted `08J`, blocked `08K` | active next stage: strict native final-holdout scorecard, baseline/stability/concentration decision, no registry or activation; may open `09` only with `stage09_allowed=true`. |
| `09` | Model registry and activation gates | Persist datasets/models/calibrations with hashes, registry state machine, artifact lifecycle, checkpoint security and candidate/champion activation lifecycle. | accepted `08I3`, accepted `08I4`, accepted `08J`, and accepted research candidate from `08K`, `08M`, or a later explicitly accepted corrective stage with `stage09_allowed=true` | registry state-machine invariant tests, API/use-case tests, corrupt/missing hash block, safe checkpoint load evidence, retention/quota config, activation/deactivation audit. |
