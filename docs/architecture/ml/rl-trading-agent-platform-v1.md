...
Correction decision on 2026-07-01: the chain `Stage 06 current selector + current features + realized-only sparse reward + DQN + action filter + Optuna` is retained as non-working evidence for Roehub-native quality. It must not be retried as the active path without a new accepted research decision. At that point the next executable stage became `08I`, not `09` and not another full training run. `08I` had to first prove or disprove step-level parity between the original `backtest_engine.py` and Roehub evaluator on the same HF checkpoint/config/data. The 2026-07-02 correction below now requires `08I2` before `08J`, `08K`, or new training. `90/60` remains failed/future research evidence, not the current article-reproduction path.
...
Correction decision on 2026-07-02: Stage `08I` found a real blocker, but that is not enough to conclude that all source-vs-Roehub methodology drift is understood. The next executable stage is `08I2`, an exhaustive methodology discrepancy audit. It must check every diagnosis surface before any repair conclusion, `08J`, `08K`, `09`, or new training. The required matrix rows are:
...
`08I2` must not stop at the first material diff. If a blocker prevents a deeper dynamic comparison, it still must record every row as `checked_no_gap`, `gap`, `blocked_by_prior_gap`, or `not_applicable_with_source_reason`, with the exact repair or recheck required. Stage `08J` can start only after this matrix is accepted and no unresolved material evaluator/session blocker remains.
...
13. Stage `08I2` performs the exhaustive methodology discrepancy audit across all diagnosis surfaces. It is now the current next stage and must be completed before `08J`, repairs, or retraining conclusions.
14. Stage `08J` materializes `article_future_10m_5pct_contrast_v1` as a separate Roehub-native dataset variant. It does not replace historical Stage `06` artifacts, does not train a model, and remains blocked until `08I2` is accepted.
15. Stage `08K` runs the next full article/demo-profile candidate path on `30/10`: HF-original control plus Roehub-native article-selector training, `Optuna`, final holdout, strict baseline-beating and stability gates.
16. Stage `08L` is a fail-closed research branch only after `08I2`/`08J` are accepted and `08K` is blocked. It may investigate reward shaping, supervised warm-start/behavior cloning, or contextual-bandit sanity, but it cannot silently replace the frozen reward/action contract or open Stage `09` without an accepted candidate scorecard.
17. `training_run` создает candidate model от frozen config. Для дообучения допустимы два режима:
...
| Level | Meaning | Gate |
|---|---|---|
| `research_candidate` | Модель может быть сохранена и анализироваться offline. | Stage `08K` or a later explicitly accepted corrective stage: positive final holdout PnL after costs, final PnL greater than best sanity baseline on the same surface, sufficient trade count, full scorecard, no zero-trade `Optuna` selection, no calibration-only overfit, no single-symbol/month domination, no pathological action bias, accepted exhaustive methodology discrepancy audit from `08I2`, and accepted article-selector dataset from `08J`. Stage `08D`/`08F`/`08G`/`08H` remain evidence and diagnostics, not enough for Stage `09`. |
...
| `08I2` | Exhaustive methodology discrepancy audit | Check every source-vs-Roehub diagnosis surface before repair conclusions, `08J`, `08K`, `09`, or new training. | blocked `08I`, accepted `08A`-`08D`, accepted HF dataset Stage `04` | Complete discrepancy matrix for session extractor, dataset geometry, past-only signal, reward, action/Q distribution, `Optuna`, baselines and evaluator/backtest parity; every row must be `checked_no_gap`, `gap`, `blocked_by_prior_gap`, or `not_applicable_with_source_reason`. |
| `08J` | Article session extractor dataset | Add article-style `article_future_10m_5pct_contrast_v1` session policy beside Stage `06` and materialize a Roehub-native article-selector dataset variant. | `08I2` | session manifest, split/leakage/lifecycle proof, distribution comparison for HF-original vs Stage `06` current selector vs article selector, no overwrite of Stage `06` artifacts. |
| `08K` | Article demo-profile training/evaluation | Rerun the source/demo `30/10` full workflow on HF-original control and Roehub-native article-selector dataset with `Optuna` and untouched final holdout. | `08I2`, `08J` | HF control plus native article-selector candidate manifests, progress/resource evidence, final holdout scorecards, strict baseline-beating gate, no pathological action bias, explicit Stage `09` allow/block verdict. |
| `08L` | Reward and warm-start research fallback | If `08K` is blocked after accepted discrepancy/parity/extractor work, investigate reward shaping, supervised warm-start, behavior cloning, or contextual-bandit sanity as separate research. | blocked `08K`, accepted `08I2`, accepted `08J` | research comparison report and next-candidate decision; cannot silently replace reward/action contract or open `09` without an accepted research scorecard. |
| `09` | Model registry and activation gates | Persist datasets/models/calibrations with hashes, registry state machine, artifact lifecycle, checkpoint security and candidate/champion activation lifecycle. | accepted `08I2`, accepted `08J`, and accepted research candidate from `08K` or a later explicitly accepted corrective stage | registry state-machine invariant tests, API/use-case tests, corrupt/missing hash block, safe checkpoint load evidence, retention/quota config, activation/deactivation audit. |
...
| `08I2` | Exhaustive methodology discrepancy matrix, row-level source/evidence status, repair backlog, recheck requirements, accepted/blocker report, and updated handoff for `08J`/`08K`. |
...
| Historical Stage `07B` candidate failed Stage `08`; corrective `08F`/`08G`/`08H` also failed native quality gates | Keep all failed candidates as rejected/superseded evidence. Do not register, promote, activate, paper/testnet/live trade, or use them as the baseline for Stage `09`; run the new `08I2` -> `08J` -> `08K` correction chain first, preserving blocked `08I` as first-diff evidence. |
| Article result depends on optimized backtest configuration and article-style session selection | The Habr article shows final backtest results in the workflow that includes `Optuna`; it does not publish a clean pre-`Optuna` final backtest table. Stage `08G` tested source-compatible `Optuna`, and `08H` tested `90/60`, but both failed native quality. Stage `08I` found evaluator parity blockers; Stage `08I2` must now check all methodology drift surfaces before `08J` adds the article session extractor and `08K` reruns the demo `30/10` article path. |
