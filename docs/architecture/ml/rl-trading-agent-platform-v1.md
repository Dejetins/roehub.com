...
| `08N` | Candidate quality reclassification | Reclassify the accepted `08M` candidate before any Stage `17+` runtime/load, soak, mainnet-readiness or rollout progression; decide whether it is promotion-grade, paper/research-only, infrastructure-only, blocked, or insufficient evidence. | accepted `08M`, accepted `09`-`16`, pending `17` | Required before continuing: per-trade, per-ticker, per-month, drawdown/stability, fee/funding/slippage stress and Stage `10` calibration review; explicit booleans for `stage17_infrastructure_only_allowed`, `stage18_soak_allowed`, `stage19_mainnet_readiness_allowed`, `stage20_mainnet_canary_allowed`, `stage21_product_rollout_allowed`. |
