...
| Optuna/tuning | Backtest thresholds and risk-management knobs are tuned through the upstream `optimize_cfg.py` path after model training. The visible article result is part of that tuned backtest workflow; the article does not provide a separate accepted backtest result before `Optuna`. | After the blocked `08F` result, missing `Optuna` was no longer enough for a model-quality conclusion. Stage `08G` ran that check and is blocked; the active quality repair is Stage `08H`, which adds oracle/supervised/session/reward diagnostics and the required `90/60` training profile. |
...
Stage `08G` final result on 2026-06-26: the full CPU-only dual-branch run completed, but did not reopen Stage `09`. HF-original became positive after `Optuna` with final holdout PnL `2914.76906569`; Roehub-native remained negative on final Stage `06` backtest with PnL `-145.16434371` and best sanity baseline `95274.46982886`. Therefore `08G` is blocked and Stage `09` now requires accepted `08H` evidence.

Corrective decision after `08G`: Stage `08H` is the new active research path. It answers the user questions that `08G` left open: whether the selected sessions contain oracle trade opportunities, whether those opportunities are predictable from past-only windows, why HF-original and Roehub-native differ, whether the current realized-only reward is too sparse, and whether the stronger `90/60` profile changes the result. Stage `08H` must not accept a model with a tiny final trade count like `19` or `3` trades without an explicit trade-sufficiency justification. Stage `09` stays blocked until accepted `08H` evidence exists.
...
11. Stage `08H` runs oracle, supervised sanity, selector and reward-sparsity diagnostics for HF-original and Roehub-native splits, then runs the required `90/60` CPU-only dual-branch training/evaluation profile. Only an accepted `08H` can reopen Stage `09`.
...
| Level | Meaning | Gate |
|---|---|---|
| `research_candidate` | Модель может быть сохранена и анализироваться offline. | Stage `08H`: oracle/supervised/session/reward diagnostics, required `90/60` CPU-only dual-branch run, upstream-backed `Optuna` calibration, positive final holdout PnL after costs, sufficient trade count, full scorecard and sanity baselines. Stage `08D`/`08F`/`08G` remain evidence and diagnostics, not enough for Stage `09` after the `08G` block. |
...
| `08H` | Oracle/supervised/selector/reward and 90/60 research repair | Diagnose whether HF-original and Roehub-native sessions contain predictable trade opportunities, then rerun dual-branch CPU training/evaluation with `agent_history_len=90` and `agent_session_len=60`. | blocked `08G`, accepted Stage `04`, accepted Stage `06` | oracle opportunity comparison, past-only supervised sanity, selector regime comparison, reward-sparsity proxy, `90/60` training/evaluation artifacts, trade-count sufficiency evidence, and Stage `09` allow/block verdict. |
| `09` | Model registry and activation gates | Persist datasets/models/calibrations with hashes, registry state machine, artifact lifecycle, checkpoint security and candidate/champion activation lifecycle. | `08H` accepted research candidate | registry state-machine invariant tests, API/use-case tests, corrupt/missing hash block, safe checkpoint load evidence, retention/quota config, activation/deactivation audit. |
...
| `08H` | Oracle/supervised/selector/reward diagnostics summary, `90/60` dual-branch orchestration command, HF-original and Roehub-native `90/60` candidate manifests, Optuna summaries, trade-count sufficiency evidence, final holdout scorecards, and Stage `09` allow/block decision. |
