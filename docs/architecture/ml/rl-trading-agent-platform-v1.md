...
Correction decision on 2026-07-06: after Stage `18` stopped safely as `monitor_only_technical_soak`, the active path returns to model-quality research instead of continuing to Stage `19`. Stage `08K` is no longer treated as a simple dead end because its Roehub-native DQN aggregate return `125.0265333%` is close to the article-level return by one coarse metric, while its blockers show concentration and ticker/regime instability. Stage `08O` must decompose `08K` from existing artifacts, reconcile PnL math, explain volatility-bucket dominance and ticker positive ratio, compare available article metrics beyond return, and decide whether `08K` remains blocked, becomes a restricted research/calibration candidate, is a close article-reproduction candidate but not product-ready, or exposes a scorecard/gate bug. `08O` is `target_host_non_production_forensic_pre_main` only: no training, no `Optuna`, no runtime activation, no registry mutation, no `post_main_production_runtime_proof`, and no Stage `19+` unlock.

Stage `08O` final result on 2026-07-06: accepted. The forensic summary `/opt/roehub/state/rl_trading/evaluation_runs/stage08o_stage08k_dqn_forensic_decomposition_v1/stage08o_stage08k_forensic_summary.json`, sha256 `19a8d1f90f5d7630fb09eeef4c801eec11cdd98bffa172ce410fb79c071355b0`, found no scorecard/gate bug in Stage `08K`: recomputed final PnL is `12502.65333026`, return is `125.0265333026%`, closed trades are `316`, and max drawdown is `-19.44498897306477%`. The volatility-bucket dominance blocker is real (`0.954610281973835` absolute-PnL share from high volatility), and the ticker positive ratio blocker is strict-gate fail-closed (`70/291=0.24054982817869416` when flat selected ticker groups remain in the denominator; active traded tickers only are `70/112=0.625`). Article comparison is aggregate-return-adjacent only, because trade count/cadence differ and source-style accuracy/average trade amount fields are not available in the Stage `08K` artifact. `08k_forensic_status=per_ticker_per_regime_calibration_candidate`, `stage09_for_08k_allowed=false`, `stage19_mainnet_readiness_allowed=false`, `stage20_mainnet_canary_allowed=false`, `stage21_product_rollout_allowed=false`, and `08p_allowed=true`. `08P` remains a required prompt-pack insertion before it can run; no `08P` prompt is generated/runnable by `08O`.
...
20. Stage `08N` reclassifies `08M` before any Stage `17+` progression. This is required because `08M` final return `2.3018418785%` over `4162` trades may be too weak for product or mainnet claims even though it historically opened the registry/plumbing chain. `08N` must append a current quality decision without rewriting historical `08M` facts.
21. Stage `09` accepted the local registry/activation gate implementation. It validates artifact hashes, registry state-machine invariants, lifecycle controls and activation boundaries before any downstream runtime use, but it does not prove backup/restore, production DB apply or runtime activation. Stage `09B` is now the next gate.
22. `training_run` создает candidate model от frozen config. Для дообучения допустимы два режима:
...
23. `candidate` никогда не активируется автоматически. Promotion требует:
...
24. `champion` модель platform-wide. Per-ticker поведение задается calibration/weights/thresholds/head metadata, но не пользовательскими training jobs.
25. `challenger` может работать только в `monitor_only` shadow mode, пока Stage `13+` не подтвердит signal parity, latency и drift evidence.
26. Drift monitoring сравнивает live feature distribution, action distribution, skipped-action reasons и paper/testnet/live outcomes с promotion baseline. Drift сам по себе не обновляет модель; он создает retraining candidate task.
27. Rollback всегда переводит активную модель/калибровку на предыдущий accepted champion без удаления audit trail.
...
| `08N` | Candidate quality reclassification | Reclassify the accepted `08M` candidate before any Stage `17+` runtime/load, soak, mainnet-readiness or rollout progression; decide whether it is promotion-grade, paper/research-only, infrastructure-only, blocked, or insufficient evidence. | accepted `08M`, accepted `09`-`16`; Stage `17`/`18` now accepted only in narrowed non-product scopes | Accepted as `quality_status=accepted_for_research_only`: `stage17_infrastructure_only_allowed=true`, `stage18_monitor_only_technical_soak_allowed=true`, `stage18_soak_allowed=false`, `stage19_mainnet_readiness_allowed=false`, `stage20_mainnet_canary_allowed=false`, `stage21_product_rollout_allowed=false`. |
| `08O` | Stage `08K` DQN forensic decomposition | Deeply decompose the blocked `08K` DQN candidate: math reconciliation, PnL by ticker/month/day/volatility/side/trade, root cause for `single_group_dominates_final_result` and `ticker_stability_obviously_broken`, article metric comparison, allowlist/regime feasibility, scorecard/gate bug audit and next research decision. | accepted `08J`; blocked `08K`; accepted `08L`/`08M`/`08N`; accepted Stage `17` infrastructure-only and Stage `18` monitor-only technical soak; Stage `19` blocked | Accepted: no scorecard/gate bug; `08k_forensic_status=per_ticker_per_regime_calibration_candidate`; summary sha256 `19a8d1f90f5d7630fb09eeef4c801eec11cdd98bffa172ce410fb79c071355b0`; `stage09_for_08k_allowed=false`; `08p_allowed=true` but no `08P` prompt is generated/runnable; no training/tuning/runtime activation and no Stage `19+` unlock. |
...
| `17` | Multi-ticker runtime/load gate | Prove quotas, inference scheduling, market feed, rate limits across tariff-like ticker counts. If `08N` is not promotion-grade, this stage is infrastructure-only and cannot claim model/product quality. | `16`, accepted `08N` with `stage17_infrastructure_only_allowed=true` or `stage17_full_runtime_allowed=true` | p95 latency, CPU/GPU/RSS, Redis lag, no DLQ growth, and explicit `infrastructure_only` vs full-runtime label from `08N`. |
| `18` | 24h/7d RL soak and incident drills | 24h minimum, 7d preferred monitor/paper/testnet soak for selected tickers plus incident drill before mainnet review. Full trade-readiness soak requires `08N` quality allowance; otherwise only bounded monitor-only technical soak may run. | `17`, accepted `08N` with `stage18_soak_allowed=true` or explicit `stage18_monitor_only_technical_soak_allowed=true` | Accepted only as `monitor_only_technical_soak`: max `20` tickers, Stage `17` input accepted, Redis execution stream growth `0`, kill switch/pause/rollback/missing-artifact/stale-feed/unknown-state drills passed, safe/degraded UI state observed, and no order/exchange side effects. 24h/7d/full trade-readiness/product/mainnet claims are not made. |
| `18A` | Stage `08K` постоянный исследовательский монитор | Запустить отдельный непрерывный DQN `08K` worker с past-only article selector, `long_only`, виртуальным закрытием через одну минуту, `ml_agent_decision -> no_intent`, Prometheus, `launchd` и Monit. | accepted `08O`, accepted infrastructure-only `17`, accepted monitor-only technical `18`; Stage `19` остаётся закрыт | Доверенные хеши `08K`, golden train/live parity, один тикер не менее часа, затем отдельные gates для `5/24h` и `20/7d`; `0` intents/orders/dispatch growth, готовый `9213`, bounded metrics, восстановление pending virtual positions и post-main production proof. |
| `19` | Mainnet readiness architecture review | Separate go/no-go review for real-money live enablement, product/legal/support readiness and backup risk. | `18`, accepted `08N` with `stage19_mainnet_readiness_allowed=true` | Blocked after accepted Stage `18` because Stage `08N` records `stage19_mainnet_readiness_allowed=false`. A later accepted quality/governance gate is required before this stage can run. |
| `20` | Bounded mainnet canary | Tiny-capital live canary on approved exchange/market/ticker count. | `19` explicit approval and accepted `08N` with `stage20_mainnet_canary_allowed=true` | real-money canary evidence, stop/close/reconcile proof, no quota bypass. |
| `21` | Product rollout | Enable tariff-limited live RL for broader users. | `20` accepted, accepted `08N` with `stage21_product_rollout_allowed=true`, and backup/support/legal/product gates closed | rollout metrics, support/runbook, alert stability, artifact backup or signed residual-risk exception. |
...
| `08N` | Candidate quality reclassification report for `08M`: per-trade expectancy, per-ticker/time/bucket stability, Stage `10` calibration impact, fee/funding/slippage stress, quality status and explicit downstream booleans for `17`-`21`. |
| `08O` | Stage `08K` DQN forensic report and sanitized JSON summary: artifact hashes, PnL math reconciliation, decomposition by ticker/month/day/volatility/side/trade, article comparison, concentration diagnosis, allowlist/regime feasibility, scorecard/gate bug audit, `08k_forensic_status`, downstream booleans and next prompt. |
...
| `13`-`18` | Inference producer app, launchd/Monit/Prometheus config, source event integration, paper/testnet evidence reports, simulator-paper parity report, incident drill reports. Stage `17`/`18` must carry the `08N` label: infrastructure-only, technical monitor-only soak, or full runtime/soak allowance. |
| `18A` | Доверенный `08K` loader, continuous Redis consumer, локальное восстановимое monitor-state, structured logs, Prometheus rules/target, `launchd`/Monit, runtime report и phased canary evidence. |
| `19`-`21` | Mainnet readiness doc, product/legal/support checklist, runbooks, canary evidence, rollback procedures, backup/residual-risk decision. These stages require explicit positive `08N` quality booleans and cannot inherit `08M stage09_allowed=true`. |
...
| Positive backtest alone is not production promotion | Stage `08K` can accept only a research candidate, and only if it also beats sanity baselines and stability/action gates. Stage `08M` later accepted a supervised warm-start/contextual-bandit candidate with final return `2.3018418785%` over `4162` trades; that aggregate pass is now considered insufficient for automatic Stage `17+` progression. Stage `08N` must reclassify `08M` quality before runtime load, soak, mainnet readiness, canary or product rollout can continue. |
| Accepted `08M` can be mistaken for production-quality RL | `08M stage09_allowed=true` opened the registry/plumbing chain historically, but it did not prove a strong trading edge, per-ticker product suitability or mainnet readiness. Stage `08N` must decide `promotion_grade_candidate`, `paper_research_candidate`, `runtime_infrastructure_only_candidate`, `accepted_for_research_only`, `blocked_for_product`, or `insufficient_evidence`, and write explicit booleans for `stage17_infrastructure_only_allowed`, `stage18_soak_allowed`, `stage19_mainnet_readiness_allowed`, `stage20_mainnet_canary_allowed`, and `stage21_product_rollout_allowed`. |
