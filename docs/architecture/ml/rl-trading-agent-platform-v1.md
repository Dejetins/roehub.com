...
| Optuna/tuning | Backtest thresholds and risk-management knobs are tuned through the upstream `optimize_cfg.py` path after model training. The visible article result is part of that tuned backtest workflow; the article does not provide a separate accepted backtest result before `Optuna`. | After the blocked `08F` result, missing `Optuna` is no longer enough for a model-quality conclusion. Stages `08C`/`08E` remain lifecycle evidence, but any new quality claim must run Stage `08G`: CPU-only retraining/evaluation for HF-original and Roehub-native branches, upstream-search-space `Optuna` calibration, and a separately recorded final holdout verdict. |
...
12. `candidate` никогда не активируется автоматически. Promotion требует:
...
Stage `08G`/`10A` не принимает candidate только по одному числу PnL. Historical Stage `08` is rejection evidence only, and blocked Stage `08F` is fixed-threshold rejection evidence only.

V1 разделяет уровни допуска:

| Level | Meaning | Gate |
|---|---|---|
| `research_candidate` | Модель может быть сохранена и анализироваться offline. | Stage `08G`: CPU-only dual-branch run, upstream-backed `Optuna` calibration, positive final holdout PnL after costs, full scorecard and sanity baselines. Stage `08D`/`08F` remain execution/parity evidence and fixed-threshold diagnostics, not enough for Stage `09` after the `08F` block. |
...
| `09` | Model registry and activation gates | Persist datasets/models/calibrations with hashes, registry state machine, artifact lifecycle, checkpoint security and candidate/champion activation lifecycle. | `08G` accepted research candidate | registry state-machine invariant tests, API/use-case tests, corrupt/missing hash block, safe checkpoint load evidence, retention/quota config, activation/deactivation audit. |
...
| `10` | Per-ticker calibration | Create per-ticker/per-market calibration thresholds, weights, or heads. | `08G`,`09B` | calibration report per ticker, no global-only threshold activation unless accepted. |
| `10A` | Retraining and promotion lifecycle | Add full-retrain/fine-tune command path, manual + scheduled triggers, hard promotion approval contract, candidate/champion gates, drift trigger, rollback manifest, host-local operator command/runbook and internal application/API contract. | `08G`,`09B`,`10` | deterministic rerun, schedule disabled-by-default proof, numeric threshold profile, candidate no-auto-activation proof, host-local rollback command/internal API test, promotion-grade report. |
...
| Article result depends on optimized backtest configuration | The Habr article shows final backtest results in the workflow that includes `Optuna`; it does not publish a clean pre-`Optuna` final backtest table. Stage `08G` must therefore test the source-compatible optimization workflow instead of assuming fixed thresholds should already be profitable. |
| `max_parallel_sessions=2` may be a source default, not an optimum | Stage `08G` treats `max_parallel_sessions=2` as copied from upstream `configs/alpha.py`. It is not a proven Roehub optimum. Because upstream `optimize_cfg.py` leaves `max_sessions` search commented out, changing it requires a separate explicit calibration decision and cannot be silently mixed into the first source-faithful Optuna run. |
| Positive backtest alone is not production promotion | Stage `08G` accepts only research candidates; Stage `10A` requires numeric promotion-grade threshold profile before paper/testnet/live progression. |
