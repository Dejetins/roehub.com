...
| `04` BTCUSDT market readiness | in_progress | `.codex/agents/generated/strategy-producer-paper-testnet-trading-v1/04-btcusdt-market-readiness.md` | `04-btcusdt-market-readiness.md` | Redis/ClickHouse/API/browser | Additive BTCUSDT readiness API/UI matrix implemented locally; parser/whitelist root causes fixed; required local gates passed; post-deploy Redis/ClickHouse/API/browser proof still required before acceptance. | post-deploy runtime proof pending | no |
...
| `04` | `uv run ruff check src/trading/contexts/market_data src/trading/contexts/strategy apps tests`; `uv run pyright src/trading/contexts/market_data src/trading/contexts/strategy apps tests`; `uv run pytest -q tests/unit/contexts/market_data tests/unit/contexts/strategy tests/unit/apps` (`501 passed, 3 warnings`); docs index check passed | Baseline Mac Studio Redis/ClickHouse evidence collected; pre-deploy API endpoint expectedly returned `404`; post-deploy API/browser and refreshed Redis/ClickHouse proof pending. | in_progress | `04-btcusdt-market-readiness.md` | none | Runtime proof pending for Bybit futures stream and refreshed precision/min-notional rows. |
...
| `04` | main delivery pending | local commit pending | none | local gates passed; `gh --version` and `gh auth status` passed | deploy/runtime status pending | Direct main delivery and Mac Studio runtime proof required before accepted. |
...
|---|---|---|---|---|---|
| none | No active blockers recorded after Stage `03` delivery. | none | Stage `04` executor | Stage `03` accepted with main delivery, CI/deploy, Mac Studio sync, smoke, and runtime SQL proof. | yes |
