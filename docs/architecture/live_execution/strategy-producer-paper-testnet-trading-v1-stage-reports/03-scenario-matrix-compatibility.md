...
| Runtime/API boundary | pending main delivery and Mac Studio smoke |
| GitHub publish prerequisite | pending `github:yeet`/`publish-ci-deploy` preflight |
| Main delivery / origin main | pending |
| CI/deploy | pending |
| Mac Studio checkout sync | pending |
| Mac Studio deploy smoke | pending |

Stage `03` is not accepted until main delivery, CI/deploy, Mac Studio checkout sync, and runtime smoke are recorded.

## Blockers / Residual Risk

| Risk | Status | Handoff |
|---|---|---|
| Local broad pytest command with default env | Environmental/pre-existing local config issue: `IDENTITY_FAIL_FAST=true` without `KEYCLOAK_BASE_URL`; env-adjusted suite passed. | CI/main delivery must still be checked through publish flow. |
...
- Stage `04` remains blocked until Stage `03` is delivered to `main` and accepted.
