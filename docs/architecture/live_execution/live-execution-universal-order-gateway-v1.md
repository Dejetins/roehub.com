...
| `docs/architecture/identity/identity-exchange-connections-live-trading-v1.md` | Stage `03`, `07`, `08`, `09`, `11`, `14`, `16` | Обновить границу: exchange connections остаются custody/control, account state projection использует read-only credentials, profile использует `exchange_connection_id`, capital reservation использует projection, execution использует usage guard. |
| `docs/runbooks/exchange-secret-management.md` | Stage `07`, `13` или `14` | Добавить read-only account projection credential boundary, exchange config verify-only boundary и exchange-execution credential resolve boundary; доказать, что `apps/api` не decrypt. |
...
Projection sync может использовать read-only private REST/websocket calls, но
только вне hot path. `exchange-execution` order submit и risk gate читают
...
| `ExchangeAccountStateStale` | high | execution owner | account projection metrics | Projection stale дольше configured max age for live/paper readiness. | Pause live starts, проверить sync worker/exchange read-only credentials, refresh projection. |
...
| `07` | Exchange account state projection + exchange configuration guard. | Реальные read-only account sync calls, DB snapshots, stale/fresh/config-mismatch transitions, metrics и `/strategies` readiness доказывают, что risk/sizing читают локальную projection, auto-config не выполняется, mismatch fail-closed. |
