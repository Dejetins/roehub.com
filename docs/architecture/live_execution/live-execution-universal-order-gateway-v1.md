...
Статус: целевая архитектура; greenfield-граница исполнения реализована до
Stage `16` плана `roehub-self-hosted-oss-platform-v1`. Пользовательское
включение и реальные `mainnet`-операции этим статусом не разрешены.

## Реализованная greenfield-граница Stage 16

Stage `16` ввёл канонический хеш намерения по организации, счёту,
инструменту, стороне, размеру, ограничениям и ключу идемпотентности. Один и тот
же ключ с другим содержимым отклоняется как `idempotency_payload_mismatch`.

`exchange-execution` выполняет две проверки допуска: до получения права на
отправку и непосредственно перед вызовом адаптера. Для одного account они
сериализуются session-level advisory lock до создания MVCC snapshot; затем
каждая PostgreSQL-проверка читает policy, identity authority, audit и текущий
risk snapshot в одной `REPEATABLE READ` transaction. Accepted pre-submit audit
служит durable reservation order и учитывается следующими intent в дневном
лимите и exposure даже после истечения worker claim lease, пока типизированная
сверка или terminal order state не докажет безопасное освобождение. Проверяются активная Identity session,
актуальная owner membership, принадлежность организации и счёта, режим,
свежая account projection и проверенная exchange configuration, денежный
размер заявки, дневной объём, текущая account exposure, `kill-switch`, точные
risk/account/credential revisions и SHA-256 revision исполняемого адаптера.
Разрешение `mainnet` хранится в PostgreSQL, выдаётся только текущим владельцем
после `recent-auth` той же активной session, ограничено сроком, отзывается и
перестаёт действовать при изменении любой привязанной редакции или полномочий.
Изменение policy и его audit event фиксируются атомарно; удалить policy или
переписать audit запись нельзя.

Credential resolver вызывается заново непосредственно перед последней
проверкой и submit. Поэтому решение никогда не использует decrypted material,
полученный только в начале обработки: version reference свежего результата
обязан совпасть с persisted safety state.

Перед отправкой процесс атомарно захватывает и непосредственно перед submit
продлевает `submit claim`. Любая финальная запись, включая post-claim guard
rejection, требует совпадающий `submit_claim_id` и неистёкший lease; устаревший
worker не может завершить заявку после передачи claim другому процессу.
Тайм-аут, `OSError`, любой HTTP error во время submit, включая Binance
`-2013`, Bybit `retCode != 0`, невалидный успешный ответ и двусмысленная
lookup-ошибка сохраняются как `unknown`; Redis delivery не подтверждается.
Binance `-2013` трактуется как `confirmed_absent` только при отдельном status
lookup. Следующая доставка сначала
проверяет провайдера по `client_order_id` и получает один из типизированных
результатов `found|confirmed_absent|unknown`. Наличие заявки завершает сверку
без повторной отправки, а подтверждённое отсутствие допускает новую попытку
только при отдельной последующей доставке. Активный claim после перезапуска не
подтверждается и не повторяется вслепую.

Размещать заявки могут только зарегистрированные `core` или `verified`
поставщики. Обычный plugin не получает capability отправки. Все решения
шлюза, отказы, изменения политики и разрешения записываются в неизменяемый
аудит без ключей и необработанных ответов провайдера.

Revision hash нативных адаптеров и emulator вычисляется из bytes реально
загруженного Python module с namespace поставщика. Статическая строка версии
не может сохранить прежний policy digest после изменения исполняемого кода.

Нативные Binance/Bybit адаптеры остаются только `testnet`. Единственный
`mainnet`-способ, использованный для доказательства Stage `16`, — локальный
`core:exchange-emulator` без сети и внешних эффектов. Это не публичная
настройка и не разрешение реальной торговли.
