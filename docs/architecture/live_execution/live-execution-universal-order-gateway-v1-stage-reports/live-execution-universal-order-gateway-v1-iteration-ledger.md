...
Статус: Stage `02` accepted; Stage `03` locally validated; direct-main delivery pending.
...
| `03` | `LiveStrategyProfile`: mode, exchange binding, sizing, limits, readiness. | local-validated | Direct-main publish/deploy and production SQL/browser proof pending. | Передать profile schema/API, safe defaults, readiness reasons и no-live-without-recent-auth evidence. |
...
| `03` | local-validated | Added `LiveStrategyProfile` domain/service/repositories, additive `strategy_live_profiles` Alembic storage, profile API endpoints, non-secret exchange-control readiness checker, bounded readiness metric, and `/strategies` live profile panel. Local HTTP proof showed default `monitor_only`, `paper` ready, `live` without recent auth blocked as `recent_auth_required`, recent-auth live ready with eligible connection, readiness refresh, metrics and redaction. Playwright proof showed the `/strategies` profile panel on desktop and mobile with no console errors. Production SQL/API/browser proof is pending direct-main delivery. | `docs/architecture/live_execution/live-execution-universal-order-gateway-v1-stage-reports/03-live-strategy-profile.md`; local gates: `uv run ruff check src/trading/contexts/strategy apps/api tests`, `uv run pyright src/trading/contexts/strategy apps/api tests`, `uv run pytest -q tests/unit/contexts/strategy tests/unit/apps`; local Uvicorn HTTP probe; Playwright screenshots under `output/playwright/`. | Pending direct-main commit/push/CI/deploy. | No Redis, decrypt, exchange submit, paper accounting, position ownership or capital reservation exists yet. |
| `04` | planned | - | - | - | - |
| `05` | planned | - | - | - | - |
| `06` | planned | - | - | - | - |
| `07` | planned | - | - | - | - |
