...
| `03` Dispatcher and provider plug-in contract | pending | `03-dispatcher-provider-plugin-contract.md` | dispatcher lease/retry/unknown tests + fake/log provider | Implement delivery dispatcher, provider adapter interface, attempts, retries, unknown/dead-letter and metrics. Telegram adapter stays feature-flagged. | nothing for fake/log provider; host-local Telegram token only for optional local canary | none | yes |
