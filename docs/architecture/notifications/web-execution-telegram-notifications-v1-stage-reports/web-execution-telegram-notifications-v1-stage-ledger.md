...
| `01` Notifications schema/domain/ports | completed-local | `01-notifications-schema-domain-ports.md` | migration/domain/unit tests + transactional Postgres schema smoke | Added additive `notification_*` schema, domain objects and `NotificationRepository` port; focused gates and real Postgres transactional schema smoke passed locally/pre-main. | nothing | pending main/CI/macstudio delivery evidence | no |
...
| `01` | `uv run pytest -q tests/unit/contexts/notifications tests/unit/apps/migrations/test_notifications_context_sql.py`; `uv run ruff check src/trading/contexts/notifications tests/unit/contexts/notifications tests/unit/apps/migrations/test_notifications_context_sql.py`; `uv run pyright src/trading/contexts/notifications tests/unit/contexts/notifications` | `macstudio` host-local Postgres transactional disposable schema smoke created and inspected 6 `notification_*` tables plus indexes/constraints, then rolled back | completed-local | `01-notifications-schema-domain-ports.md` |
