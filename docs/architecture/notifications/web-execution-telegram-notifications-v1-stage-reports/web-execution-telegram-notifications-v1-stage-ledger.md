...
| `06` Weekly/monthly report scheduler | completed-local | `06-scheduled-reports.md` | scheduler/dedupe/report-run tests + seeded scheduler/dispatcher smoke | Added idempotent weekly/monthly portfolio report scheduler for active fake/log routes, route timezone periods, report-run dedupe, quality status and delivery rows. | nothing | awaiting commit/CI/deploy/host sync before acceptance | no |
| `07` Admin notifications and ops runbooks | pending | `07-admin-notifications-runbooks.md` | synthetic admin critical/alert/report drill | Add admin route config, admin categories, alert thresholds, metrics and runbooks. | admin recipient route through host-local config or admin UI; do not paste chat ids in docs | Stage `06 accepted` | no |
...
| `06` | `uv run pytest -q tests/unit/contexts/notifications tests/unit/apps` passed: `400 passed, 3 warnings`; prompt-scope ruff and pyright passed | Seeded scheduler smoke passed: `stage06_scheduler_smoke=ok created_runs=2 deduped_runs=2 deliveries=2 sent=2 attempts=2 report_types=portfolio_monthly,portfolio_weekly qualities=complete,complete` | completed-local | `06-scheduled-reports.md` |
