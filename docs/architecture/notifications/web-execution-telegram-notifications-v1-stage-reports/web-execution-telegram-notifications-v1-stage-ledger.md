...
| `05` Stats query service day/week/month | accepted | `05-stats-query-service-day-week-month.md` | query fixtures + partial/unavailable assertions + seeded ACL smoke + CI/deploy/host sync | Added `NotificationStatsQueryService` for portfolio/strategy/exchange day/week/month snapshots, explicit quality status, missing sources, owner filters and Telegram command rendering when injected. Implementation commit `275be8702f19b06fe03e057cd719eafff25cbaf3` is on `main`, CI/deploy passed, and `macstudio` checkout/smoke passed. | nothing | none | yes |
| `06` Weekly/monthly report scheduler | accepted | `06-scheduled-reports.md` | scheduler/dedupe/report-run tests + seeded scheduler/dispatcher smoke + CI/deploy/host sync | Added idempotent weekly/monthly portfolio report scheduler for active fake/log routes, route timezone periods, report-run dedupe, quality status and delivery rows. Implementation commit `e3d28f7811ecafd6b329d1b1f2d17cd77da4d55a` is on `main`, CI/deploy passed, and `macstudio` checkout/smoke passed. | nothing | none | yes |
| `07` Admin notifications and ops runbooks | pending | `07-admin-notifications-runbooks.md` | synthetic admin critical/alert/report drill | Add admin route config, admin categories, alert thresholds, metrics and runbooks. | admin recipient route through host-local config or admin UI; do not paste chat ids in docs | none | yes |
...
| Weekly portfolio report | `portfolio_report` | report run with weekly period, dedupe key, snapshot quality, delivery | covered-stage06-report | `test_report_scheduler_creates_weekly_monthly_runs_and_suppresses_duplicates`; `stage06_scheduler_smoke=ok ... report_types=portfolio_monthly,portfolio_weekly ... attempts=2` |
| Monthly portfolio report | `portfolio_report` | report run with monthly period, dedupe key, snapshot quality, delivery | covered-stage06-report | `test_report_scheduler_creates_weekly_monthly_runs_and_suppresses_duplicates`; `stage06_scheduler_smoke=ok ... report_types=portfolio_monthly,portfolio_weekly ... attempts=2` |
...
| `05` | `uv run pytest -q tests/unit/contexts/notifications` passed: `37 passed`; prompt-scope ruff and pyright passed; GitHub CI run `28395374117` passed for `275be8702f19b06fe03e057cd719eafff25cbaf3` | Seeded ACL stats smoke passed: `stage05_stats_smoke=ok portfolio_quality=complete strategy_quality=complete foreign_quality=unavailable ... owner_filtered=True`; deploy runs Backend `28395593695`, Web `28395593653`, App Image `28395593623` passed; `macstudio` checkout reached `275be8702f19b06fe03e057cd719eafff25cbaf3` and `smoke_prod.sh` passed | accepted | `05-stats-query-service-day-week-month.md` |
| `06` | `uv run pytest -q tests/unit/contexts/notifications tests/unit/apps` passed: `400 passed, 3 warnings`; prompt-scope ruff and pyright passed; GitHub CI run `28396303302` passed for `e3d28f7811ecafd6b329d1b1f2d17cd77da4d55a` | Seeded scheduler smoke passed: `stage06_scheduler_smoke=ok created_runs=2 deduped_runs=2 deliveries=2 sent=2 attempts=2 report_types=portfolio_monthly,portfolio_weekly qualities=complete,complete`; deploy runs Backend `28396409107`, Web `28396409103`, App Image `28396409084` passed; `macstudio` checkout reached `e3d28f7811ecafd6b329d1b1f2d17cd77da4d55a` and `smoke_prod.sh` passed | accepted | `06-scheduled-reports.md` |
...
| 2026-06-29 | `05` acceptance | Accepted Stage `05` after implementation commit `275be8702f19b06fe03e057cd719eafff25cbaf3`, green CI, deploy runs and `macstudio` checkout/smoke. Stage `06` is now allowed by ledger; Stage `06` requires no user presence before start. | `gh run view 28395374117`; `gh run view 28395593695`; `gh run view 28395593653`; `gh run view 28395593623`; `bash /opt/roehub/app/scripts/macos/smoke_prod.sh` |
| 2026-06-29 | `06` local implementation | Completed local Stage `06`: idempotent weekly/monthly portfolio report scheduler, timezone-aware completed periods, report-run dedupe, quality-status rendering, fake/log delivery rows and dispatcher attempt proof. Stage remains `completed-local` until commit/CI/deploy/`macstudio` sync evidence is recorded. | `06-scheduled-reports.md`; `uv run pytest -q tests/unit/contexts/notifications tests/unit/apps`; prompt-scope ruff/pyright; `stage06_scheduler_smoke=ok` |
| 2026-06-29 | `06` acceptance | Accepted Stage `06` after implementation commit `e3d28f7811ecafd6b329d1b1f2d17cd77da4d55a`, green CI, deploy runs and `macstudio` checkout/smoke. Stage `07` is now allowed by ledger; Stage `07` must still satisfy its admin-recipient precondition without secrets in chat. | `gh run view 28396303302`; `gh run view 28396409107`; `gh run view 28396409103`; `gh run view 28396409084`; `bash /opt/roehub/app/scripts/macos/smoke_prod.sh` |
