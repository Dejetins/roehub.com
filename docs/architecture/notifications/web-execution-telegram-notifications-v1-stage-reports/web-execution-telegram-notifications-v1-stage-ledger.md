...
| `04` Telegram binding and inbound bot commands | completed-local | `04-telegram-binding-inbound-commands.md` | bot update idempotency + command fixtures + API binding tests + synthetic command smoke | Added one-time hashed binding code flow, redacted Telegram update mapper, idempotent command handler, account API endpoints and disabled-by-default worker shell. Real Telegram `/start` smoke skipped. | Telegram bot token in host-local env only for optional real provider smoke; synthetic tests require nothing | awaiting commit/CI/deploy/host sync before acceptance | no |
...
| Day stats command | `stats_response` | synthetic `/stats today` update -> command -> stats snapshot -> response delivery | covered-stage04-command | `test_bound_command_coverage_creates_command_response_delivery`; response is explicit unavailable placeholder until Stage `05` stats service |
| Week stats command | `stats_response` | synthetic `/stats week` update -> command -> stats snapshot -> response delivery | covered-stage04-command | `test_bound_command_coverage_creates_command_response_delivery`; `stage04_telegram_smoke=ok`; response is explicit unavailable placeholder until Stage `05` stats service |
| Month stats command | `stats_response` | synthetic `/stats month` update -> command -> stats snapshot -> response delivery | covered-stage04-command | `test_bound_command_coverage_creates_command_response_delivery`; response is explicit unavailable placeholder until Stage `05` stats service |
| Strategy stats command | `stats_response` | synthetic `/strategy <id> week` update with ownership filter | covered-stage04-command | `test_strategy_and_exchange_scopes_fail_closed_when_unauthorized`; unauthorized strategy failed closed, authorized synthetic scope handled; real stats query remains Stage `05` |
| Exchange stats command | `stats_response` | synthetic `/exchange <connection> month` update with ownership filter | covered-stage04-command | `test_strategy_and_exchange_scopes_fail_closed_when_unauthorized`; unauthorized exchange failed closed, authorized synthetic scope handled; real stats query remains Stage `05` |
...
| `04` | `uv run pytest -q tests/unit/contexts/notifications tests/unit/apps/api tests/unit/apps` passed: `244 passed`; `tests/unit/apps/worker` broad worker pass also passed; prompt-scope ruff and pyright passed | Synthetic binding/command smoke passed: `stage04_telegram_smoke=ok ... code_stored_as_hash=True`; real Telegram `/start` smoke skipped | completed-local | `04-telegram-binding-inbound-commands.md` |
