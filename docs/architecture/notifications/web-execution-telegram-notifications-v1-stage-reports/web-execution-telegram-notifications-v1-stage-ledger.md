...
| `03` Dispatcher and provider plug-in contract | completed-local | `03-dispatcher-provider-plugin-contract.md` | dispatcher lease/retry/unknown/dead-letter tests + fake/log provider + composition-root smoke | Added delivery dispatcher, provider adapter interface, attempts, retries, unknown/dead-letter, suppression and metrics. Telegram adapter stays disabled by default and feature-flagged. | nothing for fake/log provider; host-local Telegram token only for later real canary | none | no |
...
| Day stats command | `stats_response` | synthetic `/stats today` update -> command -> stats snapshot -> response delivery | covered-stage02-synthetic | `notifications/stats_today` source fact -> `all` user route; real command/stats lifecycle remains Stage `04`/`05` |
| Week stats command | `stats_response` | synthetic `/stats week` update -> command -> stats snapshot -> response delivery | covered-stage02-synthetic | `notifications/stats_week` source fact -> `all` user route; real command/stats lifecycle remains Stage `04`/`05` |
| Month stats command | `stats_response` | synthetic `/stats month` update -> command -> stats snapshot -> response delivery | covered-stage02-synthetic | `notifications/stats_month` source fact -> `all` user route; real command/stats lifecycle remains Stage `04`/`05` |
| Strategy stats command | `stats_response` | synthetic `/strategy <id> week` update with ownership filter | covered-stage02-synthetic | `notifications/strategy_stats_week` source fact with `scope=strategy` -> `all` user route; real ownership query remains Stage `05` |
| Exchange stats command | `stats_response` | synthetic `/exchange <connection> month` update with ownership filter | covered-stage02-synthetic | `notifications/exchange_stats_month` source fact with `scope=exchange` -> `all` user route; real ownership query remains Stage `05` |
...
| `03` | `uv run pytest -q tests/unit/contexts/notifications tests/unit/apps` passed: `374 passed, 3 warnings`; ruff and pyright prompt-scope checks passed | composition-root in-memory repository smoke drained backlog through dispatcher providers: `claimed=3`, `sent=1`, `unknown=1`, `dead_letter=1`, `attempts=3`; no Telegram network call | completed-local | `03-dispatcher-provider-plugin-contract.md` |
