...
| `02` Source router synthetic event coverage | completed-local | `02-source-router-synthetic-event-coverage.md` | unit/integration synthetic matrix + transactional host-local Postgres disposable schema smoke | Added synthetic source router, route decisions and fake/log delivery candidates for every matrix row; local gates and `macstudio` disposable-schema DB smoke passed. | nothing | pending commit/CI/host sync acceptance | no |
