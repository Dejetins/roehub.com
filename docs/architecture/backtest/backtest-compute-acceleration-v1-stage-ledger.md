...
candidate against the current exact TP/SL baseline and rejected it after
Mac Studio A/B evidence showed no pruned candidates and service-wall
regression.

## Current Execution Handoff

Current executable state: Stage 15 TP/SL total-return early abandon is
`rejected`. The active runtime tree must not keep the Stage 15 candidate code.
No TP/SL continuation stage is unblocked by Stage 15; Stage 16 remains blocked
until a fresh TP/SL telemetry/repair plan records a different cost model and
acceptance gate. Future TP/SL work must still not restore or benchmark against
removed Stage 13/13S/13R/14/14R code, prompts, harnesses or raw evidence.
...
accepted Stage 12 compiled prefix traversal and a rejected/removed Stage 13/14
TP/SL production branch. The production default backend
...
measured job. Stage 15 is rejected; any attempt to revive Stage 15 or the
removed Stage 13/14 branch remains blocked unless a new plan records a
different cost model, acceptance rows and Mac Studio A/B gate.
...
| 15 | No accepted implementation boundary remains; exact-safe upper-bound candidate removed from active tree | Historical Mac Studio A/B evidence in `benchmark_iterations/2026-06-14_matrix_bitset_stage_15_tp_sl_early_abandon_candidate/` preserved parity but pruned `0` candidates and regressed service wall | Keep rejected; block revival unless a new plan first proves cheap reject-rate on the same TP/SL fixture and records a fresh Mac Studio A/B gate |
