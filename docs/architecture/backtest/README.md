...
- `docs/architecture/backtest/backtest-ai-configurator-assistant-v1.md` — новое целевое ТЗ для `/backtests` AI configurator assistant: один чат, LM Studio chat completions + JSON schema, backend-owned context snapshot, validator/repair, UI history, Monit/metrics и benchmark gates.
- `docs/architecture/backtest/backtest-ai-configurator-mlx-v1.md` — historical reset-документ AI configurator для `/backtests`: single-shot LM Studio prompt/blob contract выведен из active runtime path; не является source of truth для новой assistant v1 реализации.
