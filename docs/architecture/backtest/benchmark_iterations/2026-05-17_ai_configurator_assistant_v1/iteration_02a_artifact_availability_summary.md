...
Статус: accepted before direct-main delivery.
...
Direct-main delivery status before publish:

- local branch: `main`
- local gates: passed
- pre-delivery Mac Studio artifact evidence: passed
- `accepted`: true
- `next_iteration_allowed`: true
- `pushed_to_main`: false
- `macstudio_verified`: true for pre-delivery artifact evidence

Final main push, CI/deploy, and deployed runtime verification must update this section and
`iteration_02a_artifact_availability_summary.json` after `publish-ci-deploy`.
