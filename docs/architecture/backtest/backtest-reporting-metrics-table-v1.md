...
- Для grid запуска: строить отчёт минимум для всех вариантов, попавших в top-K Stage B.
...
- отчёт строится как минимум для top-K (чтобы не раздувать compute/memory на весь grid).
...
- Для grid запуска:
  - отчёт строится минимум для всех вариантов, попавших в top-K Stage B,
  - trades возвращаются только для N лучших вариантов (configurable; v1 default `top_trades_n_default=3`).
...
- “Тяжёлые” отчёты для всех Stage B вариантов (в v1 отчёты минимум для top-K).
...
### 6) Политика “не раздувать response”: trades только для N лучших

В v1 (как минимум):

- для всех top-K вариантов возвращаем метрики-таблицу;
- trades возвращаем только для `top_trades_n_default` лучших вариантов (по итоговому ранжированию),
  чтобы response оставался управляемым.
...
- Для grid запуска отчёт строится минимум для top-K; trades — только для N лучших.
...
- Риск: вычисление full equity curve и ratios для каждого top-K варианта может быть CPU-heavy на длинных периодах. Митигатор v1: строим отчёт только для top-K; для jobs (Milestone 5) — батчинг.
