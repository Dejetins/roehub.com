...
| `04` | `04-implement-no-risk-funding-adjustment.md` | accepted | No-risk funding formula, candidate-pool adjustment, net metrics, focused tests, benchmark/performance evidence on artifact-backed runtime inputs. | 2026-06-22 implementation added reusable no-risk funding calculation, net/gross summary metrics, bounded candidate-pool adjustment, requested/effective ranking metadata, unavailable-funding annotations, mark-price fallback, persistence payload fields and focused tests. Required local gates passed: `uv run ruff check src/trading/contexts/backtest tests`; `uv run pyright src/trading/contexts/backtest tests`; `uv run pytest -q tests/unit/contexts/backtest` with `383 passed`; final broad gates passed with `uv run ruff check .`, `uv run pyright`, and `uv run pytest -q -ra` with `1309 passed, 3 warnings`. Code was delivered to `main` at `75992d426b7219f33709069b0ef00ca43a4effb1` plus follow-up `de3d625fa09ce7b6dc4354c1212262d7c698704a`; GitHub CI `27978360294` and `27979671433` passed; Deploy Backend `27978625433` and `27979954820` passed; app/web workflows passed or were skipped by changes detector. Mac Studio checkout was fast-forwarded to clean `de3d625f`, `/opt/roehub/app` smoke passed, `binance/futures/BTCUSDT` artifact `slot_b` with degraded funding family was published, and Mac Studio artifact-backed benchmark measured baseline before candidate over 12 samples with nonzero funding adjustments on all top-20 results. | Accepted with residual warning: benchmark artifact funding coverage is `degraded` (`3` of `4` expected events, `missing_leading_coverage`) and mark price fallback from 1m close was used. Candidate-pool net ranking intentionally remains bounded and persists `funding_adjustment_exact_global_ranking=false`. |
...
| `04` | `main` | code commits `75992d426b7219f33709069b0ef00ca43a4effb1`, `de3d625fa09ce7b6dc4354c1212262d7c698704a` | N/A | Required local gates passed: `uv run ruff check src/trading/contexts/backtest tests`; `uv run pyright src/trading/contexts/backtest tests`; `uv run pytest -q tests/unit/contexts/backtest` with `383 passed`; `python -m tools.docs.generate_docs_index --check`. Final broad gates after mark-price fallback passed: `uv run ruff check .`; `uv run pyright`; `uv run pytest -q -ra` with `1309 passed, 3 warnings`. | GitHub CI `27978360294` and `27979671433` passed. Deploy Backend `27978625433` and `27979954820` passed; Publish App Image `27978625352` passed and later `27979954983` was skipped by changes detector; Deploy Web `27978637862` passed and later `27979954239` was skipped by changes detector. Mac Studio checkout `/Users/daniildegtyarev/Projects/roehub.com` was clean at `de3d625f`; `/opt/roehub/app` `smoke_prod.sh` passed. One-symbol artifact publish produced active `binance/futures/BTCUSDT` `slot_b`/generation `2`, manifest `be8fafdabdd25a74a89de8e3355bb8d4d8df3b01d76ef5ccfcd7ad491963aeb7`, funding hash `5e0cbb9f0c7269fd3956d3b921009161d00df0156eb7a0120cc0929452840f7c`, coverage `degraded`, rows `3/4`. Mac Studio benchmark used arity `2`, `rows_per_indicator=12`, `top_n=20`, `144` combinations, `3` warmups and `12` samples: baseline wall median `1.081 ms`, p95 `1.264 ms`; funding candidate wall median `26.947 ms`, p95 `32.123 ms`; funding adjustment median `24.553 ms`, p95 `29.192 ms`; all top-20 results had nonzero funding PnL and first result net return `0.4498944891176918` vs gross `0.43708173176763127`. | Accepted. Funding coverage is degraded and mark-price fallback warning is expected for this artifact-backed proof; TP/SL funding remains Stage `05`. |
...
| 2026-06-22 | Stage `04` is accepted after direct-main delivery and Mac Studio artifact-backed funding benchmark. | Gross and net metrics, bounded candidate-pool adjustment, requested/effective ranking metadata, mark-price fallback and persistence payload fields are on `main` through `75992d42` and `de3d625f`; CI/deploy workflows passed; Mac Studio checkout/runtime smoke passed; a real `binance/futures/BTCUSDT` funding artifact was published; and the target-host benchmark showed nonzero net-of-funding adjustment on all top-20 results while preserving gross return and bounded candidate-pool scope. |
