...
| 03A OpenBao/Vault runtime provisioning | accepted; Mac Studio runtime evidence complete; direct-main delivery in progress | `docs/architecture/identity/exchange-connections-stage-reports/03a-openbao-vault-runtime-provisioning.md` | OpenBao 2.5.4 is supervised as `com.roehub.openbao` / `roehub_openbao`, Transit key `roehub-exchange-credentials` exists, `exchange-control` encrypt succeeds, `apps/api` decrypt is denied with HTTP `403`, Prometheus `up{job="openbao"} = 1`, recovery runbook added. | None |
| 03B Transit application integration | ready to start after 03A direct-main delivery is complete | `docs/architecture/identity/exchange-connections-stage-reports/03b-transit-application-integration.md` | Application `ExchangeSecretCipher` port/adapter and fail-closed config may now integrate with the accepted Stage 3A runtime. | Direct-main delivery for Stage 3A must finish first. |
...
| 03A | `main` | TBD until direct-main commit is created | TBD | Validation passed; CI/deploy watch pending direct-main push | Mac Studio runtime accepted before push: OpenBao health healthy, ACL smoke passed, `roehub_openbao OK`, `up{job="openbao"} = 1` | No stage branch or draft PR. |
...
| 03A | None | N/A | Stage 3B can start after Stage 3A direct-main delivery completes. | Stage 3A report plus Mac Studio health, ACL, Monit and Prometheus evidence. |
| 03B | Direct-main delivery for Stage 3A must complete before Stage 3B starts. | Medium | Finish Stage 3A commit/push/CI/deploy record, then implement Transit adapter in Stage 3B. | Runtime prerequisite resolved; delivery record pending until push/CI completion. |
