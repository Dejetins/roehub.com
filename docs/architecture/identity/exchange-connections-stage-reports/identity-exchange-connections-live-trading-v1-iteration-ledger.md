...
| 03B Transit application integration | ready to start | `docs/architecture/identity/exchange-connections-stage-reports/03b-transit-application-integration.md` | Application `ExchangeSecretCipher` port/adapter and fail-closed config may now integrate with the accepted Stage 3A runtime. | Stage 3B still requires its own implementation, validation and direct-main delivery. |
| 03C exchange-control internal command API | blocked until 03B accepted | `docs/architecture/identity/exchange-connections-stage-reports/03c-exchange-control-internal-command-api.md` | Local-only `apps/api -> exchange-control` command boundary, service auth, capabilities smoke and no-direct-secret-import evidence must exist before schema/backfill and validation stages. | Stage 3B is not accepted. |
...
| 03B | `none`: public API/DTO contracts unchanged; secret operations stay behind `exchange-control`. | `none`: no credential schema/backfill yet. | `compatible-change`: `exchange-control` gains Transit secret engine config and fail-closed product startup rules. | `compatible-change`: app runtime depends on accepted Stage 3A Transit service; local fake remains test/non-production only. | Rollback removes adapter/config wiring and returns `exchange-control` to no-secret-backend state; no data rollback. |
...
| 03B | None for starting Stage 3B; Stage 3B itself is not implemented yet. | N/A | Implement Transit adapter/fail-closed config in Stage 3B and repeat ACL runtime evidence there. | Stage 3A runtime prerequisite resolved. |
| 03C | `exchange-control` has only health/metrics today; internal command API/client boundary is not accepted. | High | Run Stage 3C after Stage 3B: add local-only internal API, service auth, `apps/api` client and no-direct-secret-import evidence before Stage 4/5. | Not resolved. |
