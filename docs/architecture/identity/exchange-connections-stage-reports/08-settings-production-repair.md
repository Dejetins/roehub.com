...
|---|---|---|---|---|
| Root cause | Explain why production returned `Mutation origin is not allowed` and account settings 500s. | Mac Studio deployed copy of `infra/caddy/Caddyfile.vps` lacked `X-Forwarded-Host` and `X-Forwarded-Proto`; API logs showed missing account settings columns: `username`, `integration_key`, `autorefresh_preset`, `summary`. | Confirmed. | Active VPS `/etc/caddy/Caddyfile` must be inspected by the deploy workflow because local root SSH auth is unavailable. |
| Local repair | Keep CSRF fail-closed and repair legacy schema without manual DB edits. | Focused tests cover forwarded public same-origin, Referer-only browser mutation, and cross-origin rejection with `csrf_origin_mismatch`; `0006` now repairs legacy profile/integration table shapes idempotently. | Passed locally. | Production DB repair still needs bootstrap/deploy evidence. |
...
|---|---|---|---|---|---|
| `POST /api/ui/account/exchange-connections` returned `403` with `Mutation origin is not allowed` and `csrf_origin_mismatch`. | Browser POST carried `Referer: https://roehub.com/settings` but no visible `Origin`; backend saw the internal upstream host unless the public edge forwarded `X-Forwarded-Host` and `X-Forwarded-Proto`. | Keep CSRF logic fail-closed; add exact Referer-only test and deploy-web checks that `/api/*` Caddy config contains both forwarded headers. | `uv run pytest -q tests/unit/apps/api/test_ui_account_routes.py` | Passed inside the focused suite: Referer-only forwarded public host accepted; Referer-only cross-origin rejected with `csrf_origin_mismatch`. | Local pass; production pending. |
...
| Mac Studio deployed Caddy source copy | `ssh macstudio 'cd /opt/roehub/app && sed -n "1,80p" infra/caddy/Caddyfile.vps'` | `/api/*` block lacked `header_up X-Forwarded-Host {host}` and `header_up X-Forwarded-Proto {scheme}`. | Confirms deployed-source drift; active VPS config still pending deploy-workflow inspection. |
...
|---|---|---|
| Focused tests | `uv run pytest -q tests/unit/apps/api/test_ui_account_routes.py tests/unit/apps/web/test_app_routes.py tests/unit/apps/migrations` | Passed: `63 passed, 3 warnings`. |
...
| Config / runtime | `compatible-change` | Deploy workflow now requires pinned VPS ED25519 fingerprint and validates Caddy forwarded headers before reload. Public URLs and upstreams are unchanged. |
...
|---|---|---|
| `Mutation origin is not allowed` | Inspect active VPS Caddy `/api/*` block for `X-Forwarded-Host` and `X-Forwarded-Proto`. | Both headers are forwarded and Referer-only `https://roehub.com/settings` mutations pass. |
