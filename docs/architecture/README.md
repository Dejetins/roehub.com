...
- [Identity exchange keys storage + 2FA gate policy v2](docs/architecture/identity/identity-exchange-keys-storage-2fa-gate-policy-v2.md) — `docs/architecture/identity/identity-exchange-keys-storage-2fa-gate-policy-v2.md` — Документ фиксирует контракт ID-EPIC-03: безопасное хранение exchange API keys (storage-only) с шифрованием **всех** чувствительных полей и обязательным 2FA gate для CRUD v1.
