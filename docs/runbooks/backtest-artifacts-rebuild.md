...
- `availability_summary` diagnostics for the post-publish
  `/opt/roehub/state/backtest_artifacts/v2/availability_summary.yaml` regeneration.

## Artifact availability summary

After every successful manual publish and every scheduler cycle with at least one successful symbol,
the publisher regenerates:

```text
/opt/roehub/state/backtest_artifacts/v2/availability_summary.yaml
```

This file is the AI Configurator source of truth for available `exchange/market/symbol`, date
coverage, timeframe coverage, active slot metadata, `current.yaml`/`manifest.yaml` provenance, and
`summary_hash`.

The summary is built only from publisher-owned artifact YAML state:

1. scan symbol roots under the artifact v2 root;
2. read valid `current.yaml`;
3. read the active slot `manifest.yaml`;
4. verify manifest identity and `manifest_sha256`;
5. atomically write `availability_summary.yaml.tmp` and rename it to
   `availability_summary.yaml`.

Manual summary recovery without rebuilding artifacts:
...
On Mac Studio production runtime:
...
The command does not query ClickHouse, exchange APIs, market reference, or AI services. Missing or
corrupt `current.yaml`, missing active slot, missing/corrupt active `manifest.yaml`, and identity
mismatch exclude only that instrument from the summary.
