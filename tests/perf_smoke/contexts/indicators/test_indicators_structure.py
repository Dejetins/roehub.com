...
def _candles(*, t_size: int) -> CandleArrays:
    """
    Build deterministic dense candles payload for structure perf-smoke.
    ...
    base = np.linspace(100.0, 320.0, t_size, dtype=np.float32)
    drift = np.sin(np.linspace(0.0, 40.0, t_size, dtype=np.float32)) * np.float32(0.8)
    ...
    volume_series = np.ascontiguousarray(np.linspace(30.0, 900.0, t_size, dtype=np.float32))

    open_series[::307] = np.nan
    high_series[::307] = np.nan
    low_series[::307] = np.nan
    close_series[::307] = np.nan
    volume_series[::307] = np.nan
...
def test_indicators_structure_perf_smoke(tmp_path: Path) -> None:
    """
    Run perf-smoke for `structure.zscore` and `structure.percent_rank`.
    ...
    Returns:
        None.
    Assumptions:
        Perf-smoke validates successful execution and invariants, not strict SLA.
    Raises:
    ...
    candles = _candles(t_size=120_000)

    grids = (
        GridSpec(
            indicator_id=IndicatorId("structure.zscore"),
            params={
                "window": ExplicitValuesSpec(name="window", values=(20, 200)),
            },
    ...
    assert elapsed_by_id["structure.zscore"] > 0.0
    assert elapsed_by_id["structure.percent_rank"] > 0.0
