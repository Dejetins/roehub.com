    ...
    def __init__(
        ...
        *,
        ...
        Raises:
            None.
        Side Effects:
            Stores the last execute request objects for route assertions.
        """
        self._result = result
        self._error = error
...
def test_post_backtests_variant_report_endpoint_is_removed() -> None:
    """
    Verify legacy `POST /backtests/variant-report` endpoint is no longer registered.
    ...
    Returns:
        None.
    Assumptions:
        API contract keeps only summary launch and runtime-defaults endpoints in this router.
    Raises:
        AssertionError: If legacy endpoint path is still active.
    Side Effects:
        None.
    """
    client = _build_client(use_case=_FakeRunBacktestUseCase(result=_template_mode_response()))
