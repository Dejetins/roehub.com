...
DEFAULT_OUTPUT_ROOT = Path(
    ...
    f"{datetime.now().strftime('%Y-%m-%d')}_iteration_4_4_top_result_proxy_fill"
)
PROXY_SCORE_TOLERANCE = 1e-5


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Run Iteration 4.4 no-risk top_result_proxy_fill benchmark against "
...
def _run_matrix(
    *,
    ...
    canonical_targets = _canonical_no_risk_proxy_targets(canonical)
    ...
    for direction_mode in TARGET_DIRECTION_MODES:
        for arity in TARGET_ARITIES:
            ...
            runs.append(run)
            print(
                "arity={arity} direction={direction} proxy_fill_ratio={ratio:.3f} "
                "top_identity={identity} proxy_metadata={proxy_metadata}".format(
                    arity=arity,
                    direction=direction_mode,
                    ratio=run["ratios"][NO_RISK_TOP_RESULT_PROXY_FILL_STAGE_NAME],
                    identity=run["pass"]["top_identity"],
                    proxy_metadata=run["pass"]["proxy_metadata"],
                )
            )
...
def _run_payload(
    *,
    ...
    proxy_fill_s = float(stage_timings[NO_RISK_TOP_RESULT_PROXY_FILL_STAGE_NAME])
    target_proxy_fill_s = float(target["top_result_proxy_fill_s"])
    proxy_fill_ratio = _ratio(target_proxy_fill_s, proxy_fill_s)
    ...
    pass_payload = {
        NO_RISK_TOP_RESULT_PROXY_FILL_STAGE_NAME: proxy_fill_ratio >= ACCEPTANCE_RATIO,
    ...
    return {
        ...
        "timers": {
            ...
            NO_RISK_TOP_RESULT_PROXY_FILL_STAGE_NAME: proxy_fill_s,
        },
        "canonical_targets": {
            NO_RISK_TOP_RESULT_PROXY_FILL_STAGE_NAME: target_proxy_fill_s,
        },
        "ratios": {
            NO_RISK_TOP_RESULT_PROXY_FILL_STAGE_NAME: proxy_fill_ratio,
        },
        "top_result_identity": {
...
def _canonical_no_risk_proxy_targets(
    canonical: Mapping[str, Any],
    ...
    for raw_run in _required_sequence(canonical, "runs"):
        ...
        targets[(arity, direction_mode)] = {
            ...
            "top_result_proxy_fill_s": float(
                timers[NO_RISK_TOP_RESULT_PROXY_FILL_STAGE_NAME]
            ),
...
def _canonical_proxy_metadata(
    ...
    for raw_top in top_results:
        ...
        metadata.append((int(top["confirm_count"]), float(top["proxy_score"])))
    return tuple(metadata)


def _service_proxy_metadata(top_results: Sequence[Any]) -> tuple[tuple[int, float], ...]:
    return tuple(
        (
            int(top_result.metadata["confirm_count"]),
            float(top_result.metadata["proxy_score"]),
        )
        for top_result in top_results
    )


def _proxy_metadata_matches(
    *,
    service: Sequence[tuple[int, float]],
    canonical: Sequence[tuple[int, float]],
) -> bool:
    if len(service) != len(canonical):
        return False
    for service_item, canonical_item in zip(service, canonical):
        if int(service_item[0]) != int(canonical_item[0]):
            return False
        if abs(float(service_item[1]) - float(canonical_item[1])) > PROXY_SCORE_TOLERANCE:
            return False
    return True
...
def _build_payload(
    *,
    ...
    stage_pass = all(
        bool(run["pass"][NO_RISK_TOP_RESULT_PROXY_FILL_STAGE_NAME]) for run in runs
    )
    top_identity_pass = all(bool(run["pass"]["top_identity"]) for run in runs)
    proxy_metadata_pass = all(bool(run["pass"]["proxy_metadata"]) for run in runs)
    artifact_historical_prefix_compatible = (
        artifact_manifest_hash_matches_canonical or (top_identity_pass and proxy_metadata_pass)
    )
    ...
    return {
        "schema": "backtest_iteration_4_4_top_result_proxy_fill_v1",
        ...
        "artifact_acceptance": {
            ...
            "compatibility_evidence": (
                ...
                if artifact_manifest_hash_matches_canonical
                    ...
                    "canonical request-slice top identity and proxy metadata matched "
                    "for all 14 proxy-fill runs"
                )
            ),
        },
        "scope": {
            "risk_mode": TARGET_RISK_MODE,
            "stage_scope": "top_result_proxy_fill_only",
            "implemented_stages": [NO_RISK_TOP_RESULT_PROXY_FILL_STAGE_NAME],
        ...
        "acceptance": {
            ...
            "stage_boundaries_compared": [NO_RISK_TOP_RESULT_PROXY_FILL_STAGE_NAME],
            "requires_top_result_identity_match": True,
            "requires_proxy_metadata_match": True,
            "proxy_score_tolerance": PROXY_SCORE_TOLERANCE,
        ...
        "proxy_metadata_pass": proxy_metadata_pass,
        "pass": (
            stage_pass
            and top_identity_pass
            and proxy_metadata_pass
...
def _render_summary(*, payload: Mapping[str, Any]) -> str:
    ...
    lines = [
        ...
        "## Results",
        "",
        "| arity | direction_mode | backend | proxy_fill_s | canonical_proxy_fill_s | "
        "proxy_fill_ratio | top_results | identity | proxy_metadata | pass |",
        "|---:|---|---|---:|---:|---:|---:|---|---|---|",
    ]
    for run in payload["runs"]:
        ...
        lines.append(
            "| {arity} | `{direction}` | `{backend}` | {proxy_fill:.6f} | "
            "{target_proxy_fill:.6f} | {proxy_fill_ratio:.3f} | {top_results} | "
            "`{identity}` | `{proxy_metadata}` | `{passed}` |".format(
                arity=run["arity"],
                direction=run["direction_mode"],
                backend=run["backend"],
                proxy_fill=timers[NO_RISK_TOP_RESULT_PROXY_FILL_STAGE_NAME],
                target_proxy_fill=targets[NO_RISK_TOP_RESULT_PROXY_FILL_STAGE_NAME],
                proxy_fill_ratio=ratios[NO_RISK_TOP_RESULT_PROXY_FILL_STAGE_NAME],
                top_results=run["top_results_count"],
                identity="yes" if pass_payload["top_identity"] else "no",
                proxy_metadata="yes" if pass_payload["proxy_metadata"] else "no",
    ...
    lines.extend(
        [
            ...
            f"- Proxy metadata pass: `{'yes' if payload['proxy_metadata_pass'] else 'no'}`",
            "- Artifact hash matches canonical: "
            f"`{payload['artifact_manifest_hash_matches_canonical']}`",
            "- Artifact historical-prefix compatible: "
            f"`{payload['artifact_historical_prefix_compatible']}`",
