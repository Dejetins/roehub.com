...
const evidence = resolve(root, process.env.ROEHUB_PROOF_EVIDENCE ? `${process.env.ROEHUB_PROOF_EVIDENCE}/journey` : '.codex/delivery/evidence/roehub-backtests-client-v1/browser/S6');
...
test('integrated real configure worker results save and flag-off domain preservation', async ({ page }) => {
  ...
  phase = 'client';
  await page.screenshot({ path: resolve(evidence, 'accepted-backtests-reference.png'), fullPage: true });
...
`], { cwd: root, stdio: ['ignore', 'pipe', 'ignore'] });
  ...
  try {
    ...
    // Explicit duplicate verification is separate from the one UI save gesture.
    const duplicate = await page.request.post(savedResponse.url(), {
      headers: {'Idempotency-Key': savedResponse.request().headers()['idempotency-key']},
    });
    expect(duplicate.status()).toBe(200);
    const duplicateBody = await duplicate.json();
    expect(duplicateBody.duplicate).toBe(true);
    expect(duplicateBody.strategy.strategy_id).toBe(strategyId);
    ...
    if (process.env.ROEHUB_PROOF_STRATEGIES === 'true') {
      await page.getByRole('link', {name:'Open strategy',exact:true}).click();
      await expect(page).toHaveURL(new RegExp(`/strategies/${strategyId}`));
      await expect(page.locator('#selected-strategy-heading')).toHaveText(preservedStrategy.name);
      expect(new URL(page.url()).searchParams.get('from_job')).toBe(job.job_id);
      expect(new URL(page.url()).searchParams.get('from_variant')).toBe(variant);
      await page.reload();
      await expect(page.locator('#selected-strategy-heading')).toHaveText(preservedStrategy.name);
      await page.screenshot({path:resolve(evidence,'saved-strategy.png'),fullPage:true});
      await page.goBack();
    ...
    // Diagnose and bound the normalized resting-state check; never wait forever.
    await expect.poll(async()=>page.evaluate(()=>document.getAnimations().filter(a=>a.effect?.getTiming().iterations!==Infinity&&a.playState!=='finished').map(a=>({state:a.playState,time:a.currentTime,timing:a.effect?.getTiming(),type:a.constructor.name}))),{timeout:3000}).toEqual([]);
    ...
    phase = 'flag-off';
    const off = await page.goto(`http://localhost:${Number(process.env.ROEHUB_PROOF_PORT ?? 18480)+2}${deepLink}`);
    ...
    expect(new URL(page.url()).searchParams.get('variant')).toBe(variant);
    const oldJob = await page.request.get(`http://localhost:${Number(process.env.ROEHUB_PROOF_PORT ?? 18480)+2}/api/backtests/jobs/${job.job_id}`);
    ...
    const oldStrategy = await page.request.get(`http://localhost:${Number(process.env.ROEHUB_PROOF_PORT ?? 18480)+2}/api/strategies/${strategyId}`);
    expect(oldStrategy.status()).toBe(200);
    expect(await oldStrategy.json()).toEqual(preservedStrategy);
    const oldTrades = await page.request.get(`http://localhost:${Number(process.env.ROEHUB_PROOF_PORT ?? 18480)+2}${base}/trades`);
    ...
    await page.goto(`http://localhost:${Number(process.env.ROEHUB_PROOF_PORT ?? 18480)+2}/strategies/${strategyId}`);
    ...
    await page.goto(`http://localhost:${process.env.ROEHUB_PROOF_PORT ?? 18480}${deepLink}`);
    await expect(page.locator('[data-result-variant]')).toHaveAttribute('data-result-variant', variant);
    await expect(page.getByRole('img', { name: 'Equity', exact: true })).toBeVisible();
    ...
    if (process.env.ROEHUB_PROOF_STRATEGIES === 'true') {
      // Error injection supplements the real saved journey: persisted provenance
      // prevents hard-deleting this source in the current database contract.
      await page.route(`**/api/backtests/jobs/${job.job_id}`, route => route.fulfill({status:404,json:{}}));
      await page.goto(`/strategies/${strategyId}?${new URLSearchParams({from_job:job.job_id,from_variant:variant})}`);
      await expect(page.locator('#selected-strategy-heading')).toHaveText(preservedStrategy.name);
      await page.goBack();
      await expect(page.getByText('This job was not found or is not visible to your account.',{exact:true}).first()).toBeVisible();
      await page.goBack();
      await expect(page.locator('#selected-strategy-heading')).toHaveText(preservedStrategy.name);
    }
    writeFileSync(resolve(evidence, 'integrated-journey.json'), JSON.stringify({
      ...
      save: 201, duplicate:200, sameStrategy:true, controlledSourceNotFound:process.env.ROEHUB_PROOF_STRATEGIES === 'true', bodyless: true, starts, axe: 0,
