...
async function chooseVariant(page:Page,rank:number){
 await page.getByRole('region',{name:'Ranked variants',exact:true}).getByRole('link',{name:`Variant ${rank}`,exact:true}).click();
 await openActions(page);await openJobInfo(page);
}
...
test('real results materialization CSV provenance dedupe strategy SSR and terminal deletion',async({page})=>{
 ...
 await page.getByRole('tab',{name:'Overview',exact:true}).click();await page.getByRole('button',{name:'Equity',exact:true}).click();await expect(page.getByRole('img',{name:'Equity',exact:true})).toBeVisible();await page.screenshot({path:resolve(evidence,'real-results.png'),fullPage:true});
...
async function controlled(page:Page){await signIn(page,'/backtests');await page.route(`**/api/backtests/jobs/${id}**`,async route=>{const path=new URL(route.request().url()).pathname;const v=path.includes('/v2')?'v2':'v1';
 ...
 return route.fulfill({json:controlledJob()});});await page.goto(`/backtests/${id}?variant=v1`);await expect(page.getByRole('img',{name:'Equity',exact:true})).toBeVisible();await openActions(page);await openJobInfo(page);}
...
test('controlled lazy202 GET429, degraded empty and failed detail; tabs keyboard and late variant reads',async({page})=>{
 ...
 await page.getByRole('tab',{name:'Overview',exact:true}).focus();await page.keyboard.press('ArrowRight');await expect(page.getByRole('tab',{name:'Metrics',exact:true})).toBeFocused();await page.getByRole('tab',{name:'Overview',exact:true}).click();await page.getByRole('button',{name:'Drawdown · %',exact:true}).click();await expect(page.getByText('Result detail is materializing. Reads follow the server delay.')).toBeVisible();await expect(page.getByText(/Too many requests/)).toBeVisible({timeout:10000});await expect(page.getByRole('tabpanel').getByRole('button',{name:'Refresh results'})).toBeDisabled();await page.getByRole('button',{name:'Equity',exact:true}).click();await page.getByRole('button',{name:'Drawdown · %',exact:true}).click();await expect(page.getByRole('tabpanel').getByRole('button',{name:'Refresh results'})).toBeDisabled();await page.waitForTimeout(1000);expect(reads).toBe(2);await expect(page.getByText(/Some detail is degraded/)).toBeVisible({timeout:10000});expect(times[2]!-times[1]!).toBeGreaterThanOrEqual(2900);
 await page.getByRole('tab',{name:'Monthly statistics',exact:true}).click();await expect(page.getByRole('tabpanel').getByText('No result data available.')).toBeVisible();
 await page.route(`**/api/backtests/jobs/${id}/variants/v1/trades*`,route=>route.fulfill({status:202,json:pending('v1','failed')}));await page.getByRole('tab',{name:'Trades',exact:true}).click();await expect(page.getByText('Detail materialization failed. Job state is unchanged.')).toBeVisible();
...
test('controlled delete conflict requires authoritative read; ambiguous delete is retained',async({page})=>{
 ...
 const section=page.getByRole('region',{name:'Delete history',exact:true});const remove=section.getByRole('button',{name:'Delete history',exact:true});await remove.click();await page.keyboard.press('Escape');await expect(remove).toBeFocused();expect(deletes).toBe(0);await remove.click();await page.getByRole('button',{name:'Confirm deletion',exact:true}).click();await expect(remove).toBeDisabled();await section.getByRole('button',{name:'Check deletion outcome'}).click();await expect(remove).toBeEnabled();await remove.click();await page.getByRole('button',{name:'Confirm deletion',exact:true}).click();await expect(section.getByText(/Deletion outcome is unknown/)).toBeVisible();await section.getByRole('button',{name:'Check deletion outcome'}).click();await expect(remove).toBeDisabled();await expect(page).toHaveURL(new RegExp(`/backtests/${id}`));expect(deletes).toBe(2);expect(reads).toBe(2);writeFileSync(resolve(evidence,'controlled-delete.json'),JSON.stringify({controlled:true,deletes,reads,conflictFreshRead:true,ambiguousContextRetained:true,noRetry:true}));
});
test('controlled results RU EN widths focus and axe',async({page})=>{
 await controlled(page);const captures=[];for(const locale of ['ru','en']){await page.getByRole('link',{name:locale==='ru'?'Русский':'English',exact:true}).click();await expect(selected(page)).toBeVisible();for(const width of [820,1024,1440]){await page.setViewportSize({width,height:1100});await expect(page.getByRole('img')).toBeVisible();await expect(page.getByRole('img')).toHaveAccessibleDescription(locale==='ru'?/Наведите/:/Hover for values/);await expect.poll(()=>page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBe(true);const axe=await new AxeBuilder({page}).analyze();expect(axe.violations).toEqual([]);await page.screenshot({path:resolve(evidence,`${locale}-${width}.png`),fullPage:true});captures.push({locale,width,axe:0,overflow:false});}}
...
test('controlled 202 then forbidden or missing detail stops polling across tab remount',async({page})=>{
 await controlled(page);for(const status of [403,404]){let reads=0;await page.route(`**/api/backtests/jobs/${id}/variants/v1/drawdown*`,route=>{reads++;return reads===1?route.fulfill({status:202,json:pending()}):route.fulfill({status,json:{error:{code:'restricted'}}});});await page.getByRole('button',{name:'Drawdown · %',exact:true}).click();await expect(page.getByRole('tabpanel').getByRole('alert')).toBeVisible({timeout:10000});await page.waitForTimeout(2500);expect(reads).toBe(2);await page.getByRole('button',{name:'Equity',exact:true}).click();await page.getByRole('button',{name:'Drawdown · %',exact:true}).click();await page.waitForTimeout(2500);expect(reads).toBe(2);await page.unroute(`**/api/backtests/jobs/${id}/variants/v1/drawdown*`);if(status===403)await page.reload();await openActions(page);await openJobInfo(page);}
});
...
test('controlled pending result401 closes private surface before malformed response and stops reads',async({page})=>{
 await controlled(page);let reads=0;await page.route(`**/api/backtests/jobs/${id}/variants/v1/drawdown*`,route=>{reads++;return reads===1?route.fulfill({status:202,json:pending()}):route.fulfill({status:401,body:'not-json'});});await page.getByRole('button',{name:'Drawdown · %',exact:true}).click();await expect(page.getByRole('link',{name:'Sign in',exact:true})).toBeVisible({timeout:10000});let requests=0;page.on('request',r=>{if(new URL(r.url()).pathname.startsWith('/api/'))requests++;});await page.waitForTimeout(4000);expect(requests).toBe(0);expect(reads).toBe(2);await expect(page.locator('[data-platform-client]')).toHaveCount(0);
});
test('controlled pending then transport permits explicit read recovery without inheriting old202',async({page})=>{
 await controlled(page);let reads=0;await page.route(`**/api/backtests/jobs/${id}/variants/v1/drawdown*`,route=>{reads++;return reads===1?route.fulfill({status:202,json:pending()}):reads===2?route.abort('failed'):route.fulfill({json:series('v1','drawdown')});});await page.getByRole('button',{name:'Drawdown · %',exact:true}).click();await expect(page.getByRole('tabpanel').getByRole('alert')).toBeVisible({timeout:10000});const refresh=page.getByRole('tabpanel').getByRole('button',{name:'Refresh results'});await expect(refresh).toBeEnabled();await page.waitForTimeout(2200);expect(reads).toBe(2);await refresh.click();await expect(page.getByRole('img',{name:'Drawdown · %',exact:true})).toBeVisible();expect(reads).toBe(3);
});
test('controlled delete403 closes selected private results and stops their pending reads',async({page})=>{
 await controlled(page);let reads=0;await page.route(`**/api/backtests/jobs/${id}/variants/v1/drawdown*`,route=>{reads++;return route.fulfill({status:202,json:pending()});});await page.getByRole('button',{name:'Drawdown · %',exact:true}).click();await expect(page.getByText('Result detail is materializing. Reads follow the server delay.')).toBeVisible();await page.route(`**/api/backtests/jobs/${id}`,route=>route.request().method()==='DELETE'?route.fulfill({status:403,json:{error:{code:'forbidden'}}}):route.fulfill({json:controlledJob()}));await page.getByRole('button',{name:'Delete history',exact:true}).click();await page.getByRole('button',{name:'Confirm deletion',exact:true}).click();await expect(page.locator('.job-detail').getByText(/You do not have access/)).toBeVisible();await expect(selected(page)).toHaveCount(0);await page.waitForTimeout(3000);expect(reads).toBe(1);
});
...
test('controlled result429 without hints or with short hint shares manual and remount cooldown',async({page})=>{
 ...
 for(const hint of [null,1]){
  if(hint!==null)await page.reload();await openActions(page);await openJobInfo(page);
  ...
  await page.getByRole('button',{name:'Equity',exact:true}).click();await page.getByRole('button',{name:'Drawdown · %',exact:true}).click();
