...
export default defineConfig({
  ...
  reporter: [['list']],
  use: { baseURL: `http://localhost:${process.env.ROEHUB_PROOF_PORT ?? '18480'}`, browserName: 'chromium',
    viewport: { width: 1440, height: 1000 }, trace: 'off', screenshot: 'off' },
  webServer: {
    command: '.venv/bin/python -m tools.qa.backtests_client_fixture',
    cwd: '../..',
    url: `http://127.0.0.1:${process.env.ROEHUB_PROOF_PORT ?? '18480'}/health/ready`,
    timeout: 180_000,
