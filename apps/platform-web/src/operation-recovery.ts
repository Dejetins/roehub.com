...
/** A definitive rejection ends this attempt; transport/unknown failures retain its identity. */
export function releaseRejectedOperation(subject:string,strategy:string,error:unknown){
 if(!(error instanceof ApiError)||error.outcome!=='failed')return false;
 releaseOperation(subject,strategy);return true;
}
