...
function initJobDetailsPage(pageRoot) {
  ...
  const buildSummaryMetricsNode = (summaryMetricsRaw) => {
    const summaryMetrics = asRecord(summaryMetricsRaw);
    const metrics = Object.keys(summaryMetrics)
      .sort(compareStableStrings)
      .map((metric) => `${metric}: ${formatValue(summaryMetrics[metric])}`);
    if (metrics.length === 0) {
      const empty = document.createElement("p");
      empty.className = "muted-text";
      empty.textContent = "No summary metrics.";
      return empty;
    }
  ...
  const renderTopRows = () => {
    ...
    state.topRows.forEach((rawRow) => {
      ...
      summaryCell.appendChild(buildSummaryMetricsNode(rowData.summary_metrics_json));
