...
from apps.scheduler.market_data_scheduler.wiring.modules import build_market_data_scheduler_app


def _configure_logging() -> None:
    """
    Configure process-wide logging defaults for scheduler process.
...
def _build_parser() -> argparse.ArgumentParser:
    """
    Build CLI parser for market-data scheduler process.
    ...
    - None.
    """
    parser = argparse.ArgumentParser(prog="market-data-scheduler")
    parser.add_argument(
        "--config",
...
async def _run_async(config_path: str, whitelist_path: str, metrics_port: int) -> int:
    ...
    Side effects:
    - Starts scheduler runtime and metrics endpoint.
    """
...
def main(argv: list[str] | None = None) -> int:
    """
    Entrypoint for market-data scheduler process.
    ...
    try:
        ...
        logging.getLogger(__name__).exception("market-data-scheduler failed")
        return 1
